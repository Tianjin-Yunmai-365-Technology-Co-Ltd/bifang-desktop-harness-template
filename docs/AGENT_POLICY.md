...
- GUI 初始化在相关非空单元测试后固定调用 `$desktop-test-gui-initialization-e2e`，先以 `verify-gui-lifecycle-contract.mjs` 验证 `tauri-plugin-single-instance` 依赖/首插件/回调、`tray-icon`、非透明 `icons/32x32.png`/配置引用、从 `.setup` 可达的 Menu/icon/build/关闭事件接线、稳定 ID、双语资源和五个命名回归，再验证调试二进制可构建并双启动、第二次启动退出并唤醒既有主窗口、单一长期应用主进程/主窗口、真实托盘存在可见非空图形且恰好包含显示/退出、关闭只隐藏且进程继续、左键和显示菜单项恢复并聚焦、退出菜单项结束进程，以及收起侧栏 Logo/全部图标居中和全部渲染菜单页面可达。它不询问 E2E 选择、不改写本文件，也不产生发布候选或 Verification；任一失败、唯一性无法判定、只能定位空白点击区域或无法观察都阻断初始化。
