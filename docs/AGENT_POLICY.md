...
含 GUI 的一次性初始化 E2E 是脚手架完成门禁，不属于 `milestone_e2e` 管理的发布候选 E2E。无论持久值为何，初始化器都必须在 GUI 唯一基线提交前先对强制单实例和系统托盘做结构检查，包括非透明 32px RGBA 图标/配置引用与从 `.setup` 可达的 Menu/icon/build/关闭事件接线；再双启动一次真实本机调试二进制，证明第二次启动只唤醒同一主窗口后退出且只剩一个长期应用主进程/主窗口，并实测状态栏/通知区域存在可见非空托盘图形、精确菜单、关闭隐藏、两种恢复与退出生命周期。非 GUI 初始化不执行该门禁。宿主无法判定进程/窗口唯一性、只能定位透明点击区域或无法观察/操作真实托盘时，也不能用持久 E2E 偏好跳过该门禁。
...
- GUI 初始化在相关非空单元测试后固定调用 `$desktop-test-gui-initialization-e2e`，先以 `verify-gui-lifecycle-contract.mjs` 验证 `tauri-plugin-single-instance` 依赖/首插件/回调、`tray-icon`、非透明 `icons/32x32.png`/配置引用、从 `.setup` 可达的 Menu/icon/build/关闭事件接线、稳定 ID、`rust-i18n` 标签解析、双语资源和八个有断言命名回归，再验证调试二进制可构建并双启动、第二次启动退出并唤醒既有主窗口、单一长期应用主进程/主窗口、真实托盘存在可见非空图形、中文“显示窗口/退出”与英文“Show Window/Quit”可无重启刷新且无原始 key、关闭只隐藏且进程继续、左键和显示菜单项恢复并聚焦、退出菜单项结束进程，以及固定 `136px` 单态侧栏的 Logo/图标/文字尺寸与居中、默认设置页无隐私/统计区块和全部渲染菜单页面可达。它不询问 E2E 选择、不改写本文件，也不产生发布候选或 Verification；任一失败、唯一性无法判定、只能定位空白点击区域或无法观察都阻断初始化。
