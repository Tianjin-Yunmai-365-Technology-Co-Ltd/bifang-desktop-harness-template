...
| HTTP 服务 | Axum | 仅在产品明确批准 Rust HTTP 服务时引入；路由、提取器和响应映射属于 HTTP 边界，业务规则仍调用 core；这不恢复独立 WEB 适配器 |
...
| 可观测性 | tracing | 使用结构化 event 与 span 贯穿批准的运行路径；不得记录密钥、令牌、个人数据或未脱敏业务载荷 |
...
- 不为中性脚手架默认增加 Axum、SeaORM、tracing、anyhow、thiserror、配置框架、网络客户端、依赖注入或插件系统；对应能力获批并出现真实使用路径时，必须采用上述固定技术族并从根工作区按需加入。
...
- 已启用 tracing 时，结构化 event/span 必须同时写入本地日志文件（追加或按时间/大小滚动），不得只写标准错误或直接丢弃；日志文件格式必须便于人类直接阅读与检索，至少包含时间戳、级别、target 和事件消息；标准输出仍只用于单一 JSON 文档，日志文件不得污染标准输出，也不得记录密钥、令牌、个人数据或未脱敏业务载荷。推荐使用 `tracing-subscriber` 的可读 `fmt` layer 搭配 `tracing-appender` 之类的滚动文件 writer 实现，或等效方案（见 ADR-20260806-002）。
...
- 使用 `cargo metadata --no-deps --locked --format-version 1` 或项目保留的等价检查确认每个适配器直接依赖核心、核心不通过 workspace 依赖路径到达适配器或接口框架、适配器彼此不直接或间接依赖；Python 可用时通过当前平台可用的 Python 3 解释器运行 `.agents/skills/implement-change/scripts/check_core_first.py`，不得依赖 POSIX 可执行位；不可用时必须执行并记录等价的依赖图审查。
...
- 最小目录：职责已经无法通过当前文件清楚表达时主动拆分；无论职责判断如何，所有人工维护文本仍必须满足 `docs/ENGINEERING_RULES.md` 的 400 个物理行硬上限。

## MCP 适配器硬规则

`$add-mcp-adapter` 只有在下游用户明确批准后才增加对应适配器；以下是最小可执行硬规则，完整背景与设计依据见 `docs/harness_engineering/agent_first_design.md` §15.5（MCP 契约）、§15.6（单一数据事实来源）与 §15.8（安全与可审计性）。
