...
- 托盘生命周期固定回归必须明确命名为 `tray_show_restores_and_focuses_main_window`、`close_request_hides_without_exit` 与 `tray_quit_exits_application`；它们与两个单实例回归共同组成 GUI 初始化的五个固定命名回归。
...
- 含 GUI 的中性初始化必须在相关非空单元测试后、裁剪初始化能力和唯一基线提交前调用 `$desktop-test-gui-initialization-e2e`。它先运行 `verify-gui-lifecycle-contract.mjs` 检查单实例依赖/首插件/回调、托盘 feature、非透明 32px RGBA 图标/配置引用、从 `.setup` 可达的 Menu/icon/build/关闭事件接线、稳定 ID、中英文资源和五个固定回归，再以 `pnpm tauri build --debug --no-bundle` 构建并双启动本次真实本机调试二进制。第二次启动必须退出，既有主进程和同一主窗口持续且窗口恢复聚焦，宿主枚举只剩一个长期应用主进程/主窗口。Computer Use 还必须实测真实托盘有可见非空图形、精确双项菜单、关闭隐藏且进程继续、左键与显示项恢复聚焦、退出项结束进程并移除图标，以及主窗口可见、收起侧栏 Logo/全部图标居中和所有渲染菜单页面可达；它不消费 `milestone_e2e`，不写 release/Verification，失败、唯一性无法判定、只能定位空白点击区域或无法观察即阻断初始化。
...
- `$desktop-add-gui-adapter` 只在下游用户明确批准后增加依赖核心的 Tauri 2 桌面适配器；它自动消费三候选 Logo 选择、不可省略的固定托盘/关闭隐藏/两种恢复/退出生命周期、动态标题、图标/Tooltip 完整且默认收起的 Logo→版本侧栏、三态主题设置、全局亮暗主题、含更新入口的关于页、双主题赞助页和品牌媒体基线，不自带产品业务实现。
