...
- CLI 参数/标准流/退出码、MCP schema 注解/stdio 生命周期、TUI 焦点/选择/滚动/按键/终端恢复，以及 GUI 单实例、路由/查询/交互状态、窗口/WebView/CSP/系统托盘/通知/自动启动留在对应适配器；这些机制触发的业务动作仍调用核心，中性单实例回调只恢复窗口而不触发业务。
...
- GUI 活动选项卡、查询/筛选、排序、分页页码/每页数量及同类页面工作状态必须由应用根 Jotai store 的模块级 atom 在本次应用进程内跨路由保留；route unmount、关闭隐藏与单实例唤醒不能重置，退出后从默认值开始。页面会话 atom 不得使用 `atomWithStorage` 或浏览器/Tauri/文件/数据库/URL 持久化，不得镜像 TanStack Query 数据或 core 权威状态；语言、主题等明确批准的设备偏好继续按独立契约持久化。查询范围或每页数量变化时页码重置为 1；路由返回后只有查询成功、当前页大于 1 且结果为空时才回退第 1 页并重查，加载/错误和第 1 页空结果不得触发循环。
...
- 选择 GUI 时强制使用官方 `tauri-plugin-single-instance`：根 `[workspace.dependencies]` 声明经验证的最低兼容稳定范围，GUI member 只使用 `workspace = true`，插件必须是 Tauri Builder 注册的首个 plugin。同一用户会话第二次启动相同应用身份时，第二进程必须通知后退出，只复用 `restore_main_window` 恢复、取消最小化并聚焦既有 `main` 窗口，不得创建第二个长期应用主进程或主窗口。中性回调忽略且不记录参数/工作目录，不将其解释为业务动作；文件关联、深链或参数转发需另行批准并由核心判定。固定回归为 `single_instance_plugin_is_registered_first` 与 `second_launch_restores_existing_main_window`；真实 E2E 还必须双启动精确同一二进制并证明只剩一个长期应用主进程和同一主窗口。Linux Snap/Flatpak 渠道必须声明插件所需的会话 DBus own/talk 权限并在真实沙箱候选验证。
- 选择 GUI 时强制启用 Tauri `tray-icon` feature，系统托盘不是可选增强：托盘使用应用图标，菜单稳定 ID 恰好为 `show_window` 与 `quit`，但可见标签必须通过 `rust_i18n::t!("tray.show_window")` 与 `rust_i18n::t!("tray.quit")` 按当前规范化 locale 解析；中文精确为“显示窗口/退出”，英文精确为“Show Window/Quit”，未知 locale 回退英文，运行时语言切换无需重启即可刷新，任何 `tray.*` 原始键不可见。显示动作与主鼠标左键释放都恢复、取消最小化并聚焦主窗口，主窗口 `CloseRequested` 只 `prevent_close()` 后隐藏且进程继续，只有 `quit` 显式结束应用。结构检查、固定命名回归和真实宿主生命周期都是初始化基线门禁，不能因 Draft、宿主差异或小型应用省略；宿主无法观察托盘时也必须阻断。初始化固定建立 `{applicationName} v{version} {contactChannel}:{contactValue}` 动态标题、`136px` 单态左侧菜单和 `/settings`、`/about`、`/sponsor`；不得提供展开/折叠状态或开关。侧栏顶部始终先显示 `56px` 选中 Logo、再紧接带单个 `v` 的权威当前版本。每个功能项和固定项必须有 `30px` 图标；图标在上、名称在下，名称使用 `11px` 字号、`10em` 行内宽度、水平居中和最多两行，并始终保留完整可访问名称，不得使用 Tooltip-only 名称。功能从顶部向下增长，底部固定为赞助、设置、关于。主应用窗口在 `app.windows` 中使用 `width: 1440`、`height: 900`、`minWidth: 960`、`minHeight: 640`、`center: true`、`preventOverflow: true`，默认尺寸可同时容纳固定侧栏和三张赞助档位卡。Mantine 根使用 `defaultColorScheme="auto"` 并通过唯一主题模块同时定义亮色/暗色的页面背景、surface、主/次文字、边框与强调色；设置页只提供应用/版本、中英文和浅色/深色/跟随系统三态并使用设备级本地存储持久化，不得包含隐私标题、统计同意、未配置占位或对应固定翻译键；赞助页以 `useComputedColorScheme` 选择运行时有效主题的背景叠层、surface 与对比色。关于页的“检查更新”旁固定提供元素自身绑定的“更新日志”按钮，从打包的根 `release-notes.json` 事实查看最近 5 个版本；每版“功能优化”和“问题修复”各最多 10 条，发布日期和版本标题采用固定结构，全部可见版本只带一个小写 `v`。远程更新能力默认禁用，检查按钮未配置时为 `NotConfigured`/禁用且零出站，本地更新日志按钮仍可用。关于页还显示作者、作者联系方式与三段免责声明，赞助页打包完整 `media/sponsor/*`。真实更新启用时才引入官方 Tauri updater，制品签名验证不可关闭；adapter 认证最低支持版本策略后交给 core 作严格 SemVer 强更判定，根级强更门只允许安装或退出。只有产品明确启用统计时才另建产品级明确同意界面，统计仅在同意后由 Rust adapter 以 HTTPS JSON POST 发送固定 `app_started` 最小字段，撤回即取消请求和清空有界内存队列。模板不提供固定 endpoint、服务端 secret、发布私钥或统计实例；任务必须受应用生命周期拥有并回收，失败默认 fail-open。
...
- 含 GUI 的中性初始化必须在相关非空单元测试后、裁剪初始化能力和唯一基线提交前调用 `$desktop-test-gui-initialization-e2e`。它先运行 `verify-gui-lifecycle-contract.mjs` 检查单实例依赖/首插件/回调、托盘 feature、非透明 32px RGBA 图标/配置引用、从 `.setup` 可达的 Menu/icon/build/关闭事件接线、稳定 ID、`rust-i18n` 标签解析、中英文资源和八个有断言固定回归，再以 `pnpm tauri build --debug --no-bundle` 构建并双启动本次真实本机调试二进制。第二次启动必须退出，既有主进程和同一主窗口持续且窗口恢复聚焦，宿主枚举只剩一个长期应用主进程/主窗口。Computer Use 还必须实测真实托盘有可见非空图形、中文“显示窗口/退出”与英文“Show Window/Quit”可无重启刷新且无 `tray.*` 原始键、关闭隐藏且进程继续、左键与显示项恢复聚焦、退出项结束进程并移除图标，以及主窗口可见、固定单态侧栏尺寸/排列/居中、默认设置页无隐私区块和所有渲染菜单页面可达；它不消费 `milestone_e2e`，不写 release/Verification，失败、唯一性无法判定、只能定位空白点击区域或无法观察即阻断初始化。
...
- `$desktop-add-gui-adapter` 只在下游用户明确批准后增加依赖核心的 Tauri 2 桌面适配器；它自动消费三候选 Logo 选择、不可省略的固定托盘/运行时 i18n/关闭隐藏/两种恢复/退出生命周期、动态标题、`136px` 单态图标上文字下的 Logo→版本侧栏、无隐私/统计区块的语言与三态主题设置、全局亮暗主题、含更新入口的关于页、双主题赞助页和品牌媒体基线，不自带产品业务实现。
...
- `$desktop-test-gui-initialization-e2e` 只在含 GUI 的一次性初始化提交前运行托盘结构检查和真实本机调试二进制，托盘生命周期任一缺失或无法观察都阻断；通过后随初始化能力删除，升级不得把它重新注入终端下游。
