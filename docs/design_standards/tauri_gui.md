...
- 未命中列表页组件标准的普通页面，其活动选项卡、查询/筛选、排序和分页由应用根 Jotai store 在当前进程内跨路由保留，不写入 Web Storage、URL 或 core，也不镜像 TanStack Query 数据；只有成功查询到大于第 1 页的空页才回第 1 页。命中 [Mantine 列表页设计标准](mantine_list_view.md) 时改用其更具体的 URL/sessionStorage 查询恢复、Query-only 行数据、localStorage 列偏好和越界末页规则。window-state 始终只持久原生窗口几何。
...
- 普通页面会话状态用同一根 store 验证跨路由保留，用新 store 验证真正退出后恢复默认，并覆盖成功空页回第 1 页与错误不回退；列表页改按 [Mantine 列表页设计标准](mantine_list_view.md) 的最小回归验证。独立 window-state 回归覆盖尺寸/位置/最大化恢复、忽略可见性、离屏回退和首次启动默认，不把窗口持久化当作页面状态存储。
