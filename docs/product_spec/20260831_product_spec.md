...
> 初次批准日期：2026-07-21
>
> 最近范围确认：2026-08-31（Harness 源请求边界、Git 即时提交设置及 Logo 候选先选后处理与稳定预览顺序分别见 ADR-20260831-003、ADR-20260831-002、ADR-20260831-001；此前仍有效决定已综合保留）
...
- 输入：当前 Harness 源只接收 Harness 工程维护信息，或创建终端下游所需的中文项目展示名、英文项目展示名（至少直接提供一个）、ASCII `snake_case` 标识、项目路径、负责人、目标平台、接口组合和持久 Agent 策略；项目路径可以是最终根目录或父目录。选择 GUI 时还包括四项能力的启用/禁用与可选的精简/详细侧栏选择，省略侧栏选择表示使用详细模式。产品目的、业务规则、产品专属 UI/文案/数据、远程地址、凭据、产品构建与发布不是 Harness 源输入，必须在初始化完成并切换到终端下游后重新提出。
...
- 变更标识：`HARNESS-FEAT-JUST-IN-TIME-GIT-COMMIT-SETUP`；所需 Harness 版本：`202608281139`（当前未发布时间版本，本范围不自动改变 `Version.md`）。
- Git 作者身份、版本、独立仓库边界与仓库本地提交模板只在用户明确要求配置 Git，或已授权工作流的下一项动作确实是运行 `git commit` 时检查和设置。打开仓库、读取状态、诊断、编辑、测试、脚手架生成或未来可能提交均不触发 `$desktop-configure-git-commits`。
- 新下游在表单、复制、身份改写、环境门禁、脚手架编写、测试和一次性裁剪阶段不得检查 Git 可用性/身份/配置或初始化仓库；全部检查成功、下一步就是唯一基线提交时，才即时建立独立 `main` 仓库、运行仓库本地 `install`/`check` 并提交。保护用户修改所需的只读根、分支和状态检查不属于提交配置动作。
...
- 基础字段全部解析后才进入条件阶段。推荐策略一次确认后物化既有四字段；选择自定义时四个策略值每轮补全一个。选择 GUI 时，系统托盘、关于页、赞助页、单实例和侧栏模式同样每轮补全一个；四项能力必须明确为 `enabled` 或 `disabled`，侧栏明确跳过时采用 `detailed`，显式非法值重新询问。产品目的、核心输入输出、成功标准、风险、副作用和发布事实不属于中性实例化输入；即使用户同时给出也要拒绝并要求在终端下游重提。
...
- 下游必须在全部初始化验证与裁剪成功、下一步实际创建基线提交时才检查 Git、建立独立仓库并配置提交模板；`git rev-parse --show-toplevel` 精确等于项目根，初始分支为 `main`，无远端，并只创建一个本地基线提交。
...
- [x] Git 可用性、身份、独立仓库和仓库本地提交模板不会因打开、编辑、测试或脚手架阶段提前检查/设置；只有明确配置请求或下一步实际提交时才即时执行，新下游基线提交仍保持独立 `main`、无远端和单提交。
