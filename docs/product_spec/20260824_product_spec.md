...
> 初次批准日期：2026-07-21
>
> 最近范围确认：2026-08-24（日常开发与显式构建流程收敛见 ADR-20260824-001；此前仍有效的品牌、GUI、Rust、文件治理、异步、日志、真实验收与 i18n 决定已综合保留）
...
- E2E 不与单元测试或编译混跑。选择启用或产品/渠道硬要求时，只在完整最终候选存在后对该候选运行；选择禁用时记录 `Not run` 和剩余风险。E2E、签名、发布与人工复核结果仍不得由一次构建成功推断。
...
- 不允许在完整最终候选形成前执行 E2E，也不允许用部分通过结果宣称项目已验收。
...
- 不在普通编码、单元测试、编译、打包、制品收集或发布元数据命令中混跑冒烟/E2E；显式启用的 E2E 只针对已形成的最终候选。
