...
> 初次批准日期：2026-07-21
>
> 最近范围确认：2026-08-25（GUI 初始化强制单实例、首插件与真实双启动唯一性门禁见 ADR-20260825-007；GUI 系统托盘不可省略及结构 + 真实生命周期双门禁见 ADR-20260825-006；GUI 初始化真实本机 E2E、收起侧栏居中与 `@tabler/icons-react` 固定图标库见 ADR-20260825-005；GUI 更新入口、三态主题、全局亮暗语义主题与折叠菜单图标/Tooltip 见 ADR-20260825-004；Rust/前端/受管工具的最低兼容版本范围见 ADR-20260825-003；GUI 初始化 Logo 三选一、侧栏 Logo→版本/默认收起、赞助双主题与主窗口尺寸见 ADR-20260825-002；固定侧栏、更新/强更/统计契约见 ADR-20260825-001；此前仍有效决定已综合保留）
...
- 含 GUI 的下游在初始化单元测试通过后、裁剪初始化能力和创建唯一基线提交前，必须固定执行一次 `$desktop-test-gui-initialization-e2e`：先以 `verify-gui-lifecycle-contract.mjs` 检查单实例依赖/首插件/回调、托盘 feature、非透明 32px RGBA 图标/配置引用、从 `.setup` 可达的 Menu/icon/build/关闭事件接线、稳定 ID、双语资源和五个固定命名回归，再以 `pnpm tauri build --debug --no-bundle` 生成并双启动本次真实本机调试二进制。第二次启动必须自行退出，既有主进程与同一主窗口持续且窗口恢复、取消最小化并聚焦，宿主枚举只剩一个长期应用主进程/主窗口。Computer Use 还必须实测真实托盘存在可见非空图形、菜单恰好只有本地化显示/退出、原生关闭只隐藏且进程继续、左键与显示项均恢复聚焦、退出项结束进程并移除图标，同时验证主窗口可见、收起侧栏 Logo/全部图标水平居中、可访问树中所有当前渲染菜单页面均可达。它独立于 `milestone_e2e`，失败/超时/取消/唯一性无法判定/只能定位空白点击区域/无法观察/无法执行均阻断初始化；它不生成 release 候选、不写 Verification，也不替代最终候选 E2E。
...
- 除固定 GUI 初始化 E2E 外，不允许在完整最终候选形成前执行 E2E；初始化 E2E 只能证明当前宿主的调试脚手架可构建并满足单实例双启动唯一性、强制托盘完整生命周期、侧栏居中和菜单页面可达，不得用它或其他部分通过结果宣称项目已验收。
...
- [x] GUI 初始化实际生成 3 个 1024×1024 Logo 候选并由用户选择，选中母版可追溯到运行时 Logo 与平台图标；强制建立使用应用图标、稳定 ID 恰好为 `show_window`/`quit` 的双语托盘，以及关闭隐藏、两种恢复聚焦与退出生命周期，并以结构检查、三个命名回归和真实宿主 E2E 三层证据阻止托盘再次丢失。初始化同时建立权威动态标题和默认收起的左侧菜单，Logo 永远在顶部且当前版本紧随其下，展开/折叠均显示二者。每个功能/固定菜单项都有图标；折叠时显示图标和 Tooltip 名称，展开时显示图标与名称。主应用窗口默认 1440×900、最小 960×640并与 660×400 DMG 安装卷窗口独立；应用根同时提供完整亮色/暗色语义主题，设置页可选浅色、深色、跟随系统并持久化，赞助页在同一产物中分别适配亮色/暗色。功能项从顶部向下，底部固定项的视觉顺序为赞助、设置、关于；固定 `/settings`、`/about`、`/sponsor` 路由，设置页提供中英文切换与默认关闭的统计同意，关于页提供 `NotConfigured` 检查更新并显示作者/联系人/免责声明，赞助页默认打包完整 sponsor 媒体。
- [x] GUI 固定直接依赖 `@tabler/icons-react`，适用图标使用命名组件；默认收起侧栏的 Logo 与全部渲染图标水平居中且无裁切，图表周边图标优先使用 Tabler，图表绘制能力保持独立。
- [x] 含 GUI 的下游在唯一基线提交前固定通过单实例/托盘结构门禁和一次真实本机调试二进制 E2E，证明当前项目可编译并双启动、第二次启动退出且同一主进程/主窗口被唤醒、只剩一个长期应用主进程/主窗口、真实托盘精确菜单及关闭隐藏/恢复/退出生命周期可操作、收起侧栏居中、全部渲染菜单页面可达；任一失败或宿主无法判定/观察都阻断，该结果不冒充发布候选或完整验收。
...
- 维护状态：Active。
- 未来候选：至少两个真实下游的 Harness 升级前向证据、策略解析器跨平台封装、TUI/MCP 与 Linux GUI 的统一构建产物/签名清单、Tauri xwin/Keychain profile/最终 DMG Finder 布局的真实前向构建证据、宿主级 Worktree 写入强制、依赖供应链维护 Skill，以及首次真实 GUI 下游对单实例双启动、托盘/关闭隐藏、动态标题、固定侧栏/设置/关于/赞助页、签名更新安装、强更离线恢复、统计同意/撤回、Vite/AST 门禁和最终 dist 扫描的前向构建证据。
