...
> 初次批准日期：2026-07-21
>
> 最近范围确认：2026-08-25（GUI 初始化托盘运行时 i18n、`136px` 单态图标上文字下侧栏与无隐私/统计区块设置页见 ADR-20260825-008；GUI 初始化强制单实例、首插件与真实双启动唯一性门禁见 ADR-20260825-007；GUI 系统托盘不可省略及结构 + 真实生命周期双门禁见 ADR-20260825-006；GUI 初始化真实本机 E2E 与 `@tabler/icons-react` 固定图标库见 ADR-20260825-005；GUI 更新入口、三态主题与全局亮暗语义主题见 ADR-20260825-004；Rust/前端/受管工具的最低兼容版本范围见 ADR-20260825-003；GUI 初始化 Logo 三选一、Logo→版本、赞助双主题与主窗口尺寸见 ADR-20260825-002；固定侧栏、更新/强更/统计契约见 ADR-20260825-001；此前仍有效决定已综合保留）
...
- [x] GUI 初始化实际生成 3 个 1024×1024 Logo 候选并由用户选择，选中母版可追溯到运行时 Logo 与平台图标；强制建立使用应用图标、稳定 ID 恰好为 `show_window`/`quit` 且可见标签由 `rust-i18n` 运行时解析的双语托盘，以及关闭隐藏、两种恢复聚焦与退出生命周期，并以结构检查、六个托盘生命周期/i18n 命名回归和真实宿主 E2E 三层证据阻止托盘再次丢失或泄漏原始翻译键。初始化同时建立权威动态标题和 `136px` 单态左侧菜单，`56px` Logo 永远在顶部且当前版本紧随其下。每个功能/固定菜单项都有 `30px` 图标，并以图标在上、`11px` 文字在下的方式在 `10em` 宽度内居中显示；没有展开/折叠控件。主应用窗口默认 1440×900、最小 960×640并与 660×400 DMG 安装卷窗口独立；应用根同时提供完整亮色/暗色语义主题，设置页可选浅色、深色、跟随系统并持久化，赞助页在同一产物中分别适配亮色/暗色。功能项从顶部向下，底部固定项的视觉顺序为赞助、设置、关于；固定 `/settings`、`/about`、`/sponsor` 路由，设置页提供中英文切换且不包含隐私/统计区块，关于页提供 `NotConfigured` 检查更新并显示作者/联系人/免责声明，赞助页默认打包完整 sponsor 媒体。
