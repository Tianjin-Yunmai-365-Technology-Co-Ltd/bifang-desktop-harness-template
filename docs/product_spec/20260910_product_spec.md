...
为以 AI Agent 为第一消费者、人类负责方向和关键审批的跨平台收费小工具，提供一个不实现具体业务、可一次性建立并持续维护终端下游项目的 Harness；用户可见 Task 自动拆分默认关闭并可手动开关，启用后一个 Task 只承载一个固定、可验收结果，Git Task 使用独立 Worktree，非 Git Task 使用 Local。
