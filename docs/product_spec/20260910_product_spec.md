...
- 序号修复标识：`HARNESS-FIX-PROJECT-TASK-SEQUENCE-AUTO-INCREMENT`；所需 Harness 版本：`pending`，等待下一次 Harness 时间版本发布物化。本条取代按新 Session/当前协调批次从 1 重置的分配子句。
- 所有拥有会话标题的普通当前 Session、Worktree/Local 左侧 Task 与内部 Subagent/agent thread 使用唯一格式 `{序号}|{Task简述}|{当前进度} |{功能摘要}`。四段使用 ASCII `|`，进度后固定恰好一个 ASCII 空格再接第三个分隔符；Task 简述与功能摘要非空、不含 `|`、必须单行且首尾无空白。序号是无前导零的正整数，同一结果复用。用户可见的新独立结果按同一 `hostId` 与精确 `projectId` 合并当前、pinned 和归档 Codex Task，只从合规标题提取序号并取最大有效序号加 1；空历史才从 1 开始，缺号不回填，同项目批量派发连续预留且不按批次重置。隐藏 Subagent 不占用项目序列，只在父 Task 内使用稳定批次序号。任何序号都不能来自 `threadId`/`clientThreadId`、Subagent 技术 `task_name`、Git ref 或列表顺序。进度只能为 `已分配`、`运行中`、`检查中`、`已完成`。
