...
> 初次批准日期：2026-07-21
>
> 最近范围确认：2026-08-28（新下游逐字段初始化表单与父目录/最终根路径解析规则由项目负责人本次明确确认；左侧 Task 的独立 Worktree、`codex/*` 分支、描述模板、提交与主任务整合见 ADR-20260828-001；UI 设计标准目录与 `80px` compact 标准见 ADR-20260827-003；GUI 初始化能力专门问询、可选托盘/单实例/关于页/赞助页、精简/详细侧栏及侧栏未选时默认详细见 ADR-20260827-001；此前仍有效决定已综合保留）
...
- 决策场景：创建新下游时先完成写入前初始化表单。Agent 复用用户已明确给出的合法值，每轮只询问一个尚未解析的字段；不得用一次笼统问询代替展示名、标识、路径、负责人、平台、接口、策略和适用 GUI 配置的逐项收集。全部字段收齐后展示包含最终项目根目录的完整汇总并确认。显式构建时必须为当前构建解析一次 E2E 选择，持久 `milestone_e2e` 仅提供建议默认值，不能替代本次选择。
...
- 变更标识：`HARNESS-FEAT-STEPWISE-INSTANTIATION-FORM-PATH-RESOLUTION`；所需 Harness 版本：`202608051301`（当前未发布时间版本，本范围不自动改变 `Version.md`）。
- `$desktop-instantiate-project` 在任何目录创建、复制、环境安装或 Git 初始化前维护初始化表单。字段固定覆盖项目展示名、ASCII `snake_case` 标识、项目路径、负责人、目标平台、接口组合、推荐/自定义 Agent 策略，以及 GUI 被选择时的系统托盘、关于页、赞助页、单实例和侧栏模式。用户已给出的合法字段直接复用；每轮只询问一个最靠前的未解析字段，非法值只重问该项。
- 推荐策略一次确认后物化既有四字段；自定义策略的四个值继续逐项收集。GUI 四项能力必须明确为 `enabled` 或 `disabled`，侧栏明确跳过时采用 `detailed`，显式非法值重新询问。产品目的、核心输入输出、成功标准、风险、副作用和发布事实不属于中性实例化必填项。
...
- `$desktop-instantiate-project` 只执行一次，并通过写入前分步表单收齐展示名、ASCII `snake_case` 标识、项目路径、负责人、目标平台、接口、Agent 策略和适用 GUI 配置。项目路径末级与标识精确一致时直接作为最终根，否则追加标识；只有解析后的最终根必须不存在或为空。用户确认后该目录是初始化、开发、验证和发布准备的唯一项目根目录。
...
- [x] 新下游创建在首次写入前逐项完成初始化表单，每轮只询问一个缺失字段；项目路径末级与标识精确一致时直接使用，否则固定追加标识，非空门禁只作用于最终项目根目录。
