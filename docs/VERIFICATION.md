...
- 唯一的前候选例外是含 GUI 下游的一次性初始化 E2E：相关非空单元测试通过后，`verify-gui-lifecycle-contract.mjs` 先读取 `docs/GUI_APP_PROFILE.md` 的五项配置，对启用能力检查完整依赖、运行时、资源与有断言回归，对禁用能力检查无残留，再以 `pnpm tauri build --debug --no-bundle` 构建真实本机调试二进制。只在单实例启用时执行双启动唯一性场景，只在托盘启用时验证真实非空托盘图形、中英文标签无重启刷新、关闭隐藏、两种恢复与退出；托盘禁用时验证关闭最后窗口退出。Computer Use 始终验证所选精简/详细侧栏、默认设置页无隐私/统计区块、全部实际菜单页面可达与未选页面缺席。它不消费 `milestone_e2e`，不写本索引或证据分卷，不产生 release/完整验收结论；任一适用场景失败、无法判定、只能定位空白点击区域或宿主无法观察都会阻断初始化基线提交。
...
| GUI 初始化 E2E | `verify-gui-lifecycle-contract.mjs` + `$desktop-test-gui-initialization-e2e` + Computer Use | 只在含 GUI 的一次性初始化提交前读取五项 profile，启用能力必须完整、禁用能力必须无残留；单实例启用才双启动，托盘启用才验证真实托盘和关闭隐藏/恢复/退出，托盘禁用则验证关闭最后窗口退出；所有组合验证所选侧栏、默认设置页、实际菜单页面与未选页面缺席并回收本 Skill 拥有的进程。适用场景无法判定、只能定位空白点击区域、无法观察或任一失败都阻断，不作为候选验收证据 |
