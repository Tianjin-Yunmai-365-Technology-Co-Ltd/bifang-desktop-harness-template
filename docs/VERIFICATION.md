...
- 构建请求、执行和结果，以及候选 E2E、完整验收、`pending` → `accepted` 状态更新和发布就绪复核，都不得创建或更新本索引、`docs/verification/` 或其他受 Git 跟踪的项目记忆；它们必须让当前分支保持 clean 的动态默认主分支，且 `HEAD == sourceCommit`、本地/远端主分支和远端版本 tag 指向同一提交，把全量单元测试、产物、摘要、签名状态、选择、人工签署和可观察结果仅写入忽略的 `release/` 原子候选集合、其 manifest 声明的相邻证据和最终回复。真实渠道发布成功后，发布执行方才从该已发布且带 tag 的主分支开始下一次开发生命周期，追加 Verification/发布/Product Status 记录并执行版本周期 finalize；失败、取消、tag/上传尝试、`pending`/`accepted` 或就绪结论都不能触发这些 tracked 写入。独立回顾性人工复核或长期审计不得改写活动候选、覆盖历史失败或反向授予其 `accepted`/`ready`。
...
- Git 生命周期多远端回归必须使用隔离裸远端证明：`--remote`/`state.remote` 仍是唯一主/发布远端且独占 fetch/merge、状态、release、tag 与清理；只有 `publish` 接受重复的 `--also-remote <name>`，全部目标在首个 push 前解析，补充远端按各自 advertised default branch 接收同一最终 HEAD 并逐个复读。还必须覆盖不可解析目标时零 push、补充远端不写入生命周期状态、不参与 release、tag 或清理、跨远端推送不是原子操作时如实说明可能已成功的前序范围/失败阶段/后续未尝试可能，以及完全相同参数的同一参数幂等重试；测试不得创建/配置真实用户远端或凭据。
...
| Git 生命周期 | 隔离裸远端回归 + `start`/`track-worktree`/`publish`/`release` 真实 ref 断言 | 新功能/Bug 自动建本地分支；`publish` 普通合并、切换并推送主远端且保留资源，显式重复 `--also-remote <name>` 时先解析全部目标，再把同一最终 HEAD 推向各自默认分支并逐个复读；覆盖不可解析时零 push、补充目标零持久化、跨远端部分成功与同参幂等重试。`release` 只使用主远端，在主分支推送后创建并推送版本 tag，tag 复读成功后才按 Worktree→主远端分支→本地分支精确清理；覆盖 tag 失败零清理、同名冲突、部分清理重试、dirty Worktree 和未登记资源保留 |
...
| 工作流资产 | 全文件 SHA-256 + 独立 YAML 解析 + 步骤/输入/一致性契约 | 只生成三平台 `milestoneAcceptance: pending` 候选；检出并复核具名动态默认分支、完整历史和只读 origin refs，关闭凭据持久化，以发布上下文摘要为输入在测试前和 manifest 前离线复算完整 `releaseReview`/`candidateSelections`；使用不可变 Action 提交，依次刷新、非空测试/构建、条件签名、仓库外同根暂存、目录级原子提交和精确上传，禁止冒烟、E2E、发布和写权限提升 |
...
真实渠道发布成功后，其发布、候选验收和适用人工签署证据才在下一次自动开发生命周期中按日期追加到对应证据卷；独立回顾性长期审计在自己的明确维护任务中追加，人工复核正文写入人工复核卷。候选构建、E2E、完整验收和就绪复核只更新忽略的 `release/` 原子证据集合，不写本索引或证据卷；普通缺陷修复、纯重构和内部维护也不因任务性质写入。
