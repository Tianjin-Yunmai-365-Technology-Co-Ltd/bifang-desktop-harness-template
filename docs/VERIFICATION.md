...
| 持久 Agent 策略 | 模式定义正负向单元测试 + 初始化契约 | 用户显式选择一次推荐预设或自定义；推荐预设将 `user_owned_tasks` 默认为 `disabled`，自定义可开启，最终 schema v2 五字段原子写入且不得残留 `pending`；后续同值切换零写入、真实切换只影响后续结果边界，每次构建仍单独解析 E2E |
