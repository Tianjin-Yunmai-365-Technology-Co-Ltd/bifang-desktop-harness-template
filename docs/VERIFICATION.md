...
- 唯一的前候选例外是含 GUI 下游的一次性初始化 E2E：相关非空单元测试通过后，以 `pnpm tauri build --debug --no-bundle` 构建并启动真实本机调试二进制，验证主窗口、收起侧栏 Logo/图标居中和全部渲染菜单页面可达。它不消费 `milestone_e2e`，不写本索引或证据分卷，不产生 release/完整验收结论；失败会阻断初始化基线提交。
...
| GUI 初始化 E2E | `$desktop-test-gui-initialization-e2e` + Computer Use | 只在含 GUI 的一次性初始化提交前运行真实本机调试二进制；必须证明可构建启动、收起侧栏 Logo/全部图标水平居中、全部渲染菜单页面可达，并回收子进程；不作为候选验收证据 |
