...
- 唯一的前候选例外是含 GUI 下游的一次性初始化 E2E：相关非空单元测试通过后，先以 `verify-gui-lifecycle-contract.mjs` 检查强制单实例依赖/首插件/回调、托盘 feature、非透明 32px RGBA 图标/配置引用、从 `.setup` 可达的 Menu/icon/build/关闭事件接线、稳定 ID、`rust-i18n` 标签解析、双语资源和八个有断言固定命名回归，再以 `pnpm tauri build --debug --no-bundle` 构建并双启动真实本机调试二进制。第二次启动必须退出且唤醒同一主窗口，宿主只剩一个长期应用主进程/主窗口；Computer Use 还必须实测托盘有可见非空图形、中文“显示窗口/退出”与英文“Show Window/Quit”可无重启刷新且无原始 key、关闭隐藏且进程继续、左键与显示项恢复聚焦、退出项结束进程并移除图标，以及主窗口、固定单态侧栏尺寸/排列/居中、默认设置页无隐私/统计区块和全部渲染菜单页面可达。它不消费 `milestone_e2e`，不写本索引或证据分卷，不产生 release/完整验收结论；失败、唯一性无法判定、只能定位空白点击区域或宿主无法观察都会阻断初始化基线提交。
...
| GUI 初始化 E2E | `verify-gui-lifecycle-contract.mjs` + `$desktop-test-gui-initialization-e2e` + Computer Use | 只在含 GUI 的一次性初始化提交前先检查强制单实例、非透明 32px 图标/配置、托盘运行时接线与 `rust-i18n` 标签解析，再双启动真实本机调试二进制；必须证明第二次启动退出并唤醒既有主窗口、只剩一个长期应用主进程/同一主窗口，托盘有可见非空图形、中英文精确标签可无重启刷新且无原始 key、关闭隐藏、两种恢复和退出生命周期可操作，以及可构建启动、固定单态侧栏尺寸/排列/居中、默认设置页无隐私/统计区块、全部渲染菜单页面可达，并回收两个子进程句柄；无法判定、只能定位空白点击区域、无法观察或任一失败都阻断，不作为候选验收证据 |
