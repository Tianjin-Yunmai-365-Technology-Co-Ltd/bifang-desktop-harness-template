...
- 唯一的前候选例外是含 GUI 下游的一次性初始化 E2E：相关非空单元测试通过后，检查器读取七项 profile，对启用能力检查完整依赖、运行时、资源与有断言回归，对禁用能力检查无残留，再构建真实本机调试二进制。按选择验证单实例、托盘、系统通知权限/投递和开机自启登录项切换/恢复；托盘禁用时验证关闭最后窗口退出。Computer Use 始终验证所选侧栏、设置页、实际菜单页面和未选页面缺席。任一适用场景失败、无法观察或状态无法恢复都会阻断初始化基线提交。
- GUI 发布性能是独立硬门禁，不受 `milestone_e2e` 影响。它只接受绑定 clean HEAD、release profile、候选摘要、目标平台和整个 Tauri/WebView 进程树的真实测量；debug、旧候选、样本不足、只采父进程、无法观察或进程未回收均失败。失败先回开发循环，用户显式继续只能记录 `performanceStatus: waived` 与原失败证据，不能改判为通过。
...
| GUI 初始化 E2E | `verify-gui-lifecycle-contract.mjs` + `$desktop-test-gui-initialization-e2e` + Computer Use | 只在含 GUI 的一次性初始化提交前读取七项 profile；按选择验证单实例、托盘、通知和自启恢复，禁用能力无残留；所有组合验证侧栏、设置页、实际菜单页面并回收进程。无法观察或恢复即阻断，不作为候选验收证据 |
| GUI 发布性能 | `$desktop-test-gui-release-performance` + release-profile 真实探针候选 | 每个平台测 5 次冷启动、至少 20 次交互、整进程树 CPU/RSS、内存增长和退出回收；失败先修复重建。只有显式 waiver 可继续，失败证据保持失败；xwin 为 `Unverified` |
