...
| LIM-013 | 固定 TUI 与 Tauri GUI React 前端技术族尚无真实下游兼容下界证据 | Ratatui/tui-realm/tui-realm-stdlib 的候选下界可能与 Rust 1.90 不兼容；React/Mantine/TanStack/Jotai 的候选下界也可能受 Node.js 或 Tauri WebView 约束 | 在真实 TUI 与 GUI 下游分别声明最低兼容稳定范围，以最低直接版本和项目最低工具链完成代码规范检查、非空测试、生产/发布构建、真实产物启动和关键交互验收，再以正常锁文件固定实际解析结果 | Open |
...
| LIM-020 | 随附的 CLI 测试曾不符合项目声明的最低 Rust 1.90 工具链 rustfmt | 5 处链式调用已按 Rust 1.90 rustfmt 机械更新；fmt、锁定依赖检查、Clippy 与 7 个非空测试在 2026-07-29 重跑通过 | 后续修改继续使用根 `Cargo.toml` 声明的最低 MSRV 工具链运行格式门禁 | Closed |
