...
- Rust 技术选型新增统一事实标准：Tokio、Axum、Clap、SeaORM、tracing、anyhow、thiserror、serde 与 jiff 分别覆盖异步、HTTP、CLI、关系型 ORM、可观测性、应用错误上下文、稳定类型化错误、序列化和日期时间；初始化与实施入口按真实能力引入，不给中性 scaffold 预装未使用依赖。
...
- 默认 Harness Python 回归 112 条通过，其中包含 Rust 技术选型事实标准、行数/core-first 检查器专属回归、项目记忆正反向规则、文档分卷事实守恒和既有治理/发布/工作流门禁；28 条 Harness 升级器隔离 Git 回归另行通过。
