...
- 新增 `$rename-project-identity`，支持预览后全量修改展示名、项目标识/前缀、项目自有配置、维护路径、文档、Skills 和许可证，并阻断覆盖与符号链接。

## 变更

- `docs/RELEASE.md` 改为只维护版本与发布规则，并链接根 `Version.md`；README、AGENTS、当前产品规格、工程规则、方法论、技术债和 `$prepare-release` 已同步新的版本事实边界。
- `$instantiate-project` 明确不把 Harness 专用 `Version.md` 复制到 Rust 下游，避免与下游根 `Cargo.toml` 形成冲突版本事实。
- 全部 Markdown 文档已纳入当前事实、链接、旧身份、已替代规则和本机路径残留审计；Harness 校验器增加版本文件及同步契约门禁。
...
- AGENTS、README、产品规格、产品状态、工作计划、ADR、发布和 Harness 校验器已同步专有商业许可与下游继承边界。

## 验证边界

- Harness 静态和负向门禁结果记录于 `docs/VERIFICATION.md`。
- 全部文档与版本门禁已通过；随附的 Rust 资产仍有 LIM-020 所记录的 Rust 1.90 rustfmt 失败，因此总体为 `Partially verified`，`1.0.0` 保持 `Unreleased` / `Not ready`。
