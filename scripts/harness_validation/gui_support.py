...
def validate_gui_support_contract(
    ...
    *,
    ...
    required = {
        skill_path: (
            "七项初始化配置",
            "`system_notification = enabled` 时按 `$desktop-add-gui-system-notifications`",
            "`autostart = enabled` 时按 `$desktop-add-gui-autostart`",
            "系统通知与开机自启分别覆盖 Switch 自身点击",
        ...
        pages_reference_path: (
            "## 资产包内容",
            "关于页、赞助页、系统托盘、系统通知、开机自启、单实例和侧栏模式则严格消费 GUI 初始化专门问询",
