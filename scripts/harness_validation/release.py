...
def validate_build_skill_contract(
    ...
    required = {
        build_skill: (
            "默认直接在当前宿主本地构建",
            "当前宿主候选继续保存在目标 `release/` 之外的唯一暂存目录",
            "不配置、触发或等待任何 CI/CD",
            "必需平台缺失、失败、超时或取消必须阻断对应交付",
            "在执行任何单元测试或构建命令前",
        ...
        cross_platform_skill: (
            "默认当前宿主的 `$desktop-build-rust-release` 路线",
            "不配置、触发或等待任何 CI/CD",
            "在执行任何单元测试或构建命令前",
            "release_notes.py check --file release-notes.json --expected-version",
            "不得生成或改写更新日志",
            ...
            "signingStatus: unsigned",
            "结构化 `signingEvidence`",
            "固定 `version`、`source_commit`、本地 `buildRun` 和 `e2e_selection`",
        ...
        collect_skill: (
            "只接受用户提供的本地结果目录，不连接 CI/CD、不下载远端制品、不上传",
