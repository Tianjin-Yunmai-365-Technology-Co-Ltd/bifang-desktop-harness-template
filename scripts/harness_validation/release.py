...
def validate_build_skill_contract(
    ...
    required = {
        ...
        cross_platform_skill: (
            ...
            "不得在受保护的 clean 默认分支 closing commit 上安装、更新或改写 workflow",
        ...
        cross_platform_envelope_helper: (
            ...
            'resolve_ref(root, "refs/remotes/origin/Release", missing_ok=True)',
...
def validate_release_selection_contract(
    ...
    *,
    ...
    branch_remote: Path = BRANCH_CHAIN_REMOTE,  # noqa: F405
    ...
    branch_race_tests: Path = BRANCH_CHAIN_RACE_TESTS,  # noqa: F405
    branch_contract_tests: Path = BRANCH_CHAIN_CONTRACT_TESTS,  # noqa: F405
    ...
    required = {
        ...
        branch_skill: (
            ...
            "旧关闭提交只允许在旧式远端 `Release` 事务已经完整完成时执行 local-only 收尾",
            "绝不能新推进远端默认分支",
            "legacy-local-cleanup-complete",
            "正常路径不会发送针对缺失 `Release` 的删除 refspec",
            "远端 `HEAD` 动态解析出的默认分支必须精确为 `main` 或 `master`",
            "helper 不创建或使用 `Release` 中转分支",
            "默认分支更新只能是发布事务内基于冻结旧 OID 的严格 fast-forward",
            "随后一次 `git push --atomic` 在默认分支冻结旧 OID 的 lease 下把 closing commit 严格快进到 `refs/heads/<defaultBranch>`",
            "并以逐 ref `--force-with-lease` 删除状态中精确列出的远端 feature refs",
        ),
        branch_state: (
        ...
        branch_operations: (
            ...
            "legacy closing state cannot update the remote default branch",
            "closing state does not authorize a direct-default release",
            "a legacy closing commit cannot update the remote default branch",
            "closing commit is not a fast-forward of the frozen default head",
        ...
        branch_remote: (
            ...
            "snapshot[default_branch] == closing_head",
            'snapshot["Release"] is None',
            'return "pending-release-missing"',
            "def require_legacy_remote_complete(",
            'snapshot["Release"] == closing_head',
        ...
        branch_tests: (
            ...
            "test_legacy_close_without_review_cannot_update_remote_default",
        ...
        branch_race_tests: (
            ...
            "test_pending_retry_rejects_local_default_drift_before_push",
            "test_complete_retry_from_base_default_skips_commit_gate_and_fast_forwards",
            "test_local_cleanup_hook_mutation_cannot_report_released",
        ),
        ...
        rust_skill: (
            ...
            "当前 clean 动态默认 `main`/`master` closing commit",
            "`lastClosedChain.releaseReview` 与 `candidateSelections`",
            "其返回的 `releaseBranch` 必须等于动态默认分支",
            ...
            "要求返回的 `releaseBranch`/`releaseHead`/完整信封逐字段等于第 1 步",
        ...
        collect_skill: (
            ...
            "并锁定其 `releaseBranch`、`releaseHead`、`releaseReview` 与 `candidateSelections`",
            "当前分支、本地/远端默认 ref 和所有源 manifest 的 `sourceCommit` 必须分别等于该 `releaseBranch`/`releaseHead`",
        ...
        verify_skill: (
            ...
            "当前是完成远端默认 ref 推进和本地收尾的 clean 动态默认 `main`/`master` closing commit",
            "`releaseBranch` 等于当前默认分支",
            "把该 `releaseHead`、分支和两份规范信封锁定为准入快照",
        ...
        release_doc: (
            ...
            "候选构建一律从完成远端和本地收尾的 clean 动态默认分支 closing commit 只读验证并消费",
            "不能现场解析、补写或从对话恢复这些值",
    ...
    *,
    ...
    required = {
        prepare_skill: (
            "用户明确提出发布时，该请求本身授权",
            "不再追加提交或构建审批",
            "活动叶 push、默认分支严格 fast-forward、状态登记 feature refs 精确清理",
            ...
            "命令成功后当前分支必须是同名默认分支",
            "状态登记的本地/远端 feature refs 均不存在",
            "不得扫描 `codex/*` 或其他未登记分支",
            "构建所用 `sourceCommit` 必须等于该 `releaseHead`",
            "默认分支推进和精确清链已经是本次 Git 发布终态",
            "不再保留 `Release` 中转或用户 Merge/PR 步骤",
            "不再询问是否提交或是否开始构建",
            "普通构建不自动提交",
        ),
        ...
        release_doc: (
            ...
            "普通开发构建/本地试包不升级为候选，也不提交、关闭链路或修改默认分支",
            "任何请求都不授权 tag、上传、商店提交或真实渠道发布",
            "从新的源码 HEAD",
            "以单次 atomic push 严格快进动态默认 `main`/`master` 并按 lease 删除登记的远端链",
            "再切回/快进本地默认分支并精确清理本地链",
            "最终必须位于 clean 默认分支",
            "构建 `sourceCommit` 固定等于该值",
            "默认分支已经由受管发布事务完成严格快进，不再等待额外 Merge/PR",
            "没有创建 `Release` 中转或扫描删除链外/`codex/*` refs",
        ),
        readme: (
            "$desktop-manage-git-branch-chain",
            "把登记的完整线性链严格快进到动态远端默认 `main`/`master`",
            "以逐 ref lease 原子删除远端链路后切回本地默认分支并清理本地链路",
            "从该 clean closing commit 构建，不重复审批",
            "普通“构建/打包/本地试包”不会自动升级为发布候选、提交、关闭分支链或修改默认分支",
            "只有明确发布会自动完成上述默认分支快进、切换与精确链路清理",
        ),
    }
    validate_fragment_contract(
        errors,
