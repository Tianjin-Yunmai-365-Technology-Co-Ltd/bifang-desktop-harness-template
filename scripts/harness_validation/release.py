...
def validate_release_selection_contract(
    ...
    *,
    ...
    required = {
        ...
        lifecycle_skill: (
            "publish` 要求可解析主/发布远端",
            "`release` 无论从主 Worktree 还是关联 Task Worktree 发起",
            "`v{version}-{YYYYMMDD}`",
            "不设置任何分支门禁",
        ),
        context_helper: (
        ...
        release_doc: (
            ...
            "`sourceCommit`、`expectedTag`、`releaseReview` 和 `candidateSelections`",
            ...
            "候选构建一律从已经推送且带预期 tag 的 clean 动态默认主分支",
        ),
        verification_doc: (
    ...
    *,
    ...
    required = {
        prepare_skill: (
            ...
            "$desktop-manage-git-lifecycle release",
            "helper 必须先普通合并本周期登记分支，切到动态默认分支并推送",
            "再创建/复用仅在同一提交上的轻量 tag、推送并复读远端 tag",
        ...
        lifecycle_skill: (
            ...
            "`v{version}-{YYYYMMDD}`",
            "远端标签成功确认之前不会开始清理",
            "依次移除精确登记且干净的 Worktree、精确登记的主远端分支、精确登记的本地分支",
            "不创建 `Release` 或其他中转分支",
            "也不设置任何分支门禁",
        ),
        lifecycle_script: (
        ...
        release_doc: (
            ...
            "普通合并全部登记开发分支，切换到主远端的动态默认主分支，推送并复读主分支",
            "--also-remote",
            "补充远端不写入生命周期状态",
            "不参与 release、tag 或清理",
            "在当前主分支 HEAD 创建轻量 tag `v{version}-{YYYYMMDD}`",
            "只有主远端 tag 精确指向当前主分支 HEAD",
            "才先删除状态登记的 Worktree，再删除对应主远端分支，最后删除对应本地分支",
            "不设置保护分支、严格线性",
        ...
        readme: (
            ...
            "创建并推送 `v{版本}-{YYYYMMDD}`",
            "tag 复读成功后依次清理登记 Worktree、远端分支和本地分支",
            "只有明确发布会自动执行 tag 前主分支推送和 tag 后精确清理",
        ),
    }
    validate_fragment_contract(
        errors,
