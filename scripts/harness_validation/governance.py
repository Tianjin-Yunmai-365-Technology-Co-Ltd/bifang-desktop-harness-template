...
def validate_engineering_contract(errors: list[str]) -> None:
    ...
    required_fragments = {
        ENGINEERING_RULES: (
            ...
            "任务被称为“修复”或“重构”不能绕过相应的安全、发布或记录要求",
            ...
            "Git common dir",
            "普通合并本周期登记分支、切换动态默认主分支并推送复读",
            "远端 tag 复读成功后才按 Worktree、远端分支、本地分支顺序精确删除",
            "候选事实只写入忽略的 `release/` 原子集合",
            "真实渠道发布成功后，下一次开发再自动创建新的 feature 分支",
        ...
        AGENT_POLICY: (
            ...
            "产品/渠道要求时执行完整门禁",
            "`$desktop-prepare-release` 必须把当次审查结果、GUI 性能选择和适用的 macOS 签名选择/来源写入当前发布的 `.harness/release-context.json`",
            "候选构建在清理、测试前和写 manifest 前只读验证并消费这份上下文",
            "只另外解析当前候选的 E2E 选择",
        ),
        PRODUCT_SPEC: (
            "docs/ENGINEERING_RULES.md",
    ...
    required_fragments = {
        ROOT / "README.md": (
            ...
            *PROJECT_TASK_SEQUENCE_REQUIRED_FRAGMENTS,
            ...
            "GUI 同轮解析性能与 macOS 签名选择，并把它们写入 `.harness/release-context.json`",
            ...
            "相同 Git common dir",
            "即使初始为 detached HEAD，也会直接创建唯一临时 `feature-*` 分支并自动登记该 Worktree",
            "`codex/unit-*`",
            'title="{序号}|{Task简述}|已分配 |{功能摘要}"',
            "派发前记录不可变 Task key",
            "不得把 `{序号}|{Task简述}|{当前进度} |{功能摘要}` 原样当作分支名",
            "不以标题冒充项目/Worktree 身份",
            "生命周期阶段变化就拆 Task",
        ...
        AGENT_POLICY: (
            ...
            *PROJECT_TASK_SEQUENCE_REQUIRED_FRAGMENTS,
            ...
            "隐藏 Subagent 不出现在 `list_threads` 时改用",
            ...
            "把结果写入 `.harness/release-context.json` 的 `candidateSelections`",
            "候选构建在清理、测试前和写 manifest 前只读验证并消费这份上下文",
            "只另外解析当前候选的 E2E 选择",
            ...
            "不要自行合并默认/集成分支",
            "生命周期 helper 才按 Worktree、远端分支、本地分支的顺序精确清理",
            "## 开发分支与主分支发布生命周期",
            ...
            "允许普通 merge commit",
            "Git common dir 下的 `agent-first-harness/git-lifecycle.json`",
            "只有主分支推送和 tag 推送均已确认成功",
            "先删除登记 Worktree，再删除对应远端分支，最后删除对应本地分支",
            "没有发布中转分支",
            "这里没有分支保护、活动叶子、单写入者、严格线性历史、fast-forward-only、lease、atomic push",
        ),
        ...
        SKILLS_ROOT / "desktop-implement-change" / "SKILL.md": (
            ...
            *PROJECT_TASK_SEQUENCE_REQUIRED_FRAGMENTS,
            ...
            "每次真实转换至多尝试一次",
            "更新为终态 `已完成`",
            "结果、验证、文档、本地提交、clean 状态，以及用户本次明确要求的推送或发布全部完成后",
            ...
            "只更新被独立事件触发的记忆",
            "当前说明来自左侧 Task 模板",
            "保存项目完整路径、`projectId`、repository identity、所选 Worktree/Local 环境、起始分支和起始提交",
            "git rev-parse --path-format=absolute --git-common-dir",
            "git worktree list --porcelain",
            'title="{序号}|{Task简述}|已分配 |{功能摘要}"',
            "稳定正整数序号",
            "显示标题与 Git ref 始终是两项独立事实",
            "$desktop-manage-git-lifecycle start",
            "把当前发布周期登记分支普通合并到主分支",
            "创建 `v{版本}-{日期}` 标签并推送远端",
            "普通完成到此为止，不自动 push",
            "只有用户明确说“推送”时",
            "$desktop-manage-git-lifecycle publish",
            "不退化到另一种环境、其他 Task 或内部 Subagent 继续实施",
            "每完成一个逻辑闭环",
            "`git status --porcelain=v1 --untracked-files=all`",
            "只有用户明确说“发布”时",
        ),
        PARALLEL_SKILL / "SKILL.md": (
            ...
            "当前存在 feature 开发周期不构成拒绝 sibling",
            "不得读取或解释旧分支链状态来阻断并行",
            "它不调用 `create_thread`，不创建新的左侧 Task",
            "保存项目根与当前 Task 的 source Git 顶层",
            "相同规范化 Git common-dir",
            "`codex/unit-<task>-<unit>`",
            "`{序号}|{Task简述}|{当前进度} |{功能摘要}` 显示标题",
            "不得把 `{序号}|{Task简述}|{当前进度} |{功能摘要}` 显示标题原样传给",
            "不得把自己称为新的左侧 Task",
            "在消息中写入完整逻辑初始标题",
            "隐藏 Subagent 不占用用户可见的项目序列",
            "追加批次必须避开该父 Task 已经分配的序号",
            "隐藏 Subagent 不出现在 `list_threads` 时",
        ...
        SKILLS_ROOT / "desktop-initialize-rust-project" / "SKILL.md": (
            ...
            'title="{序号}|{Task简述}|已分配 |{功能摘要}"',
            *PROJECT_TASK_SEQUENCE_REQUIRED_FRAGMENTS,
            "显示标题与独立 ASCII `feature-summary` 分离",
        ...
        INSTANTIATE_SKILL: (
            ...
            "精确保存项目/`projectId`",
            "`SETUP_PENDING` 有界返回",
            "从具名或 detached HEAD 建立并登记 `feature-*` 分支",
            "`codex/unit-*`",
            'title="{序号}|{Task简述}|已分配 |{功能摘要}"',
            *PROJECT_TASK_SEQUENCE_REQUIRED_FRAGMENTS,
        ...
        PRODUCT_SPEC: (
            ...
            "HARNESS-CHANGE-UNIFIED-SESSION-TITLE-FORMAT",
            "HARNESS-FIX-PROJECT-TASK-SEQUENCE-AUTO-INCREMENT",
            "`{序号}|{Task简述}|{当前进度} |{功能摘要}`",
            *PROJECT_TASK_SEQUENCE_REQUIRED_FRAGMENTS,
            'create_thread(title="{序号}|{Task简述}|已分配 |{功能摘要}")',
            "检查失败返工时回到 `运行中`",
            ...
            "相同规范化 Git common dir",
            "由统一生命周期 helper 创建并登记的 `feature-*` 分支",
            "`codex/unit-<task>-<unit>`",
            "不存在保护分支、严格线性、active leaf、单写入者",
            "postflight",
        ),
        PARALLEL_WORKTREE_SCRIPT: (
            '"rev-parse", "--show-toplevel"',
            "def track_lifecycle_worktree",
        ...
        PARALLEL_WORKTREE_TESTS: (
            "test_create_tracks_exact_worktree_in_current_release_cycle",
            "test_create_rolls_back_when_lifecycle_tracking_fails",
            "test_remove_defers_worktree_and_branch_cleanup_to_release",
            "test_create_rejects_dirty_source_including_untracked_files",
            "test_create_accepts_local_primary_source_and_requires_exact_source_cwd",
            ...
            "test_remove_rejects_dirty_but_accepts_clean_unmerged_unit",
            "test_remove_runs_postflight_before_clean_state_cleanup",
        ),
        GIT_LIFECYCLE_SKILL: (
        ...
        VERIFY_DELIVERY_SKILL: (
            ...
            "返回 `$desktop-implement-change` 修复并增加回归",
        ),
        E2E_SKILL: (
            "当前构建选择为 `enabled`",
        ...
        TAURI_RELEASE_SKILL: (
            ...
            "同一 `sourceCommit` 重跑复用上下文，新发布重新生成",
            "打包前只先运行项目全部非空单元测试",
            "cargo test --workspace --all-targets --all-features --locked",
        ...
        ROOT / "docs" / "RUST_CLI_TEMPLATE.md": (
            ...
            "只写入忽略的 `release/` 原子集合",
            "随后 `$desktop-manage-git-lifecycle release` 普通合并登记分支",
        ),
        ROOT / "docs" / "VERIFICATION.md": (
            "候选 E2E、完整验收、`pending` → `accepted` 状态更新和发布就绪复核",
            "仅写入忽略的 `release/` 原子候选集合",
            "真实渠道发布成功后，发布执行方才从该已发布且带 tag 的主分支开始下一次开发生命周期",
            "独立回顾性人工复核或长期审计不得改写活动候选",
