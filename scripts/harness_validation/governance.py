...
def validate_engineering_contract(errors: list[str]) -> None:
    ...
    required_fragments = {
        ...
        AGENT_POLICY: (
            ...
            "产品/渠道要求时执行完整门禁",
            "`$desktop-prepare-release` 必须把 `gitPublication: local | remote`、当次审查结果、GUI 性能选择和适用的 macOS 签名选择/来源写入当前发布的 `.harness/release-context.json`",
    ...
    required_fragments = {
        ROOT / "README.md": (
            ...
            *PROJECT_TASK_SEQUENCE_REQUIRED_FRAGMENTS,
            ...
            "内部并行仍由独立的 `parallel_worktree_subagents` 控制",
            "## 开始一个左侧 Task",
            "`list_projects` 核对项目名称、完整路径和 Git 状态",
            "精确 `projectId`",
            "仅有 `clientThreadId` 表示仍在 setup",
            "不得重复创建",
            "干净工作区和起始提交",
            "`codex/unit-*`",
        ...
        AGENT_POLICY: (
            ...
            "本文的 Task 一律指 Codex 左侧菜单",
            "`Task {序号} | {当前进度} | {单一结果}`",
            *PROJECT_TASK_SEQUENCE_REQUIRED_FRAGMENTS,
            "`已分配`、`运行中`、`检查中`、`已完成`",
            "每次真实转换至多尝试一次 `set_thread_title`",
        ...
        SKILLS_ROOT / "desktop-implement-change" / "SKILL.md": (
            ...
            *PROJECT_TASK_SEQUENCE_REQUIRED_FRAGMENTS,
            ...
            "只更新被独立事件触发的记忆",
            "当前说明来自左侧 user-owned Task",
            "保存项目完整路径和 `projectId`；Git Task 还须核对 repository identity",
            "非 Git Local Task 须核对 cwd",
            "git rev-parse --path-format=absolute --git-common-dir",
            "git worktree list --porcelain",
            "Task {序号} | 已分配 | {单一结果}",
            "Task 编号和单一结果全程稳定",
            ...
            "把当前发布周期登记分支普通合并到主分支",
            "询问/锁定 `gitPublication: local | remote`",
            "本地模式使用 `release ... --release-context-sha256 <sha256> --local-only`",
            "远端模式使用 `release ... --release-context-sha256 <sha256> --remote <name>`",
            "普通完成到此为止，不自动 push",
            "只有用户明确说“推送”时",
        ...
        PARALLEL_SKILL / "SKILL.md": (
            ...
            "当前 source 可以是通过 BOUND 门禁的左侧 Git Task Worktree",
            "相同规范化 Git common-dir",
            "`codex/unit-<task>-<unit>`",
            "不受 `user_owned_tasks` 开关控制",
            "不使用 `Task {序号} | {当前进度} | {单一结果}` 标题合同",
            "不占用用户可见项目序列",
        ...
        PRODUCT_SPEC: (
            ...
            "HARNESS-FEAT-OPTIONAL-USER-OWNED-TASKS",
            "`Task {序号} | {当前进度} | {单一结果}`",
            *PROJECT_TASK_SEQUENCE_REQUIRED_FRAGMENTS,
            'create_thread(title="Task {序号} | 已分配 | {单一结果}")',
            "一个 Task 固定一个可验收结果",
            "`clientThreadId` 只表示 setup",
            "Git 使用 Worktree，非 Git 使用 Local",
            "`codex/unit-*`",
        ...
        PREPARE_RELEASE_SKILL: (
            "在任何提交或发布元数据写入前解析当次 Git 发布位置与 `reviewSelection: enabled | disabled`",
        ...
        ROOT / "docs" / "RUST_CLI_TEMPLATE.md": (
            ...
            "只写入忽略的 `release/` 原子集合",
            "每次正式发布先由 `$desktop-prepare-release`",
            "本地模式调用 `$desktop-manage-git-lifecycle release ... --release-context-sha256 <sha256> --local-only`",
        ...
        ROOT / "docs" / "VERIFICATION.md": (
            ...
            "真实渠道发布成功后，发布执行方才从该带 tag 的主分支开始下一次开发生命周期",
