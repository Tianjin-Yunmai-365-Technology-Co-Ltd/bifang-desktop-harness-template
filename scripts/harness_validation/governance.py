...
def validate_engineering_contract(errors: list[str]) -> None:
    ...
    required_fragments = {
        ENGINEERING_RULES: (
            ...
            "明确发布的受管原子事务是推进 `Release` 的唯一例外",
            ".harness/git-branch-chain.json",
            "`main`、`master`、动态远端默认分支与 `Release` 禁止日常写入",
            "候选事实只写入忽略的 `release/` 原子集合",
            "真实渠道发布成功后，才从已发布 `Release` 开始后续受管 feature 生命周期",
            "候选构建、E2E、完整验收和就绪复核不得为了留证污染固定 `Release`",
        ),
        ROOT / "README.md": (
...
def validate_streamlined_development_and_build(errors: list[str]) -> None:
    ...
    required_fragments = {
        ...
        AGENT_POLICY: (
            ...
            "普通调用 Task（即没有为本结果调用 `create_thread`",
            "`{task}-{id}-{feature}` 是同一格式的英文占位别名",
            ...
            "精确 `main`、`master`、动态解析出的远端默认分支以及 `Release` 都是保护分支",
            "用一次 atomic push 将精确 `Release` 快进到关闭提交",
            "以每个登记远端 feature ref 的旧 OID 作为 lease 删除整链",
            "Agent 永远不得执行、代建或自动化 `Release` 到 `main`",
        ),
        ...
        SKILLS_ROOT / "desktop-implement-change" / "SKILL.md": (
            "直接实现用户请求",
            "Session 收尾契约至多调用一次 `set_thread_title`",
            "`{task}-{id}-{feature}`",
            "再按真实 `threadId` 用 `list_threads` 做一次有界复读",
            "不阻断已经完成的任务结果",
            "只运行第 6 步的测试",
        ...
        INSTANTIATE_SKILL: (
            ...
            "`codex/unit-*`",
            'title="{任务}-{ID}-{摘要}"',
            "返回的 `threadId`/`clientThreadId` 不得反填标题",
        ),
        ...
        BRANCH_CHAIN_SKILL: (
            ...
            "git push --atomic",
            "--force-with-lease",
            "helper 永不提供或执行 `Release` 到默认分支的命令",
        ),
        BRANCH_CHAIN_SCRIPT: (
        ...
        BRANCH_CHAIN_TESTS: (
            ...
            "test_start_creates_state_commit_and_pushes_exact_feature_ref",
            ...
            "test_start_rejects_default_branch_and_release_as_remote_default",
        ...
        BRANCH_CHAIN_RACE_TESTS: (
            "test_release_race_uses_frozen_lease_and_preserves_feature_refs",
        ...
        COLLECT_RELEASE_SKILL: (
            ...
            "锁定其 `releaseHead`、`releaseReview` 与 `candidateSelections`",
        ...
        ROOT / "docs" / "RELEASE.md": (
            ...
            "`releaseReview` 与 `candidateSelections` 和链关闭状态写入同一个 closing commit",
            ...
            "真实渠道发布成功后，才从该已发布 `Release` 开始后续受管 feature 生命周期",
        ),
        ROOT / "docs" / "RUST_CLI_TEMPLATE.md": (
        ...
        ROOT / "docs" / "VERIFICATION.md": (
            ...
            "真实渠道发布成功后，发布执行方才从该已发布 `Release` 开始后续受管 feature 生命周期",
