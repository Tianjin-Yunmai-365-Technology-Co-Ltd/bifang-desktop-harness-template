...
def validate_parallel_and_tiered_verification(errors: list[str]) -> None:
    ...
    required_fragments = {
        ROOT / "AGENTS.md": (
            ...
            "不得降级",
            "不强制 `$desktop-define-product`、`$desktop-plan-change`",
            "代码行为变化必须以相关非空测试",
            "纯文档、元数据、格式或不可合理单测",
            "只有里程碑路径要求当前批次 Todo 全部为 `done`",
            "Mock、stub、占位页面、中性 scaffold",
            "重开或新增具体 Todo",
            "返回 `$desktop-implement-change`",
        ...
        PARALLEL_SKILL / "SKILL.md": (
            ...
            "标准路径直接返回 `$desktop-implement-change` 收口",
        ),
        ...
        SKILLS_ROOT / "desktop-rename-project-identity" / "SKILL.md": (
            "实例化身份重置",
            "现有产品改名",
            "必须先通过 `$desktop-define-product`",
            "不得以快速或标准路径完成",
            "交给 `$desktop-verify-delivery` 验收",
        ),
        ...
        E2E_SKILL: (
            ...
            "$desktop-verify-delivery` 已进入里程碑验收",
            "完整真实产物",
            "重开或新增修复 Todo",
            "返回 `$desktop-implement-change`",
        ),
        BUILD_RELEASE_SKILL: (
            "不运行冒烟或 E2E",
            "不得启动二进制文件或运行冒烟/E2E",
            "把精确的最终字节和清单交给 `$desktop-verify-delivery`",
        ),
        TAURI_RELEASE_SKILL: (
            "不得在本 Skill 中运行冒烟/E2E",
            "不得启动安装包、应用或 Windows 二进制",
            "把精确最终字节交给 `$desktop-verify-delivery`",
        ...
        PREPARE_RELEASE_SKILL: (
            "由 `$desktop-verify-delivery` 给出的 `Milestone accepted` 候选",
            "不得在此运行任一测试",
            "绝不得在发布准备中运行冒烟/E2E",
            "scripts/verify-dmg-layout.sh <final-dmg>",
            "仅含普通缺陷修复或纯重构",
        ...
        ENGINEERING_RULES: (
            ...
            "重开 Todo 并返回实现",
            "任何远程能力在未批准时必须保持禁用和零出站",
            "后台任务必须有 owner、取消路径与关闭回收",
        ),
        ...
        UPGRADE_SKILL / "SKILL.md": (
            ...
            "$desktop-prepare-gui-support-surfaces",
            "docs/GUI_SUPPORT_SURFACES.md` 是 `protected`",
        ),
        UPGRADE_POLICY: (
            "managed",
            "managed-self",
