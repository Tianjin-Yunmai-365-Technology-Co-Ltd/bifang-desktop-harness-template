    ...
    return {
        INSTANTIATE_SKILL: (
            ...
            *PROJECT_TASK_SEQUENCE_REQUIRED_FRAGMENTS,
            "`parallel_worktree_subagents` 仍只控制 `codex/unit-*` 内部并行",
            "内部 Subagent 不使用标题合同",
            "Git 使用独立 Worktree",
            "`codex/unit-*`",
        ...
        skill_file: (
            ...
            "创建零写入门禁",
            *PROJECT_TASK_SEQUENCE_REQUIRED_FRAGMENTS,
            "内部 Subagent/Worktree",
            "用户可见 Task 的标题/粒度/创建门禁",
            "`user_owned_tasks` 的默认关闭与手动开关",
            "`codex/unit-*`",
            ...
            "紧邻真实初始化基线提交时",
            "选择 GUI 时同时完整保留 `$desktop-add-gui-dialog`、`$desktop-prepare-gui-support-surfaces`（包括 `assets/brand-support/**`）与 `$mantine-list-view`",
            "不得预创建 `docs/GUI_SUPPORT_SURFACES.md`",
            "必须在同一首轮一次列出",
            "每轮只询问一个尚未解析的 GUI 条件字段",
            "不得在复制后重新询问接口、中英文名称、目标平台或策略",
            ...
            "不含隐私/统计",
            "浅色/深色/跟随系统",
            "普通页面工作状态继续由应用根 Jotai store 仅在当前进程跨路由保留",
            "初始化只保留该知识 Skill，不生成列表页或预装 dnd-kit",
            "三态主题和亮暗语义主题始终存在",
        ...
        SKILLS_ROOT / "desktop-implement-change" / "SKILL.md": (
            ...
            "更新为 `检查中`",
            "更新为终态 `已完成`",
            "按真实 `threadId` 用 `list_threads` 有界复读",
        ...
        REACT_BASELINE: (
            ...
            "普通页面的 URL/search 只用于明确批准的可分享导航事实",
            "列表查询状态按 `$mantine-list-view` 使用 URL/sessionStorage",
