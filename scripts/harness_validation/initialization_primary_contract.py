...
def primary_required_fragments(
    ...
    return {
        INSTANTIATE_SKILL: (
            ...
            "`.setup`/Menu/icon/build/窗口事件真实接线",
            "第二次启动自行退出",
            "只剩一个长期应用主进程和一个主窗口",
            "中文菜单精确为“显示窗口/退出”、英文精确为“Show Window/Quit”",
            "托盘左键和本地化显示项都能恢复并聚焦",
            "当前宿主无法判定进程/窗口唯一性",
            "无法观察托盘",
            "只能定位空白点击区域",
        ...
        skill_file: (
            ...
            "不得包含隐私标题、统计同意",
            "浅色/深色/跟随系统",
            "关于页在手动“检查更新”旁提供自身绑定的“更新日志”按钮",
            "页面会话状态的进程内跨路由/退出重置/成功空页回退",
            ...
            "非 GUI 中性初始化不自动追加格式、lint、静态、全仓门禁、独立构建、冒烟或 E2E",
            "$desktop-test-gui-initialization-e2e",
            "verify-gui-lifecycle-contract.mjs",
            "tauri-plugin-single-instance",
            "作为 Tauri Builder 的首个 plugin 注册",
            ...
            "中文“显示窗口/退出”和英文“Show Window/Quit”",
            "本地化显示项均恢复并聚焦",
            "无法观察或无法运行都阻断初始化",
            "可见非空图形",
            "空白点击区域",
        ...
        REACT_BASELINE: (
            ...
            "初始化固定建立 `136px` 单态左侧菜单、`/settings`、`/about`、`/sponsor`",
            "不得建立展开/折叠状态",
            "底部组按赞助、设置、关于渲染",
            "真实更新、强更或统计上报",
            "作者、联系方式和三段免责声明",
            "选中的 `56px` 本地 Logo 永远位于侧栏顶部",
            'defaultColorScheme="auto"',
            "useComputedColorScheme",
            "图标在上、名称在下",
            "不渲染隐私/统计区块",
            "light`/`dark`/`auto",
            "关于页显示当前应用名/版本、作者、联系方式和三段免责声明",
            "pageSessionState.ts",
        ...
        GUI_SKILL: (
            ...
            "`tray-icon` feature",
            "icons/32x32.png",
            "从 Tauri Builder 的 `.setup(...)` 实际调用",
            "`.menu(&menu)`",
            "`default_window_icon()` 解析为必需值",
            "`.build(app)?`",
            "`.on_window_event(...)` 实际注册",
            "`show_window`/`quit` 两项",
            ...
            "language_change_updates_tray_menu_labels",
            "浅色/深色/跟随系统",
            "关于页始终在“检查更新”旁渲染元素自身绑定的“更新日志”按钮",
            "应用根 Provider/store",
        ...
        GUI_BASELINE: (
            ...
            "tauri-plugin-single-instance",
            "首个 plugin 注册",
            "Tauri `tauri` 依赖强制启用 `tray-icon` feature",
            "src-tauri/icons/32x32.png",
            "`.setup(...)` 实际调用",
            "`.menu(&menu)`",
            "`default_window_icon()` 当作必需值",
            "成功 `.build(app)`",
            "主窗口 `WindowEvent::CloseRequested` 处理必须由 Builder `.on_window_event(...)` 实际注册",
            "只有 `quit` 显式结束应用",
            ...
            "tray_labels_fall_back_to_english",
            "language_change_updates_tray_menu_labels",
            "宿主无法判定进程/窗口唯一性或观察托盘同样失败",
            "可见图形而非空白点击区域",
            "`{applicationName} v{version} {contactChannel}:{contactValue}`",
            "初始化固定建立 `136px` 单态左侧菜单与 `/settings`、`/about`、`/sponsor` 文件路由",
            "底部固定组按视觉顺序为赞助、设置、关于",
            "初始化设置页不提供隐私/统计控件",
            "签名验证不可关闭",
            "作者、作者联系方式和三段免责声明",
            ...
            "侧栏顶部始终先显示 `56px` 选中 Logo、再紧接带一个小写 `v` 的应用版本",
            ...
            "图标在上、名称在下",
            "light`/`dark`/`auto",
            "关于页在手动“检查更新”旁提供元素自身绑定的“更新日志”按钮",
        ...
        MANTINE_GUIDELINES: (
            ...
            'defaultColorScheme="auto"',
            "每个功能项和固定项必须提供从 `@tabler/icons-react`",
            "固定 `136px` 单态左侧菜单",
            "图标在上、名称在下",
            "@tabler/icons-react",
            "水平居中",
