...
2. 确认当前构建由用户显式请求、仓库根是独立 Git 顶层目录，并且调用方 `git status --porcelain=v1 --untracked-files=all` 为空。在任何派发前先运行 `$desktop-manage-git-branch-chain verify-release-review`，要求成功返回当前 clean 动态默认 `main`/`master` closing commit 的 `releaseBranch`、`releaseHead`、`releaseReview` 与 `candidateSelections`；当前分支、本地/远端默认 ref 必须分别等于 `releaseBranch`/`releaseHead`。把 `releaseHead` 锁定为唯一 `source_commit`，并立即读取该提交中 `.harness/git-branch-chain.json` Git blob 的原始字节、计算跨平台稳定的 `branch_chain_state_sha256`，同时要求普通非符号链接工作树状态经 Git clean filter 后仍映射到同一 blob，再复核分支/HEAD/clean 和返回的两份规范信封都未变化。Rust CLI 只接受性能与 macOS 签名选择/来源均为 `not-applicable` 且原因/风险均为空的候选信封。手动提供方工作流保留固定的 `confirm_candidate_build`、`version`、`source_commit`、`branch_chain_state_sha256` 和 `e2e_selection` 输入；状态摘要只能来自上述机械复核，不接受对话补写的信封、摘要或选择。普通构建不得自行暂存、提交或配置身份；dirty、旧式无信封 closing commit、远端/本地收尾未完成或候选适用性错配时停止并返回 `$desktop-build-rust-release`，只有明确发布流程可以事先提交。调用 `$desktop-manage-version check --phase build`，并要求固定 `version` 输入精确等于门禁返回的当前目标；任何运行器都不得提升版本或重置周期。在派发前及每个原生运行器的任何测试/编译前，只读运行 `python3 .agents/skills/desktop-prepare-release/scripts/release_notes.py check --file release-notes.json --expected-version <version>` 并计算 SHA-256；缺失、不一致或格式无效时失败，本 Skill 不得生成或改写更新日志。`e2e_selection` 只接受 `enabled`/`disabled`，由 `$desktop-build-rust-release` 完成逐次解析后提供。存在用户要求的活动 Todo 时不得构建尚未完成的范围，但 Work Plan 不是前置条件。必须在禁用凭据持久化的情况下检出该提交，并在执行仓库代码或签名钩子前验证当前分支是提供方动态默认 `main`/`master`、`HEAD == source_commit` 且工作树 clean。不得推断发布授权。
...
- 缺少构建授权、调用方或运行器 dirty、HEAD/source_commit/动态默认 `main`/`master` 不一致、状态摘要或双信封不一致、远端跟踪快照/关闭历史/适用性复核失败、MSRV 设置失败、测试缺失或失败、构建失败、已启动原生作业失败、候选缺失、版本不匹配、条件签名失败、校验和生成失败或结果取回不完整时，必须失败。
- 普通构建永不自动暂存或提交；只有 `$desktop-prepare-release` 在用户明确发布后可以先形成并推送受控 feature 提交、原子关闭登记链并严格快进动态默认分支，再把本地/远端一致的 clean 默认分支 closing commit 交给本工作流。
