...
9. 把最终候选按 `<product>-v<version>-<platform>-<arch>.<ext>` 打包到项目根同级唯一暂存目录，随后生成相邻 SHA-256 和清单。清单必须精确包含 `project`、`version`、`sourceCommit`、`buildRun`、`buildMode`、`platform`、`architecture`、`target`、`host`、`archive`、`sha256`、`tests`、`e2eSelection`、`signingStatus`、`signingReason`、结构化 `signingEvidence` 和 `milestoneAcceptance` 字段，其中必须记录 `milestoneAcceptance: pending`。默认就地钩子必须记录固定钩子验证且不包含独立签名文件；`unsigned` 候选把验证记录为不适用。要求暂存目录文件名集合精确等于这三个普通文件。
