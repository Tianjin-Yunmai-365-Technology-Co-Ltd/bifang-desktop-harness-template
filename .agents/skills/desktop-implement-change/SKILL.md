...
12. 每完成一个逻辑闭环，都紧邻真实提交调用 `$desktop-configure-git-commits`，以表达结果的提交信息把当前范围的精确路径创建为可审查的本地提交，正常运行 hooks 且不得使用 `--no-verify`。普通完成到此为止，不自动 push，也不删除本周期的 feature 分支、并行单元分支或 Worktree。只有用户明确说“推送”时，才调用 `$desktop-manage-git-lifecycle publish`：把当前发布周期登记分支普通合并到主分支，切换到主分支并推送主分支；该操作不创建标签、不清理本周期资源。`--remote` 继续指定唯一主/发布远端；只有用户对本次推送明确授权其他具名目标时，才把每个目标按授权顺序作为可重复的 `--also-remote <name>` 传给 `publish`，不得从已配置远端推断授权。helper 必须在首个 push 前解析全部目标，补充远端不写入生命周期状态，只接收同一最终 HEAD，并按各自 advertised default branch 非强制推送和逐个复读；补充远端不参与 release、tag 或清理。跨远端推送不是原子操作，后续目标失败时如实报告部分成功，并保留完全相同参数的同一参数幂等重试路径。只有用户明确说“发布”时，才路由 `$desktop-prepare-release`，按“把登记分支普通合并到主分支并切换主分支 → 推送并复读主分支 → 创建 `v{版本}-{日期}` 标签并推送主远端 → 确认主远端标签成功 → 删除本周期登记的 Worktree、主远端分支和本地分支”的固定顺序执行；`release` 不接受补充远端。任何明确要求的 push 或 release 失败都阻断对应操作，不 force、不更换 remote、不创建或配置远端/凭据，也不得在标签远端成功前执行删除。最终交付前确认任务要求的测试已执行、权威文档已同步、当前范围已按本次要求提交，且 `git status --porcelain=v1 --untracked-files=all` 为空；随后报告当前分支、提交哈希、实际验证、未执行项和剩余风险，只有实际执行推送或发布时才报告每个实际目标的远端复读结果。
...
- `$desktop-manage-git-lifecycle start` 只建立或复用本地 feature 开发周期，不要求或配置 remote。`publish` 只在用户明确要求“推送”时合并、切换并推送主分支；用户另行明确授权时可用重复的 `--also-remote <name>` 把同一最终 HEAD 补充推送到其他已配置远端，不打标签、不清理。`--remote`/`state.remote` 始终是唯一主/发布远端，补充远端不写入生命周期状态，不参与 release、tag 或清理。正式“发布”由 `$desktop-prepare-release` 独立授权主远端标签与本周期精确资源清理，并保证远端标签成功发生在任何删除之前。两条路径都不授权创建或配置 remote/凭据、force push、扫描删除未登记分支或引入中转发布分支。
