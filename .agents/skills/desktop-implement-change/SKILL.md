...
完成当前授权范围。新功能或独立 Bug 以“自动创建或复用本发布周期 feature 分支 + 实现 + 本次必要单元测试 + 事件触发记录 + 可审查的本地提交”为日常开发闭环；普通完成不自动推送。用户明确说“推送”时才合并到主分支、切换主分支并推送；用户明确说“发布”时才进入逐次选择本地或远端、带标签和周期资源清理的正式发布流程。
...
12. 每完成一个逻辑闭环，都紧邻真实提交调用 `$desktop-configure-git-commits`，以表达结果的提交信息把当前范围的精确路径创建为可审查的本地提交，正常运行 hooks 且不得使用 `--no-verify`。普通完成到此为止，不自动 push，也不删除本周期的 feature 分支、并行单元分支或 Worktree。只有用户明确说“推送”时，才调用 `$desktop-manage-git-lifecycle publish`：把当前发布周期登记分支普通合并到主分支，切换到主分支并推送主分支；该操作不创建标签、不清理本周期资源。`--remote` 继续指定唯一主远端；只有用户对本次推送明确授权其他具名目标时，才把每个目标按授权顺序作为可重复的 `--also-remote <name>` 传给 `publish`，不得从已配置远端推断授权。helper 必须在首个 push 前解析全部目标，补充远端不写入生命周期状态，只接收同一最终 HEAD，并按各自 advertised default branch 非强制推送和逐个复读；补充远端不参与 release、tag 或清理。跨远端推送不是原子操作，后续目标失败时如实报告部分成功，并保留完全相同参数的同一参数幂等重试路径。只有用户明确说“发布”时，才路由 `$desktop-prepare-release` 并在开始时询问/锁定 `gitPublication: local | remote`；tracked 发布上下文是唯一冻结选择，两种调用都必须传精确 `--release-context-sha256 <sha256>`。本地模式使用 `release ... --release-context-sha256 <sha256> --local-only`，按“合并登记分支并切换本地默认主分支 → 创建并复读本地 `v{版本}-{日期}` → 删除登记 Worktree 和本地分支”执行，完全不访问远端；远端模式使用 `release ... --release-context-sha256 <sha256> --remote <name>`，首次只在本地整合并冻结 final HEAD，再按“推送/复读主分支 → 创建、推送并复读 tag → 删除登记 Worktree、主远端分支和本地分支”执行。`release` 不接受补充远端；pending HEAD 非空的重试不得重新 fetch/merge 或吸收远端漂移。任何明确要求的 push、上下文不一致或模式内 release 门禁失败都阻断对应操作，不 force、不更换模式或 remote、不创建或配置远端/凭据，也不得在模式对应 tag 复读成功前执行删除。最终交付前确认任务要求的测试已执行、权威文档已同步、当前范围已按本次要求提交，且 `git status --porcelain=v1 --untracked-files=all` 为空；随后报告当前分支、提交哈希、实际验证、未执行项和剩余风险，只有实际执行远端推送或远端发布时才报告实际目标的远端复读结果。
13. 第 12 步要求的结果、验证、文档、本地提交、clean 状态，以及用户本次明确要求的推送或发布全部完成后，用户可见 Task 才把标题进度更新为终态 `已完成` 并按真实 `threadId` 用 `list_threads` 有界复读。更新失败不创建替代 Task、不无限重试、不推翻已经完成的结果，并在最终回复报告最后确认状态；阻断时保留最后真实进度。

## 边界与输出

- `$desktop-manage-git-lifecycle start` 只建立或复用本地 feature 开发周期，不要求或配置 remote。`publish` 只在用户明确要求“推送”时合并、切换并推送主分支；用户另行明确授权时可用重复的 `--also-remote <name>` 把同一最终 HEAD 补充推送到其他已配置远端，不打标签、不清理。`--remote`/`state.remote` 始终是唯一主远端，补充远端不写入生命周期状态，不参与 release、tag 或清理。正式“发布”由 `$desktop-prepare-release` 独立锁定 `gitPublication` 并授权本周期精确资源清理：`local` 只以本地 tag 成功为删除前提，`remote` 才授权主分支/tag 远端门禁与主远端分支清理。两条路径都不授权创建或配置 remote/凭据、force push、扫描删除未登记分支或引入中转发布分支。
