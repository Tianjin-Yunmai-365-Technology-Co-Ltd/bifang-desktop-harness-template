...
1. 构建前运行 `node .agents/skills/desktop-test-gui-initialization-e2e/scripts/verify-gui-lifecycle-contract.mjs --root . --gui-dir <project-id>_gui`。检查器必须读取配置块并条件验证：
   ...
   - `global_shortcut: enabled`：固定 `CommandOrControl+Shift+Space → restore_main_window`、Rust-only 插件、真实注册状态/冲突反馈、退出注销、设置页状态与四个固定回归；disabled 时全部缺席。
   ...
   - 前端：`/settings` 与设置页组件始终存在；通知/自启 Switch 及 i18n 严格按选择存在，全局快捷键启用时显示不可编辑的固定 chord 与真实注册/冲突状态；不得通过 localStorage/sessionStorage、Jotai atom 或 TanStack Query 建立能力状态的第二权威副本。关于/赞助路由和运行时组件分别与选择一致；实际侧栏接线必须等于 `sidebar_mode`。
...
10. 仅当全局快捷键启用时，实际触发 `CommandOrControl+Shift+Space` 并确认同一主窗口恢复；注册冲突必须在设置页可见。随后注销并确认键位释放，清理失败阻断。托盘未启用时另用原生关闭控件确认最后窗口关闭即退出。
11. 无论成功、失败、超时或取消，都先恢复本 Skill 修改的开机自启状态、window-state 数据和全局快捷键注册，再终止并等待回收本 Skill 拥有的全部进程，禁止遗留 detached task、测试登录项、状态文件或 chord。
...
- 已选单实例、托盘、系统通知、开机自启、深链接与全局快捷键分别通过适用结构和可执行宿主场景；需要打包的 macOS 深链接明确留作阻断式后续验证；自启、窗口状态和快捷键均已恢复；
- 发布专用映射始终存在且未污染调试构建；关于页启用时命令/加载失败状态存在，禁用时运行时链路缺席；关于/赞助入口和页面按选择存在或缺席；
- 所有进程句柄已回收。

## 结果边界

初始化最终回复逐项报告九项配置、三项固定基线、结构检查、构建、二进制路径、侧栏模式/持久折叠、设置页与逐菜单结果。六个条件宿主能力及关于/赞助只报告适用场景；开机自启、window-state 和快捷键报告恢复后状态，系统通知/需打包深链明确标记未在此证明的边界；未选项报告缺席证据。
