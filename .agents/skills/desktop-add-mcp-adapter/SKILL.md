...
6. 执行时检查软件包仓库和 MCP Rust SDK 官方文档。Rust MSRV 下界为 `1.95`，已存在更高稳定工具链时直接通过且不得降级或重装。截至 2026-09-01 的 MCP 最低兼容稳定下界为 `rmcp 3.1.4`、`tokio 1.53.1`、`serde 1.0.229` 和 `serde_json 1.0.151`。写入清单前优先采用同时满足 Rust 1.95、所用 API、目标平台、安全约束与特性兼容性的最新非预发布稳定版作为新下界；最新版冲突时保留上述下界并记录原因。使用普通完整三段 Cargo 兼容范围，关闭默认特性，只启用已批准的服务器、宏和 stdio 传输特性，不得写入精确 `=`、`latest`、通配符或 tag。缺少 `rust-version` 不能视为 MSRV 兼容依据；必须以临时最低直接版本解析和 Rust 1.95 证明兼容性，再由正常 `Cargo.lock` 固定实际解析结果。
