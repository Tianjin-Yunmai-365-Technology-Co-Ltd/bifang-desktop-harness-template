...
- 当前经过元数据筛选的完整三段下界为 `rmcp 3.1.4`、`tokio 1.53.1`、`serde 1.0.229` 与 `serde_json 1.0.151`。调用时先查询 registry，优先采用满足 Rust 1.95、目标平台、所用 API、安全和特性约束的最新非预发布稳定版作为新的 Cargo 兼容下界；以最低直接版本解析和 Rust 1.95 证明兼容，再由正常锁文件固定实际解析结果。crate 未声明 MSRV 表示未知，不能作为兼容证据；清单不得使用精确 `=`、字面 `latest`、通配符或 tag。
...
- Rust 1.95 最低直接版本和每个声称支持的原生平台都有实际证据；否则标记为 `Unverified`。
