...
4. 解析初始化需要的接口，只能从 `CLI`、`TUI`、`MCP` 和 `GUI` 中选择，并允许任意组合。由 `$desktop-instantiate-project` 发起时，必须复用首次写入前已经确认的初始化表单，不得在复制后重新询问接口。直接调用本 Skill 时，接口组合和 Agent 策略模式是直接初始化的基础决定；任一尚未解析时，必须在同一首轮一次列出全部未解析基础决定，不能先问接口、下一轮再问策略模式。用户明确选择默认值或跳过接口选择时记录 `CLI`；用户明确选择其他接口时，不得静默附加 CLI。
