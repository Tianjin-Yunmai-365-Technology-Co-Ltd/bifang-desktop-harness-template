...
4. 解析初始化需要的接口，只能从 `CLI`、`TUI`、`MCP` 和 `GUI` 中选择，并允许任意组合。由 `$desktop-instantiate-project` 发起时，必须复用首次写入前已经确认的初始化表单，不得在复制后重新询问接口；直接调用本 Skill 且接口尚未解析时，每轮只询问一个接口组合字段。用户明确选择默认值或跳过接口选择时记录 `CLI`；用户明确选择其他接口时，不得静默附加 CLI。
   - 只有选择包含 `GUI` 时才解析 GUI 初始化配置。由实例化流程发起时，复用表单中已经逐项确认的五项值；直接调用时每轮只询问一个尚未解析的 GUI 字段，依次把系统托盘、关于页、赞助页、单实例四项能力解析为 `enabled` 或 `disabled`，再向用户提供 `compact`（精简）与 `detailed`（详细）侧栏模式选择。四项能力不得根据 Harness 旧基线、推荐策略预设或未勾选状态静默推断；用户省略、留空或跳过侧栏选择时，未选择侧栏模式必须写入 `sidebar_mode = detailed`。用户明确选择 `compact` 或 `detailed` 时保持原值；显式非法值不得按未选择处理，必须重新确认。
