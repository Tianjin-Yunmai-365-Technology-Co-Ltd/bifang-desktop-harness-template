...
13. During ordinary implementation, run discovered Rust/frontend formatting, typecheck, lint, non-empty tests, relevant check/build checks, and targeted interaction/accessibility tests. When the user initiates a final-artifact build or prepares a release, first capture this run's release-stage selection before the production build starts. Always run the locked desktop production build, final-artifact existence, and real application startup smoke. Run approved UI acceptance, including `$test-final-artifact-e2e` when appropriate, only when the user explicitly selected that check for this run or an approved product/channel rule makes it mandatory; never assume prior approval. If a selected interactive acceptance check fails, times out, is cancelled, or is not executed after selection, block packaging and treat the candidate as not ready. Missing signing identity, certificate, notarization credentials or updater key must not block local release-stage verification; record the artifact as unsigned. If optional interactive acceptance was not selected, record `Not run` plus residual risk. A delivery-status review only inspects existing evidence and does not start a build or interactive acceptance. Verify only the current native platform unless native evidence exists for others; mark untested platforms `Unverified`.
