...
11. When the user requests delivery acceptance or prepares a release, run the locked release build, final-artifact existence, real binary smoke, and available MCP inspector or target-host acceptance, then use `$verify-delivery`. In either path update affected product memory and mark untested hosts/platforms `Unverified`; do not claim human approval.
