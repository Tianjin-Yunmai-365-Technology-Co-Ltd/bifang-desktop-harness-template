...
- 通知插件在单实例插件之后注册；若单实例未启用，它可以是首个插件。WebView 不安装 `@tauri-apps/plugin-notification`，也不授予 `notification:*` capability；只公开设置所需的窄 Rust command。有效 bundle identifier 和打包应用签名属于 macOS 原生通知验收前提。
