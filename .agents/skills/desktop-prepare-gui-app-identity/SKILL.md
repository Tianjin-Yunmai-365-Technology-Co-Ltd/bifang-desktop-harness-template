...
3. 把 3 个候选作为独立图片同时展示并编号，不得用拼图遮蔽细节；必须等待用户明确选择其中一个，不能静默采用第一张，也不能以中性占位图完成 GUI 初始化。选择前候选只能位于本次任务精确拥有的临时目录；选择后只把选中候选逐字节保存为 `<project-id>_gui/src-tauri/icons/app-icon-master.png`，并逐字节复制为 `<project-id>_gui/public/app-identity/logo.png`。使用项目本地真实 Tauri `icon` 命令从该母版生成平台图标集；不得手工编写二进制图标文件。拒绝的临时候选可在确认最终路径与摘要后清理，不得删除用户原始上传。
4. 初始化时创建或更新 `docs/GUI_APP_PROFILE.md`，记录状态、3 个候选的方向/来源/预览证据/SHA-256、用户选择、1024×1024 母版路径、运行时 Logo 路径、两者字节一致证据、平台变体和仍待首次真实 GUI 开发确认的元数据。Logo 选择不批准产品业务，也不允许虚构商标或法律权属；初始化基线提交不得残留未选择状态或临时占位图。
...
8. 将资料直接交给 `$desktop-add-gui-adapter`：选中 Logo 必须接入 Tauri 平台图标和侧栏顶部，当前版本紧随 Logo 下方；不自动创建 Work Plan 或完整验收。更新显示名后同时验证平台图标、侧栏 Logo、原生标题、`document.title` 和关于页仍消费同一身份与权威版本。固定标题、关于页与赞助页已经由 GUI 初始化启用；只有修改这些默认界面、增加其他支持界面、更新检查或遥测时，才再次调用 `$desktop-prepare-gui-support-surfaces`。
...
报告 3 个候选的预览与摘要、用户选择、母版/运行时 Logo/平台图标证据、已批准和未解决的元数据、资料位置、打包阻断项，以及下一项 GUI 实施操作。
