...
1. 读取 `docs/RELEASE.md`、活动构建请求、本次构建的 E2E 选择，以及用户提供的每项已完成构建结果；只有候选已经经过独立验收时才读取与其状态匹配的可用验收证据。要求规范化的 Git 顶层目录等于项目根。在接触目标目录前，确定项目、版本、源码提交、平台矩阵、产物名称、校验和、构建标识、签名状态和候选状态。
