...
2. 确认模式与批准。`$instantiate-project` 期间是中性“实例化身份重置”，用户提供的目标身份即构成替换所复制 Harness 身份的批准，不建立产品里程碑。除此之外均是“现有产品改名”：它会改变公开身份、路径、配置或许可证，必须先通过 `$define-product` 确认范围，再由 `$plan-change` 建立里程碑路径 Todo；不得以快速或标准路径完成。
...
9. 实例化身份重置完成后返回 `$instantiate-project`/`$initialize-rust-project` 继续中性收尾，不调用 `$verify-delivery`。现有产品改名则持续完成里程碑计划中的全部 Todo，构建包含受影响公开接口、包身份、GUI/发布元数据和许可证的完整真实候选，再交给 `$verify-delivery` 验收；失败时重开 Todo，不得以残留扫描或局部测试替代完整候选。
