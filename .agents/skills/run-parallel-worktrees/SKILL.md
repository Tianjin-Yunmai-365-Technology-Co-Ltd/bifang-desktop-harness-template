...
1. 读取产品规格、活动 Todo 与里程碑工作计划、日期最新的 ADR、工程规则和相关文件。
...
5. 整合后运行非空单元或回归测试，以及相关格式、代码规范检查、静态、集成或契约检查。Todo 实施期间不得运行冒烟或 E2E。
6. 只有整合后的开发检查通过，才能把 Todo 标记为 `done`。整个批次均为 `done` 后，把真实候选交给 `$verify-delivery`。
