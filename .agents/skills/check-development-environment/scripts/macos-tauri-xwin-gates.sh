...
# 使用 Homebrew 事实定位 keg-only LLVM/NSIS；查询不安装、不更新也不修改环境。
...
probe_all() {
    ...
    llvm_missing=0
    if [ -z "$llvm_rc_path" ] || [ -z "$lld_link_path" ] ||
        ! "$llvm_rc_path" --version >/dev/null 2>&1 ||
        ! "$lld_link_path" --version >/dev/null 2>&1; then
        llvm_missing=1
    fi
    nsis_missing=0
    ...
    xwin_missing=0
    if [ -z "$cargo_xwin_path" ] || ! "$cargo_xwin_path" --version >/dev/null 2>&1; then
        xwin_missing=1
    fi
    target_missing=1
...
brew_path=$(find_tool brew 2>/dev/null || true)
if [ "$llvm_missing" -eq 1 ] || [ "$nsis_missing" -eq 1 ]; then
    [ -n "$brew_path" ] || {
        emit_status
        printf '%s\n' "错误：自动安装 LLVM/NSIS 需要既有 Homebrew；本门禁不自动安装 Homebrew" >&2
        exit 32
    }
fi

if [ "$llvm_missing" -eq 1 ]; then
    if "$brew_path" --prefix llvm >/dev/null 2>&1; then
        printf '%s\n' "错误：既有 LLVM 缺少 llvm-rc 或 lld-link，拒绝静默升级/重装" >&2
        exit 33
    fi
...
if [ "$nsis_missing" -eq 1 ]; then
    if "$brew_path" --prefix nsis >/dev/null 2>&1; then
...
if [ "$xwin_missing" -eq 1 ]; then
    ...
    "$cargo_path" install --locked cargo-xwin || {
...
path_prepend=
for candidate in "$LLVM_BIN_DIR" "$NSIS_BIN_DIR" "$CARGO_BIN_DIR"; do
