...
- Parse stable `gate.<environment>.<field>=<value>` lines. Rust is always required. Windows also requires MSVC Build Tools. Node.js and pnpm are required only when the selection contains `GUI`; otherwise both must be `not-required`.
...
| Node.js | `GUI` selected | `node --version` | Install the supported Node.js LTS from an official, checksum-verified host archive, then re-probe. |
| pnpm | `GUI` selected | `pnpm --version` | Use Node's npm client to install stable `pnpm@latest` from the official npm registry into a user-level prefix, then re-probe. |
...
- pnpm is installed only for GUI through the npm client delivered with Node.js. npm verifies registry integrity metadata; the gate never executes downloaded text, disables TLS, or selects prerelease pnpm.
