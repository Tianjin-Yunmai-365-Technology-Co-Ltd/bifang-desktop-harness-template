...
> GUI 初始化已经包含固定动态标题、默认收起且顶部 Logo→版本的左侧菜单、设置页、关于页和亮色/暗色赞助页。仅在产品要修改该基线、增加其他支持界面或配置更新/统计等出站能力后，把本文件复制为 `docs/GUI_SUPPORT_SURFACES.md`。删除所有未选择章节和模板说明，不得把本模板本身当作产品批准记录。
...
- 默认：enabled；固定在窗口左侧并支持展开/折叠，折叠状态由 GUI 本地交互状态持有且初始为收起。
- Logo 与版本：`docs/GUI_APP_PROFILE.md` 选中的本地 Logo 永远位于最顶部，当前权威打包版本紧随其下；展开和折叠状态都直接显示二者，不得只放在 tooltip 或关于页。
...
- [ ] 侧栏默认收起；选中 Logo 在版本上方且展开/折叠都直接可见；功能从顶部向下增长，固定底部顺序为赞助、设置、关于。
