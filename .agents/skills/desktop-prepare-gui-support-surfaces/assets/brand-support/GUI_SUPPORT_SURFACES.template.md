...
> GUI 初始化已经包含动态标题、所选精简/详细 Logo→版本侧栏、不含隐私/统计区块的语言与三态主题设置及应用级亮暗语义主题；系统托盘、系统通知、开机自启、关于页、赞助页、单实例、深链接和全局快捷键由 `docs/GUI_APP_PROFILE.md` 逐项选择，全局快捷键界面再服从独立 action contract。仅在产品要修改该基线、增加界面或配置出站能力后，把本文件复制为 `docs/GUI_SUPPORT_SURFACES.md`。
...
- [ ] `/settings` 固定存在；系统通知/开机自启 Switch、`/about`、`/sponsor` 及运行时媒体严格按九项初始化选择存在或缺席；全局快捷键再按 contract 分流为空动作无占位、固定动作只读状态、可编辑动作录制/取消/清空。
