...
> GUI 初始化已经包含固定动态标题、图标/Tooltip 完整且默认收起的 Logo→版本侧栏、三态主题设置、应用级亮色/暗色语义主题、含更新入口的关于页和双主题赞助页。仅在产品要修改该基线、增加其他支持界面或配置更新/统计等出站能力后，把本文件复制为 `docs/GUI_SUPPORT_SURFACES.md`。删除所有未选择章节和模板说明，不得把本模板本身当作产品批准记录。
...
- 菜单图标：每个功能项和赞助/设置/关于固定项都必须提供图标；折叠时显示图标与本地化 Tooltip，展开时显示图标与名称，两种状态均保留可访问名称。
