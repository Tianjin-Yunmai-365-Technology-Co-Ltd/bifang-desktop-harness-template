...
> GUI 初始化已经包含固定动态标题、`136px` 单态图标上文字下的 Logo→版本侧栏、不含隐私/统计区块的语言与三态主题设置、应用级亮色/暗色语义主题、含更新入口的关于页和双主题赞助页。仅在产品要修改该基线、增加其他支持界面或配置更新/统计等出站能力后，把本文件复制为 `docs/GUI_SUPPORT_SURFACES.md`。删除所有未选择章节和模板说明，不得把本模板本身当作产品批准记录。
...
- [ ] 侧栏固定为 `136px` 单态且没有展开/折叠控件；`56px` Logo 在版本上方，菜单为 `30px` 图标上方、`11px`/`10em` 文字下方且居中；功能从顶部向下增长，固定底部顺序为赞助、设置、关于。
