...
固定模板以 `@tabler/icons-react` 作为唯一图标库。`compact` 精简侧栏固定 `136px`，使用 `56px` Logo 和 `30px` 图标上/`11px`、`10em` 文字下布局，名称持续可见。`detailed` 详细侧栏首次默认 `248px` 展开并显示图标+名称，按钮可收起为 `76px` 图标模式，名称通过 Mantine `Tooltip` 显示；折叠状态使用独立 localStorage 键设备级持久化。两种模式都保留完整可访问名称，按钮事件绑定在按钮自身。

## 工作流程

1. 先判断调用模式。读取 `docs/ENGINEERING_RULES.md`；初始化中必须读取并校验 `docs/GUI_APP_PROFILE.md` 的唯一 `gui-initialization-config` 代码块，确认四项能力为 `enabled`/`disabled`、侧栏模式为 `compact`/`detailed` 且无 `pending`；侧栏未选择时的 `detailed` 缺省必须已由初始化器写入，本 Skill 不得重复询问、再次应用缺省值或以旧默认覆盖。初始化后增加能力时再读取 Product Spec/ADR 并先过相应范围门禁。未选择 GUI 时停止。
2. 固定本地基线包括标题公式 `{applicationName} v{version} {contactChannel}:{contactValue}`、本地 `/app-identity/logo.png`、设置页、i18n、三态主题与应用级亮暗语义。所有 GUI 复制 `tauri/tauri.release.conf.json` 作为正式构建专用合并配置；它只把根 `release-notes.json` 映射为候选资源 `release-notes.json`，中性调试构建不使用该配置。侧栏由 `AppSidebarTemplate.tsx` 的 `mode` 参数选择：精简模式固定 `136px` 且图标上文字下；详细模式默认 `248px` 展开、`76px` 收起，展开显示图标+名称，收起只显示图标并以 Tooltip 揭示名称，状态使用 `APP_SIDEBAR_COLLAPSED_STORAGE_KEY` 持久化。产品功能从顶部向下增长；底部由 `buildSupportNavigationItems` 组装为已选赞助、固定设置、已选关于。`/about` 与 `/sponsor` 只在对应配置启用时建立，未选时没有路由、菜单或占位。关于页启用时包含检查更新，并复制 `rust/release_notes.rs`、`react/releaseNotesResource.ts` 及其回归，注册 `load_release_notes`，包含候选资源的 loading/error/retry、近 5 版更新日志、作者、联系方式和免责声明；赞助页启用时包含品牌内容与媒体。页面会话状态继续使用应用根 Jotai store，仅在当前进程跨路由保存，不与侧栏设备偏好混用。
...
11. 使用 `$desktop-implement-change` 直接实施。非空回归必须覆盖配置块、发布专用资源映射、两个侧栏模式、详细模式默认展开/按钮事件/持久恢复/折叠 Tooltip、精简模式持续名称、支持菜单的条件组装、未选关于/赞助没有入口，以及设置页/主题/i18n 固定基线。关于页启用时必须覆盖 Rust 异步资源读取与 schema 失败关闭、Tauri handler 注册、前端窄命令解码、加载失败/重试和五版/十条显示；禁用时命令、加载器、弹窗和路由缺席。赞助、托盘各自的页面/媒体/i18n 测试只在启用时要求，并同时覆盖禁用态不接线。页面会话状态继续以同一根 store 与新 store 分别验证进程内保留和退出重置；侧栏折叠偏好则明确使用独立 localStorage 键。
