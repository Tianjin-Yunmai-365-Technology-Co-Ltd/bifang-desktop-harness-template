...
4. 先划分职责再实现：
   ...
   - 窗口标题、系统浏览器、原生通知、平台元数据采集、Tauri 权限和 WebView 展示属于 GUI adapter；React 只拥有路由、展示和纯交互状态，语言与主题选择使用设备级本地偏好，不进入 core。普通页面工作状态由应用根 Jotai store 在本次进程跨路由保存，退出后重置且禁止持久化/Query 镜像；成功空页在 page>1 时回第 1 页。列表页改由 `$mantine-list-view` 约束 URL/session 查询恢复、Query-only 行数据、localStorage 列偏好和越界末页。交互 handler 绑定在实际拥有动作的按钮、链接、`Switch`、`Checkbox` 或菜单项本身，不得由 Card、`Table.Tr`、`Table.Td` 等父级代理；
