...
| `react/AppSidebarTemplate.tsx` | 固定可收起左侧菜单、持续可见版本、顶部功能区和贴底支持区 | GUI 初始化默认壳层 |
...
3. 将 Sidebar、Settings、About、Sponsor、MandatoryUpdateGate 与更新展示类型复制到产品前端并建立 `/settings`、`/about`、`/sponsor` 文件路由。侧栏功能项从顶部向下增长，固定底部按赞助、设置、关于渲染；展开和折叠时版本都直接可见。保留 Mantine、响应式 `SimpleGrid`、本地路径校验、图片替代文本以及视频 captions/transcript 约束。
...
8. 初始化运行模板非空测试、路由/导航、i18n/主题/键盘和媒体摘要检查；生产构建和最终 `dist` 扫描只在显式构建或对应治理事件触发。真实候选验收仍只在里程碑触发。
