...
description: 通过 Computer Use 对完整真实 CLI、TUI、MCP 宿主或 GUI 产物执行里程碑阶段端到端验收。仅可由 $desktop-verify-delivery 在每个 Todo 均为 done，且持久项目策略启用 E2E 或已批准产品/渠道规则要求 E2E 时调用；绝不得在规划、编码、常规构建、收集或发布元数据工作中使用。
---
...
2. 要求里程碑批次中的每个 Todo 均为 `done`，并且 `$desktop-verify-delivery` 已进入里程碑验收。否则必须停止并报告 `Not run`。
...
10. 任何失败、超时、取消或已选但未运行的场景出现时，必须记录证据、拒绝里程碑、重开或新增修复 Todo，并返回 `$desktop-implement-change`。修复后，重新运行完整里程碑场景集。
11. 将未实际执行的操作系统、显示环境、浏览器、终端、MCP 宿主和软件包格式标记为 `Unverified`。项目、渠道或不可逆交付要求人工最终复核时，它仍是独立步骤；普通里程碑不凭空增加签署要求。
