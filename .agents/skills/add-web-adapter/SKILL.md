...
9. Test core mappings, routes/API schemas, query loading/error/refetch behavior, client-state transitions, highest-risk failure, authorization boundary, input validation, deterministic browser keyboard/accessibility behavior, and shutdown. Defer real final-artifact browser acceptance to the explicitly selected release-stage path in step 10.
10. During ordinary implementation, run format, typecheck, lint, non-empty tests, relevant build checks, and targeted browser/API tests. When the user initiates a final-artifact build or prepares a release, first capture this run's release-stage selection before the production build starts. Always run the locked production build, artifact existence, and bounded server startup/shutdown smoke. Run `$test-final-artifact-e2e` for final browser flows only when the user explicitly selected it for this run or an approved product/channel rule makes it mandatory; never assume prior approval. If a selected E2E check fails, times out, is cancelled, or is not executed after selection, block packaging and hand the failed evidence to `$verify-delivery`. If it was not selected and is not mandatory, record `Not run` plus residual risk. A delivery-status review only inspects existing evidence and does not start these actions.
