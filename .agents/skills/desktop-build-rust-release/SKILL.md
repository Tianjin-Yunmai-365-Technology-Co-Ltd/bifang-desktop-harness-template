...
1. 读取 `Cargo.toml`、`docs/RUST_CLI_TEMPLATE.md`、`docs/AGENT_POLICY.md`、`docs/RELEASE.md`，以及任何已批准的发布渠道或签名配置；只有 E2E、完整验收或发布被独立触发时才读取其相关验证记录。依据 Cargo 元数据和仓库事实确定软件包、二进制文件、版本、目标平台和产物名称。在任何测试、编译或 `release/` 清理前先调用 `$desktop-manage-git-branch-chain verify-release-review`，只接受当前 clean 动态默认 `main`/`master` closing commit 中已封存且复算通过的 `lastClosedChain.releaseReview` 与 `candidateSelections`；Rust 非 GUI 候选要求后者的性能与 macOS 签名选择都精确为 `not-applicable`。其返回的 `releaseBranch` 必须等于动态默认分支，`releaseHead` 是后续唯一构建提交；审查选择、状态、范围摘要、启用证据或 `Not run` 原因/风险都只能从该信封读取，不能从对话参数补写、重新询问或推断。命令失败时，有活动 feature 链则转入 `$desktop-prepare-release`，否则只报告 `Not ready`；旧式无信封关闭提交、未完成远端/本地收尾、候选适用性不符或禁用审查与当前安全/渠道硬要求冲突时都不得形成候选。随后解析当前构建的 E2E 选择：本次请求已明确 `enabled`/`disabled` 时直接复用，否则在任何测试或编译前询问用户一次；`milestone_e2e` 只作为建议默认值。E2E 选择和封存的发布审查选择都只属于本次候选，不得静默写回持久策略；同一 closing commit 重跑复用其信封，新发布重新由 prepare-release 解析。构建 Skill 不自行执行发布语义审查。
   若发现目标是 Tauri GUI 安装包，停止本 Skill 并转交 `$desktop-build-tauri-release`；不得用 CLI raw-binary 打包/签名模型处理 DMG、NSIS 或公证。
2. 要求本次构建已由用户显式请求，下游项目根同时是其独立 Git 顶层目录。立即把 `git rev-parse --verify HEAD^{commit}` 的 40 位小写结果锁定为 `buildSourceCommit`，要求它逐字等于上一步已验证的 `releaseHead`，确认当前分支精确等于 `releaseBranch` 与远端动态默认分支、`HEAD` 解析到真实源码提交，并要求 `git status --porcelain=v1 --untracked-files=all` 为空；dirty、无 HEAD、非默认分支、提交不等或符号提交歧义都停止。普通构建只报告“请先提交或改用明确发布流程”，绝不得自行调用 `git add`/`git commit`、身份 bootstrap 或提交模板配置。先调用 `$desktop-manage-version check --phase build`，确认根 Cargo 当前版本与 `.harness/version-state.json` 目标一致；构建不得计算、提升版本或重置正式发布周期。随后在任何测试或编译前只读运行 `python3 .agents/skills/desktop-prepare-release/scripts/release_notes.py check --file release-notes.json --expected-version <Cargo-version>`，并计算该普通非符号链接文件的 SHA-256；缺失、不一致、超过近 5 版/每类 10 条或格式无效时停止并要求先执行 `$desktop-prepare-release` 的候选前本地提交与更新日志阶段，本 Skill 绝不得生成或改写更新日志。确认 `Cargo.lock`、`rust-version`、分支、宿主和架构；存在用户要求的活动 Todo 时不得构建尚未完成的范围，但 Work Plan 不是构建前置条件。
...
8. 仅打包最终已签名或明确为 `unsigned` 的二进制文件，并把项目根 `release-notes.json` 以相对路径 `release-notes.json` 原样纳入同一归档；若原产物模型是裸二进制，必须改用包含二进制与更新日志的确定性归档，不能把更新日志作为 `release/` 中第四个旁路文件。每次会改变字节的打包或签名操作完成后，必须计算 SHA-256。在写 manifest 前再次运行只读 `verify-release-review`，要求返回的 `releaseBranch`/`releaseHead`/完整信封逐字段等于第 1 步，并要求工作树 clean、当前分支仍为同一默认分支且 `HEAD == buildSourceCommit`。在项目根同级唯一暂存目录中创建归档、相邻校验和及清单；不得把 Cargo 中间产物放入其中。验证暂存目录仅包含清单声明的三个普通文件，随后通过不跟随链接的目录级原子替换，把完整目录提交到项目根 `release/`。移动后重新验证最终路径和精确文件集。每份清单必须精确记录 `project`、机器 `version`、`sourceCommit`（值必须等于 `buildSourceCommit`/当前默认分支 `HEAD`）、`buildRun`、`buildMode`、`platform`、`architecture`、`target`、`host`、`archive`、`sha256`、`tests`、`e2eSelection`、信封中的 `reviewSelection`/`reviewStatus`、带一个小写 `v` 的 `releaseNotesVersion`、`releaseNotesSha256`、`releaseNotesPath: release-notes.json`、结构化 `signingEvidence` 和 `milestoneAcceptance: pending`。`reviewSelection: enabled` 时，结构化 `reviewEvidence` 必须逐字段复制信封的 `scopeBase`、`sourceHead`、`scopeDiffSha256`、五项 `checks` 与 `evidenceSummary`，独立 `reviewedSourceCommit` 必须等于信封同名字段；不得重写摘要或补充对话内证据。`disabled` 且无硬要求时只复制非空 `reviewReason`/`reviewRemainingRisk`，且 `reviewEvidence` 与 `reviewedSourceCommit` 都缺席。对于默认就地钩子，签名证据记录固定钩子验证是否成功退出，并保持 `detachedFiles` 为空；`unsigned` 结果把验证记录为不适用。
