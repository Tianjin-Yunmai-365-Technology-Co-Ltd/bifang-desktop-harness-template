...
1. 读取 `Cargo.toml`、`docs/RUST_CLI_TEMPLATE.md`、`docs/AGENT_POLICY.md`、`docs/RELEASE.md`，以及任何已批准的发布渠道或签名配置；只有 E2E、完整验收或发布被独立触发时才读取其相关验证记录。依据 Cargo 元数据和仓库事实确定软件包、二进制文件、版本、目标平台和产物名称。在任何测试、编译或 `release/` 清理前运行 `python3 .agents/skills/desktop-prepare-release/scripts/release_context.py verify --project-root .`，只接受工作树 clean、当前具名分支等于远端动态默认分支、远端同名分支与上下文 `expectedTag` 都指向当前 HEAD，且 `.harness/release-context.json` 的工作树字节、HEAD blob 与规范 schema 一致的结果。锁定其 `sourceCommit` 与 `releaseContextSha256`；Rust 非 GUI 候选要求上下文 `candidateSelections` 中性能与 macOS 签名选择都精确为 `not-applicable`。审查选择、状态、范围摘要、启用证据或 `Not run` 原因/风险只能从上下文读取，不能从对话补写、重新询问或推断。上下文缺失、tag/远端未完成、候选适用性不符或禁用审查与当前安全/渠道硬要求冲突时转入 `$desktop-prepare-release` 或报告 `Not ready`。随后解析当前构建的 E2E 选择：本次请求已明确 `enabled`/`disabled` 时直接复用，否则在任何测试或编译前询问用户一次；`milestone_e2e` 只作为建议默认值。同一 `sourceCommit` 重跑复用上下文，新发布重新生成。构建 Skill 不自行执行发布语义审查，也不设置分支拓扑门禁。
   若发现目标是 Tauri GUI 安装包，停止本 Skill 并转交 `$desktop-build-tauri-release`；不得用 CLI raw-binary 打包/签名模型处理 DMG、NSIS 或公证。
2. 要求本次构建已由用户显式请求，下游项目根同时是其独立 Git 顶层目录。以 `git status --porcelain=v1 --untracked-files=all` 明确复核 clean，并要求 `HEAD` 解析到真实源码提交；把当前 40 位 HEAD 锁定为 `buildSourceCommit`，要求等于上一步 `sourceCommit`，并锁定 `releaseContextSha256`；dirty、无 HEAD、上下文/远端主分支/tag 不一致时停止。普通构建只报告“请先提交或改用明确发布流程”，绝不得自行调用 `git add`/`git commit`、身份 bootstrap 或提交模板配置。先调用 `$desktop-manage-version check --phase build`，确认根 Cargo 当前版本、`.harness/version-state.json` 目标和发布上下文版本一致；构建不得计算、提升版本或重置正式发布周期。随后在任何测试或编译前只读运行 `release_notes.py check --file release-notes.json --expected-version <version>` 并计算普通非符号链接文件的 SHA-256；缺失、不一致或格式无效时返回 `$desktop-prepare-release`，本 Skill 绝不得生成或改写更新日志。确认 `Cargo.lock`、`rust-version`、宿主和架构；存在用户要求的活动 Todo 时不得构建尚未完成的范围。
...
- 本 Skill 不发布、不上传到发布渠道、不创建或移动标签、不更改版本、不改写 `release-notes.json`、不调用 `finalize-release`、不执行公证、不配置签名器，也不声称运行时行为；它只验证发布流程已经推送的版本 tag。
