...
- 执行时为 Ratatui、tui-realm 和 tui-realm-stdlib 声明带完整三段下界的 Cargo 兼容范围。候选下界必须彼此兼容，并满足项目的 Rust MSRV、Windows/macOS/Linux 目标和锁定验证要求。

固定技术栈同样适用于中性的 `Draft` 脚手架状态。选择目标是通过全部约束的最低兼容稳定组合；临时最低直接版本解析与项目最低 MSRV 测试证明下界，正常 `Cargo.lock` 固定实际解析结果。不得使用精确 `=`、预发布版、未经批准的 Git 修订、通配符或“最新”替代该兼容下界。
...
- 记录评估过的候选下界、最终兼容范围、特性集、最低直接版本解析、MSRV/平台兼容性及正常锁文件结果。
