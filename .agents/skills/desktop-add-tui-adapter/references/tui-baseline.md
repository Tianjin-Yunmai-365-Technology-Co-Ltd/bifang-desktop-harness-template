...
- 当前经过元数据筛选的完整三段下界为 `ratatui 0.30.2`、Cargo 包 `tuirealm 4.1.0`、`tui-realm-stdlib 4.1.0`，异步入口使用 `tokio 1.53.1`；它们均须在 Rust 1.95 重新完成最低版本解析和真实测试。执行时先查询 registry，若存在同时满足这些包彼此约束、Rust 1.95、Windows/macOS/Linux 目标、所用 API 与锁定验证要求的更新非预发布稳定组合，则把该组合写成新的完整三段下界。

固定技术栈同样适用于中性的 `Draft` 脚手架状态。上表是已验证的最低兼容稳定组合；执行时在全部约束内优先采用最新兼容稳定组合，并把实际验证过的版本写成兼容下界。临时最低直接版本解析与 Rust 1.95 测试证明下界，正常 `Cargo.lock` 固定实际解析结果。不得使用精确 `=`、预发布版、未经批准的 Git 修订、通配符或字面 `latest` 替代该兼容下界。
...
- 记录 registry 最新稳定候选、被排除候选的具体冲突、最终完整三段下界、特性集、最低直接版本解析、Rust 1.95/平台兼容性及正常锁文件结果。
