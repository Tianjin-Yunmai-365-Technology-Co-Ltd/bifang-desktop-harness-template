...
5. Rust MSRV 下界为 `1.98.1`，已存在更高稳定工具链时直接通过且不得降级或重装。截至 2026-09-12 经 registry 元数据筛选的 TUI 候选完整三段下界为 `ratatui 0.30.2`、Cargo 包 `tuirealm 4.1.0`、`tui-realm-stdlib 4.1.0` 和 `tokio 1.53.1`；在真实 TUI 下游完成最低直接版本解析和测试前保持 `Unverified`。写入清单前查询 registry，优先采用同时满足 Rust 1.98.1、目标终端/平台、最小特性集、所用 API 及三个 TUI 包彼此约束的最新非预发布稳定组合；若最新版组合不兼容则保留上述元数据候选并记录具体冲突与未验证范围。每个 registry 直接依赖使用普通完整三段 Cargo 兼容范围，不得写入精确 `=`、`latest`、通配符或 tag；先以临时 `direct-minimal-versions` 解析和 Rust 1.98.1 运行相关测试，再由正常 `Cargo.lock` 固定实际解析结果，只有实际通过后才把候选称为项目兼容下界。若不存在兼容组合，停止并进入硬规则例外流程，不得替换固定 TUI 技术栈规避。
