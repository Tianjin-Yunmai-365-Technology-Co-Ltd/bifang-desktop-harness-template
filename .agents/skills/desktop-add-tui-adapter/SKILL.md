...
5. 执行时为 Ratatui、tui-realm 与 tui-realm-stdlib 选择满足已声明 MSRV、目标终端/平台、最小特性集、所用 API 和验证门禁的最低兼容稳定组合。每个 registry 直接依赖使用完整三段 Cargo 兼容下界，先以临时 `direct-minimal-versions` 解析和项目最低 MSRV 工具链运行相关测试，再由正常 `Cargo.lock` 固定实际解析结果。记录候选下界、提高原因和锁定结果；若不存在兼容组合，停止并进入硬规则例外流程，不得改用精确版本、“最新”或其他 TUI 技术栈规避。
