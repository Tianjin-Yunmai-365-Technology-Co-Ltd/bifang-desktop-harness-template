...
2. 确认用户确实要准备发布。要求存在由 `$desktop-verify-delivery` 给出的 `Milestone accepted` 候选；开发证据、一次构建或 `pending` 候选均不充分。要求独立 Git 顶层目录和 `HEAD` 与已验收源码提交匹配；尚未生成的 `HEAD` 无法标识发布源码。检查持久冒烟/E2E 策略的解析结果和执行结果，但不得在此运行任一测试。
