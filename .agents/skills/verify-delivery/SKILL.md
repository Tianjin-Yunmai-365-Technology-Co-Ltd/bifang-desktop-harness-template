...
3. 针对当前候选重新运行适用的编译/构建、相关非空单元/回归测试、必需集成/契约和产物存在性检查。非代码治理候选执行已声明的解析、链接、静态、现有回归或差异检查；代码行为测试数为零或缺少必需覆盖时拒绝里程碑。若批准场景要求 Windows 原生运行，macOS xwin 候选必须转交真实 Windows 环境完成对应检查，不能以 `Unverified` 验收该场景。
4. 按以下顺序解析可选运行时检查：
   - 产品、渠道和安全硬要求；
   - 当前任务中更严格的用户约束；
