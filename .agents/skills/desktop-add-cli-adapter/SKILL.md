...
4. 使用 Tokio current-thread 异步入口，并只启用最低所需特性。Rust MSRV 下界为 `1.95`，已存在更高稳定工具链时直接通过且不得降级或重装。截至 2026-09-01 的 CLI 当前完整三段 Cargo 兼容下界为 `clap 4.6.6`、`tokio 1.53.1`、`serde 1.0.229`、`serde_json 1.0.151` 和黑盒测试所需 `assert_cmd 2.2.2`；需要日期时间时使用 `jiff 0.2.35`。写入清单前查询 registry，优先采用同时满足 Rust 1.95、目标平台、所用 API 与特性约束的最新非预发布稳定版作为新的完整三段下界；若最新版本不兼容则保留上述已验证下界并记录冲突。Cargo 中使用普通兼容范围而不是精确 `=`，不得写入 `latest`、通配符或 tag。以临时最低直接版本解析和 Rust 1.95 运行相关测试，再由正常 `Cargo.lock` 固定实际解析结果。I/O、等待、计时器、进程及其他延迟型工作从命令到核心的路径默认保持异步；不得仅因使用 Tokio 就启用 `rt-multi-thread`。只有测量确认的 CPU 密集工作才可考虑 `spawn_blocking`、专用线程或多线程运行时，并记录所有权、取消、并发上限、资源预算和测试。仅支持阻塞调用的依赖应替换为异步能力，否则停止并进入范围或硬规则例外流程；不得静默用线程包裹。
