...
4. 使用 Tokio current-thread 异步入口，并只启用最低所需特性。Rust MSRV 下界为 `1.98.1`，已存在更高稳定工具链时直接通过且不得降级或重装。截至 2026-09-12 的 CLI 当前完整三段 Cargo 兼容下界为 `clap 4.6.6`、`tokio 1.53.1`、`serde 1.0.229`、`serde_json 1.0.151` 和黑盒测试所需 `assert_cmd 2.2.2`；需要日期时间时使用 `jiff 0.2.35`。写入清单前查询 registry，优先采用同时满足 Rust 1.98.1、目标平台、所用 API 与特性约束的最新非预发布稳定版作为新的完整三段下界；若最新版本不兼容则保留上述已验证下界并记录冲突。Cargo 中使用普通兼容范围而不是精确 `=`，不得写入 `latest`、通配符或 tag。以临时最低直接版本解析和 Rust 1.98.1 运行相关测试，再由正常 `Cargo.lock` 固定实际解析结果。I/O、等待、计时器、进程及其他延迟型工作从命令到核心的路径默认保持异步；不得仅因使用 Tokio 就启用 `rt-multi-thread`。只有测量确认的 CPU 密集工作才可考虑 `spawn_blocking`、专用线程或多线程运行时，并记录所有权、取消、并发上限、资源预算和测试。仅支持阻塞调用的依赖应替换为异步能力，否则停止并进入范围或硬规则例外流程；不得静默用线程包裹。
5. 完整实现 `docs/CLI_CONTRACT.md` 规定的非交互行为和 `--json` 行为。`--version` 必须从 Cargo 权威版本生成 `<product> v<version>`，且不得出现重复 `v`；JSON 元数据和 SemVer 比较保留不带前缀的机器版本。CLI 只处理参数是否可解析、协议必填项、标准流和退出码；值域、跨字段约束、资源状态、业务权限和影响业务结果的默认值必须交给 core。不得把 TUI 代码路径作为调用核心操作的唯一方式。
6. 先测试 core 成功路径和最高风险领域失败，再测试真实二进制的命令到 core 映射、JSON 解析、标准输出/标准错误分离、退出码、`--help`、恰有一个小写 `v` 前缀的 `--version`，以及拒绝等待输入。处于 `Draft` 时还要拒绝未批准的业务命令。
...
8. 中性初始化完成本次必要测试后返回 `$desktop-initialize-rust-project`。初始化后新增真实 CLI 也直接收口；普通编译/构建仍是开发命令，不产生候选或询问发布选择。只有用户另行明确请求正式发布候选时才先调用 `$desktop-prepare-release`，由它关闭并封存发布范围后进入 `$desktop-build-rust-release`；构建只另外确认 E2E 并全量运行单元测试。记录实际验证平台，其他平台标记为 `Unverified`。
...
报告每个命令的“CLI 命令 → core API → core 测试”映射、最终直接依赖下界/最新稳定候选冲突、输出契约、本次实际运行的测试、已验证平台、未验证平台和剩余风险；未显式构建时不得虚构制品路径或发布结论。初始化后新增时还报告 `change_id`、`required_version` 与是否实际提升；中性初始化不报告版本分类。
