...
- Tauri xwin 只在 macOS、GUI 已选且 Windows x64 NSIS 已批准时触发。LLVM、LLD 与 NSIS 使用既有 Homebrew，Rust target 使用 rustup，`cargo-xwin` 使用 Cargo 的 `--locked` 安装；缺少 Homebrew 时阻断并报告，不执行远程 shell 安装器。已存在但缺失必需命令的 formula 视为损坏或不兼容，必须阻断，不得静默升级或重装。
