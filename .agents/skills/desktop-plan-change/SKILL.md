...
2. 确认任务采用 `标准` 或 `里程碑` 路径并说明原因。若范围仍会改变产品目标、边界或成功标准，先转 `$desktop-define-product`。
...
5. 按依赖顺序建立最少数量的 Todo。每项至少记录：
   ...
   - 必要的静态、集成或契约检查。
   对业务行为，Todo 顺序必须先建立或复用 core 用例及其成功/最高风险失败测试，再实施各适配器映射；每个公开操作记录“适配器操作 → core API → core 测试”。仅修改布局、协议映射或系统托盘等接口/宿主机制时，可以没有 core 修改，但必须记录其专属性理由和接口级验证。“当前只有一个适配器”不是把业务逻辑安排在适配器的理由。
...
8. 创建或更新当天 Work Plan；新日从前一份综合仍有效的活动事项。普通缺陷修复、纯重构等维护类型只有在复杂度、风险、并行、交接或用户要求独立触发标准/里程碑路径时才进入计划。只有重要阻断、交接或用户要求时才同步 Product Status；计划存在和 Todo 完成都不自动触发 Product Spec、ADR、Product Status、Verification 或 Changelog。

## 质量规则

- 使用仓库事实，不编造文件、命令、API、依赖或产物。
- 计划必须保持 core-first：领域校验、业务默认值、状态转换和包含条件/重试/状态决策的调用编排属于 core；适配器 Todo 只拥有语法/协议、展示/交互、运行时装配和结果映射。
