...
- 应用壳层按 `docs/GUI_APP_PROFILE.md` 的 `sidebar_mode` 使用精简或详细左侧菜单，不建立 Jotai 折叠 atom。两种模式都让 Logo 位于顶部、版本紧随其下，功能项在可滚动区域按产品顺序向下增长，底部按已选赞助、固定设置、已选关于的视觉顺序生成。主内容按当前侧栏宽度偏移，不能被 fixed 区域覆盖。
- 精简模式固定 `136px`，使用 `56px` Logo 与 `30px` 图标在上、`11px`/`10em` 名称在下的布局，名称持续可见且没有折叠按钮。详细模式首次默认 `248px` 展开，使用 `72px` Logo 与图标+名称横排；拥有折叠动作的 `ActionIcon` 可收起为 `76px`，收起后使用 `44px` Logo、只显示图标并通过 Mantine Tooltip 显示名称。详细模式折叠偏好使用独立 local-storage 键持久化，不得混入页面会话 store。所有 Logo、图标和文字沿同一中心线且无裁切；菜单项使用链接或 button 语义、完整可访问名称、可见选中态和清晰焦点。
...
- GUI 初始化的主窗口使用 1440×900 逻辑像素、最小 960×640、居中并防止溢出工作区；1440×900 必须容纳详细模式展开侧栏，选择赞助页时还要让三张档位卡同屏可见。不要把它与 660×400 的 macOS DMG 安装卷窗口配置混为一谈。
- Mantine provider 使用 `defaultColorScheme="auto"`，共享主题同时为亮色与暗色定义页面背景、surface、主文字、次文字、边框与强调色。应用根与所选侧栏必须消费这些语义变量；所选 Sponsor 等带背景图的页面从运行时有效主题选择明确叠层、surface 和对比色。不透明的亮色插图应放入受控的中性承载面，避免在暗色卡片上形成无意的刺眼方块。

## 5. 组件语义

### 5.1 动作与导航

- Button 文案使用“动词 + 对象”。仅图标动作使用 `ActionIcon`，必须提供可访问名称；Tooltip 只能补充可见提示，不能替代 `aria-label` 等可访问名称。详细侧栏收起后允许用 Tooltip 显示被隐藏的菜单名称。
...
- 交互 handler 必须绑定到实际拥有动作的元素本身。按钮、链接、`Switch`、`Checkbox` 或菜单项不得把动作委托给 Card、`Table.Tr`、`Table.Td` 等父级；表格中的 `Switch` 只能因点击/键盘操作该 `Switch` 而切换，点击行或单元格不得切换。父级确有独立动作时只执行自己的语义，并在传播冲突时明确隔离，不能调用或模拟子控件动作。
- Tabs、查询/筛选控件、排序和 Pagination 等用于继续页面工作的选择状态必须接到应用根 Jotai store 的页面级模块 atom，跨路由返回时恢复；不能只放在 route 组件局部 state，也不能用 localStorage、sessionStorage、URL 或 Tauri Store 延长到下一次程序运行。更改数据范围或每页数量时页码归 1；只有成功查询到大于第 1 页的空页才回退第 1 页并重查，loading/error 不作空页处理。
...
- UI 测试使用 Testing Library 从角色、名称和用户操作验证，不以 class、内部 DOM 或大快照代替语义断言；精简侧栏回归锁定 `136px`/`56px`/`30px`/`11px`/`10em`、图标上文字下和无折叠控件，详细侧栏回归锁定默认 `248px` 展开、图标+名称、自身 ActionIcon、`76px` 收起、`44px` Logo、图标-only + Tooltip 与偏好持久化。真实本机调试窗口再由初始化 E2E 复核可见居中。
- 行内按钮、链接、`Switch`、`Checkbox` 等动作测试必须分别点击控件和其父级周围区域，断言只有控件操作触发对应动作；不能只验证冒泡后的最终状态。
- 页面会话状态测试必须用同一应用根 store 穿过 route unmount/remount，并用新 store 模拟下一次程序运行；同时覆盖 Tabs/查询/分页恢复、成功空页回退、第 1 页不循环以及 loading/error 不回退，且断言没有持久存储或 Query 数据镜像。
