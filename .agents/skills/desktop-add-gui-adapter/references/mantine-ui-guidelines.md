...
- 应用壳层使用固定 `136px` 单态左侧菜单，不建立 Jotai 折叠状态：`56px` 应用 Logo 永远位于最顶部，当前版本紧随其下，功能项再在可滚动区域按产品顺序向下增长，赞助/设置/关于按该视觉顺序固定贴底。主内容按 `136px` 侧栏宽度偏移，不能被 fixed 区域覆盖。
- 侧栏不得提供展开/折叠按钮。每个功能项和固定项必须提供从 `@tabler/icons-react` 命名导入的语义明确 `30px` 图标；图标在上、名称在下，名称使用 `11px` 字号、`10em` 行内宽度、水平居中和最多两行。Logo、图标和文字沿同一侧栏中心线且无裁切；菜单项同时使用链接或 button 语义、完整可访问名称、可见选中态和清晰焦点，不得用 Tooltip 代替可见名称。
...
- GUI 初始化的主窗口使用 1440×900 逻辑像素、最小 960×640、居中并防止溢出工作区；1440×900 必须让固定 `136px` 侧栏与赞助页三张档位卡同时可见。不要把它与 660×400 的 macOS DMG 安装卷窗口配置混为一谈。
...
- UI 测试使用 Testing Library 从角色、名称和用户操作验证，不以 class、内部 DOM 或大快照代替语义断言；侧栏回归同时锁定 Tabler 组件来源、`136px`/`56px`/`30px`/`11px`/`10em` 尺寸、图标上文字下排列和不存在折叠控件，真实本机调试窗口再由初始化 E2E 复核可见居中。
