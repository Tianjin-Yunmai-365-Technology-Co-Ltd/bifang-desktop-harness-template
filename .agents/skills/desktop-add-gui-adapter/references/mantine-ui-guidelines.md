...
- 侧栏展开/折叠按钮使用 `ActionIcon` 并具有本地化名称；菜单项使用链接或 button 语义、可见选中态和清晰焦点。Tooltip 不能是折叠态的唯一名称，版本也不能只存在于 tooltip。
...
- Mantine provider 使用 `defaultColorScheme="auto"`，共享主题同时定义亮色与暗色语义值。Sponsor 等带背景图的页面必须从运行时有效主题选择明确叠层、surface 和对比色；不透明的亮色插图应放入受控的中性承载面，避免在暗色卡片上形成无意的刺眼方块。
...
- UI 测试使用 Testing Library 从角色、名称和用户操作验证，不以 class、内部 DOM 或大快照代替语义断言。
