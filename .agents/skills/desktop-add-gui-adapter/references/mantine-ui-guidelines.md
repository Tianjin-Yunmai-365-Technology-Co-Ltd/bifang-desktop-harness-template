...
- 侧栏展开/折叠按钮使用 `ActionIcon` 并具有本地化名称；每个功能项和固定项必须提供语义明确的图标。折叠时显示图标并以本地化 Tooltip 补充名称，展开时显示图标与名称；菜单项同时使用链接或 button 语义、独立可访问名称、可见选中态和清晰焦点。Tooltip 不能是折叠态的唯一名称，版本也不能只存在于 tooltip。
