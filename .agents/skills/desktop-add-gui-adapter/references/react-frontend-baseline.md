...
- 在 Tauri 中打包本地前端资产。远程内容、遥测、Cookie、身份验证和网络访问需要明确范围；关于/支持/赞助、更新检查或遥测只有经 `$desktop-prepare-gui-support-surfaces` 逐项批准后才能出现。GUI 下游完整保留该 Skill 的品牌源资产，但未选择时不得加入路由、运行时媒体、配置或请求；选择赞助页时按 manifest 原样复制完整 sponsor 媒体，禁止优化支付二维码。
...
- 翻译资源按功能域拆分文件并使用稳定的层级 key（如 `settings.language.label`），不得在组件中拼接原始中文/英文字符串；核心领域错误标识作为 key 的一部分由前端映射为当前语言文案，业务判断本身不得放入翻译资源或组件。选择关于/赞助模板时把品牌包的中英文 JSON 注册为 `brandSupport` namespace，仍复用唯一 i18next 实例和语言偏好。
...
- Testing Library 通过角色、可访问名称和用户交互验证可观察行为；不得用 DOM class、实现细节或大快照代替语义断言。测试运行环境使用 jsdom，并在需要 Mantine provider、Router、QueryClient 或 i18n 时装配真实最小 provider。赞助模板测试必须覆盖固定价格/联系人、两张支付码 alt、响应式危险回归和本地媒体路径；视频模板测试必须覆盖 controls、无 autoplay、字幕与文字稿。
...
- 运行 `pnpm format:check`、`pnpm typecheck`、`pnpm lint`、非空 `pnpm test` 和锁定 `pnpm build`；构建脚本同时执行最终 `dist` 静态扫描。
