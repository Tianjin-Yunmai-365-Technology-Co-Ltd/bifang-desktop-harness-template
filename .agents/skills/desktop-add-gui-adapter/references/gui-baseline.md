...
- Tauri `tauri` 依赖强制启用 `tray-icon` feature，系统托盘不是可选增强。项目本地 Tauri `icon` 命令必须产出普通非符号链接的 `src-tauri/icons/32x32.png`；它必须是 32×32、8-bit RGBA、非交错且至少含一个非透明像素，并被 `tauri.conf.json` 的 `bundle.icon` 精确引用。托盘安装函数必须由 Tauri Builder `.setup(...)` 实际调用；在同一实现中组装仅含稳定 ID `show_window`/`quit` 的 `Menu`、以 `.menu(&menu)` 绑定、把 `default_window_icon()` 当作必需值（缺失即失败）、以 `.icon(...)` 绑定并成功 `.build(app)`，不得在图标缺失时继续创建无图标托盘。可见标签必须在创建或更新菜单时通过 `rust_i18n::t!("tray.show_window")` 与 `rust_i18n::t!("tray.quit")` 按当前规范化 locale 解析：中文精确为“显示窗口/退出”，英文精确为“Show Window/Quit”，未知 locale 回退英文，运行时切换语言后无需重启即可刷新；稳定 ID 或 `tray.*` 原始键不可见。该菜单绑定也满足 Tauri 官方说明的 Linux 图标可见性要求。显示动作及主鼠标左键释放都复用 `restore_main_window` 执行主窗口 `show`、取消最小化并聚焦；主窗口 `WindowEvent::CloseRequested` 处理必须由 Builder `.on_window_event(...)` 实际注册，随后 `prevent_close()` 并隐藏且进程继续，只有 `quit` 显式结束应用。默认不加入自动启动、后台业务或其他托盘菜单项；不得因 Draft、宿主差异、页面简单或实现偏好省略，单实例/托盘的资产、配置引用、运行时接线、回归或真实宿主证据任一缺失都阻断初始化。
...
- GUI 图标固定使用 `@tabler/icons-react` 的命名组件。存在适用图标时不得引入其他图标库、手写 SVG、字符或 emoji；图表相关操作、状态和空态优先使用 Tabler 图标，实际图表绘制库仍按产品需求选择。固定单态侧栏中的 `56px` Logo 与每一个 `30px` 当前渲染图标必须水平居中且无裁切。
- 初始化固定建立 `136px` 单态左侧菜单与 `/settings`、`/about`、`/sponsor` 文件路由，不得提供展开/折叠状态或开关。侧栏顶部始终先显示 `56px` 选中 Logo、再紧接带一个小写 `v` 的应用版本；产品功能项和赞助/设置/关于固定项均必须提供 `30px` 图标，图标在上、名称在下，名称以 `11px` 字号在 `10em` 行内宽度中居中、最多两行并始终保留完整可访问名称，不得使用 Tooltip-only 名称。产品功能项从顶部向下增长，底部固定组按视觉顺序为赞助、设置、关于。设置页只提供应用/版本、中英文和浅色/深色/跟随系统，不得包含隐私标题、统计同意、未配置占位或对应固定翻译键；关于页在手动“检查更新”旁提供元素自身绑定的“更新日志”按钮，从候选内同一 `release-notes.json` 查看最近 5 个版本，每版功能优化/问题修复各最多 10 条且全部版本只带一个小写 `v`。未配置远端能力时检查按钮显示 `NotConfigured`/禁用并保持零出站，本地更新日志按钮仍可用。关于页还显示当前应用名、权威版本、作者、作者联系方式和三段免责声明；赞助页使用固定品牌档位、权益、双支付码和完整 sponsor 本地媒体。
- `tauri.conf.json` 的主应用窗口固定以逻辑像素初始化为 1440×900，最小 960×640，居中且 `preventOverflow: true`；默认尺寸确保固定 `136px` 侧栏与三张赞助档位卡仍同屏横向呈现，较小窗口由响应式布局降列。该窗口与 `bundle.macOS.dmg.windowSize` 的 660×400 安装卷窗口互不替代。
...
- 原生窗口标题与 `document.title` 固定使用同一公式 `{applicationName} v{version} {contactChannel}:{contactValue}`。应用名/版本来自权威 Tauri/打包元数据，展示边界保证只有一个小写 `v`，联系字段来自品牌 profile 的 `contacts.windowTitle`，不得写死一次构建版本。
- 活动选项卡、已应用查询/筛选、排序、分页页码/每页数量及同类可恢复页面工作状态固定由应用根 Jotai store 的模块级 atom 持有，只在当前程序进程内跨路由、关闭隐藏和单实例唤醒保留；真正退出后回到默认值。不得使用浏览器/Tauri/文件/数据库/URL 持久化，不得把 TanStack Query 结果或 core 权威状态镜像进 atom。查询范围或每页数量变化时页码归 1；只有成功返回的当前页大于 1 且为空时才回退第 1 页并重查，加载/错误和第 1 页空结果不循环。
- 不得加载远程内容。
...
- `/settings` 必须提供可发现的中文/英文切换；用户手动切换后的选择必须持久化，并在后续启动时覆盖系统探测结果，直至用户重置。切换事件必须同时刷新当前托盘菜单标签，不得要求重启。
...
- 固定单实例、系统托盘与关闭隐藏生命周期位于 GUI adapter；只有显式托盘退出结束应用，存在 owned task 时必须先取消并回收。实现必须包含 `single_instance_plugin_is_registered_first`、`second_launch_restores_existing_main_window`、`tray_show_restores_and_focuses_main_window`、`close_request_hides_without_exit`、`tray_quit_exits_application`、`tray_labels_resolve_for_supported_locales`、`tray_labels_fall_back_to_english`、`language_change_updates_tray_menu_labels` 八个固定命名回归，并在初始化基线前通过 `verify-gui-lifecycle-contract.mjs` 对 32px RGBA 非透明资产/配置引用、`.setup`/菜单/图标/构建/窗口事件接线、`rust-i18n` 标签解析和有断言回归失败关闭，再通过真实宿主双启动/托盘 E2E 证明托盘有可见图形而非空白点击区域并实测中英文切换后的精确标签；宿主无法判定进程/窗口唯一性或观察托盘同样失败。通知、自动启动和快捷键等其他桌面机制仍需独立批准；它们触发的业务动作调用既有核心用例，不能直接修改权威业务状态。
...
- 三个 Logo 候选均有真实预览与摘要，用户选择后的母版、运行时 Logo 和平台图标可追溯；侧栏为 `136px` 单态，`56px` Logo 位于版本上方，菜单以 `30px` 图标上方和 `11px`/`10em` 文字下方居中呈现，且不存在折叠控件。产品功能项向下增长，固定底部顺序为赞助、设置、关于；`/settings`、`/about`、`/sponsor` 存在，默认设置页没有隐私/统计控件或翻译键，赞助页分别通过亮色/暗色渲染，关于页作者、联系方式和三段免责声明完整。未配置更新或统计时请求数为零；启用后测试允许地址/字段、策略与制品签名、SemVer 强更、同意撤回、响应、重定向、超时、取消和日志脱敏中的最高风险失败。
- GUI 生命周期结构检查证明单实例依赖与首插件顺序、只恢复既有窗口的回调、`tray-icon`、非透明 32px RGBA 应用图标及配置引用、从 `.setup` 可达且绑定 Menu/icon/build 的托盘安装、已注册的关闭事件、稳定 ID、`rust-i18n` 双语资源和八个固定命名回归齐全；真实宿主 E2E 再证明第二次启动自行退出且只剩同一主进程/主窗口，状态栏/通知区域出现可见非空图形，菜单恰好只有本地化显示/退出，中文为“显示窗口/退出”、英文为“Show Window/Quit”、切换无需重启且无 `tray.*` 原始键，主鼠标左键与显示菜单项都恢复并聚焦、关闭主窗口只隐藏且进程继续、托盘退出结束进程并移除图标。原生标题与 `document.title` 使用同一权威名称、版本和联系字段。
...
- GUI 中性初始化结束前，必须先由 `verify-gui-lifecycle-contract.mjs` 通过结构门禁，再由 `$desktop-test-gui-initialization-e2e` 构建并双启动真实本机调试二进制，实测第二次启动只唤醒既有窗口后退出、单一长期主进程/主窗口、托盘中英文精确菜单/运行时刷新与关闭隐藏/两种恢复/退出生命周期，并验证固定单态侧栏的尺寸/居中/图标上文字下、默认设置页无隐私区块和所有渲染菜单页面可达；任一失败或无法观察都阻断。真实打包应用或发布模式应用的最终候选验收仍是独立门禁。
...
单实例、固定两项托盘菜单、托盘左键显示、关闭隐藏、显式退出和运行时正确的托盘 i18n 是不可省略的 GUI 初始化硬门禁；改变或移除它们必须先变更产品/平台范围并记录硬规则 ADR，不得以无法观察或 `Not run` 通过。`136px` 单态图标上文字下侧栏、版本展示、`@tabler/icons-react` 图标来源、精简设置页与设置/关于/赞助路由同属 GUI 基线。伴随进程、自动启动、真实 updater 配置、更宽泛的平台 API、表单/图表绘制包、网络 client 与统计传输仍是项目特定选择。Vite、pnpm、ESLint/`typescript-eslint`、Prettier、Vitest 和 Testing Library 属于固定前端基线；替换时必须记录硬规则例外。其他包只有存在已批准需求，并同步依赖、安全、打包和测试变更时，才能增加或推荐。
