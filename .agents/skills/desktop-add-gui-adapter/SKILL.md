...
4. 首次真实产品 GUI 开发任务前，要求存在由 `$desktop-prepare-gui-app-identity` 生成且已批准的 `docs/GUI_APP_PROFILE.md`。它必须覆盖应用显示名称、固定标题公式、说明、应用标识符和用户选择的图标路径；选择 macOS DMG 时还必须记录对初始化生成的本地拖拽背景 `<project-id>_gui/src-tauri/dmg/background.png` 的预览批准，或记录在同一路径完成的替换，以及尺寸、落点和文案语言。中性初始化使用当前项目展示身份和 workspace 初始版本组装标题；首次产品身份批准后必须更新名称来源但保留公式。只有在明确阻断打包/发布时，临时中性脚手架图标才能解除非打包开发的阻断。
5. 选择 GUI 即调用 `$desktop-prepare-gui-support-surfaces` 消费固定本地基线：动态标题、持续可见版本、可收起左侧菜单、`/settings`、`/about`、`/sponsor`、应用导航入口和完整 sponsor 媒体在初始化时直接建立。产品功能项从侧栏顶部向下增长，底部固定组按视觉顺序为赞助、设置、关于，即从底部向上为关于、设置、赞助。设置页固定展示应用版本、中英文切换、手动检查更新状态和统计上报同意；没有产品 profile 时更新显示 `NotConfigured`、统计开关默认关闭并禁用，全部远程请求为零。关于页固定展示当前应用名、权威版本、作者、作者联系方式和三段免责声明；赞助页固定使用品牌 profile、翻译与双支付码。初始化不得预创建 `docs/GUI_SUPPORT_SURFACES.md`，但 GUI 下游必须完整保留该 Skill 的品牌源资产和更新/统计契约供默认页面和未来受控配置使用。
...
9. 在 `<project-id>_gui` 中增加桌面应用边界，并将该标识用于 Cargo 软件包、Rust crate 和真实应用二进制。不得要求独立二进制名称；面向用户的名称来自已批准 GUI 资料。由中性 `$desktop-initialize-rust-project` 调用时，接收其逐字节复制的 `<project-id>_gui/src-tauri/dmg/background.png`，并在 `tauri.conf.json` 的 `bundle.macOS.dmg` 中写入 `background: "./dmg/background.png"`、`windowSize: { width: 660, height: 400 }`、`appPosition: { x: 180, y: 220 }` 与 `applicationFolderPosition: { x: 480, y: 220 }`；不得把已删除初始化 Skill 的资产路径写入运行时配置。建立职责单一的桌面生命周期模块：托盘只含稳定 ID 的本地化“显示窗口”和“退出”，左键或“显示窗口”执行 `show`、取消最小化并聚焦主窗口；主窗口收到 `WindowEvent::CloseRequested` 时调用 `api.prevent_close()` 并隐藏，不能退出进程；只有托盘“退出”执行显式、可回收的应用退出，存在 owned task 时先取消并等待回收。不得默认加入自动启动。复用 Tauri 基于 Tokio 的单例异步运行时；不得创建嵌套 Tokio 运行时。Tauri Rust 适配器可以依赖核心；核心不得依赖 Tauri、WebView、React、路由、查询、命令、窗口或前端状态类型。维护的 Rust/前端代码和测试必须遵守工程规则。
...
15. 先用 core 测试覆盖业务成功路径、最高风险领域失败和状态转换，再测试 Rust 异步命令映射、任务取消、托盘显示/退出映射、关闭隐藏且进程继续、动态标题、展开/折叠版本可见、功能区与固定底部顺序、设置/关于/赞助路由、语言切换、`NotConfigured` 零出站、作者/联系人/免责声明、赞助媒体、查询生命周期、Jotai 纯交互转换、前端交互、键盘导航、无障碍语义、能力拒绝和并发刷新/写入行为。启用更新/统计后，按支持 Skill 的参考增加 SemVer 强更、签名、同意撤回、字段白名单、任务回收和失败语义测试。布局属于验收范围时使用视觉质量检查。
...
- 除固定本地标题、可收起侧栏、设置/关于/赞助页、应用导航和托盘生命周期外，不得捆绑其他页面、业务操作或产品状态。设置页只读取当前应用身份、本地偏好和 adapter 能力状态，未配置远端能力时不得发请求；随附中文注释检查器只属于治理工具参考，不是页面或业务资产。

## 完成输出

报告已解析的 Tauri/前端版本、`tray-icon`、显示/退出与关闭隐藏行为、动态标题公式、侧栏折叠与版本、功能区和固定底部顺序、设置/关于/赞助路由、语言切换、更新/强更/统计配置状态、默认 sponsor 媒体、Mantine 组件、Router/Query/Jotai 所有权，每个业务操作的“GUI 事件/命令 → core API → core 测试”映射，i18n 接入范围、TypeScript 中文注释门禁与最终 `dist` 扫描、adapter-only 平台机制理由、能力/CSP/签名/同意边界、运行过的检查、已验证平台、未验证范围和剩余风险。
