...
18. macOS 发布默认由 `$desktop-prepare-release` 把 `macosSigningSelection = disabled`、`macosSigningSource = not-requested` 写入发布上下文的 `candidateSelections`，再由 `$desktop-build-tauri-release` 只读消费、使用 `--no-sign`，且不得探测本机身份、凭据或 profile；只有用户已配置过、当次主动要求或渠道硬要求时才启用。启用后签名、公证与 stapling 必须作为一个不可降级阶段完成，任一步缺失或失败都阻断候选。updater 制品签名是独立门禁：启用 updater 时每个平台候选仍必须生成官方 updater 制品和 `.sig`，缺少发布私钥安全引用或签名验证时阻断该 updater 候选。渠道要求商店提交或签名更新器制品时，在独立渠道门禁通过前发布就绪保持受阻。
