...
5. 选择 GUI 即调用 `$desktop-prepare-gui-support-surfaces` 消费固定本地基线：动态标题、默认收起且顶部固定 Logo→当前版本的左侧菜单、`/settings`、`/about`、`/sponsor`、应用导航入口和完整 sponsor 媒体在初始化时直接建立。每个产品功能项和赞助/设置/关于固定项必须提供图标；折叠时显示图标和本地化 Tooltip 名称，展开时显示图标与名称，且两种状态均保留可访问名称。产品功能项从侧栏顶部向下增长，底部固定组按视觉顺序为赞助、设置、关于，即从底部向上为关于、设置、赞助。设置页固定展示应用版本、中英文、浅色/深色/跟随系统和统计上报同意；关于页固定展示当前应用名、权威版本、手动检查更新、作者、作者联系方式和三段免责声明。没有产品 profile 时关于页更新显示 `NotConfigured`、统计开关默认关闭并禁用，全部远程请求为零。应用根初始化亮色/暗色的页面背景、surface、主/次文字、边框与强调色，并持久化主题偏好；赞助页固定使用品牌 profile、翻译与双支付码，并从 Mantine 已解析主题同时支持亮色和暗色呈现。初始化不得预创建 `docs/GUI_SUPPORT_SURFACES.md`，但 GUI 下游必须完整保留该 Skill 的品牌源资产和更新/统计契约供默认页面和未来受控配置使用。
...
11. 只实现已批准管理闭环和固定 GUI 基线。使用 Mantine UI 负责组件/布局，TanStack Router 文件路由负责导航并生成唯一 `src/routeTree.gen.ts`。以固定侧栏作为应用壳层：Jotai 折叠 atom 必须以 `DEFAULT_SIDEBAR_COLLAPSED = true` 初始化；顶部第一项始终渲染选中的 `/app-identity/logo.png`，其下紧接当前版本，展开/折叠均不得隐藏二者；产品功能项位于可滚动顶部区域，底部固定组严格为赞助、设置、关于并分别到达 `/sponsor`、`/settings`、`/about`。所有菜单项的图标为必填；折叠只隐藏文本标签，并用 Tooltip 补充名称，不能删除 `aria-label`。Mantine 根使用唯一 `AppThemeProviderTemplate`、`defaultColorScheme="auto"`、明确的 local-storage manager，并同时提供亮色/暗色的页面背景、surface、主/次文字、边框与强调色；设置页通过 Mantine color-scheme context 提交 `light`、`dark`、`auto`，赞助页以 `useComputedColorScheme` 消费运行时有效主题。不得在初始化时只生成宿主当前主题版本。TanStack Query 负责命令支撑的异步状态，Jotai 只负责侧栏折叠等跨组件客户端交互状态；语言与主题偏好由 GUI adapter 的设备级本地存储持久化。主窗口标题固定动态组装为 `{applicationName} {version} {contactChannel}:{contactValue}`，应用名与版本来自权威 Tauri/打包元数据，联系字段来自品牌 profile 的 `contacts.windowTitle`；原生窗口标题和 `document.title` 必须消费同一组事实并保持一致。不得把 Query/核心/持久数据镜像到 atom 中，也不得在 React event handler、Router loader、Query mutation 或 atom 中实现业务规则和领域状态转换。
12. 设置页始终渲染当前应用名/版本、中文/英文、浅色/深色/跟随系统和统计上报同意；关于页始终渲染手动检查更新与状态。未配置更新能力时关于页只呈现 `NotConfigured`/禁用状态，不安装网络插件、不创建 endpoint 或占位请求。产品启用更新时才接入官方 Tauri updater：更新制品签名验证不可关闭，默认拒绝降级和 target/arch/channel 不匹配；不得信任远端 `forcedUpdate` 布尔值，强更只能由 adapter 验证策略后把 `minimumSupportedVersion` 交给 core 作严格 SemVer 判定，根级强更门不挂载普通功能且只允许安装或安全退出。产品启用统计时只由 Rust adapter 在明确同意后以 HTTPS JSON POST 发送固定 `app_started` 字段，禁止 GET/query、稳定标识符和桌面内置 secret；撤回同意必须取消请求并清空有界内存队列。所有任务由应用生命周期拥有并回收，失败默认 fail-open。
...
- 除初始化选中的应用 Logo、固定本地标题、默认收起侧栏、设置/关于/赞助页、应用导航和托盘生命周期外，不得捆绑其他页面、业务操作或产品状态。设置页只读取当前应用身份、语言/主题偏好和统计能力状态；关于页只读取当前应用身份与 adapter 提供的更新状态，未配置远端能力时不得发请求。随附中文注释检查器只属于治理工具参考，不是页面或业务资产。
