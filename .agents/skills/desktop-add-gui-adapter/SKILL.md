...
1. 先判断调用模式。由 `$desktop-initialize-rust-project` 分派时是“中性初始化”，只读 `AGENTS.md`、Agent Policy、`docs/ENGINEERING_RULES.md` 和 Rust/GUI 基线，不要求 Product Spec、Work Plan、ADR 或 Verification；初始化后新增 GUI 是公开接口、权限和分发边界变更，进入里程碑路径并按需读取当前产品事实和计划。
...
7. 中性初始化直接按接口选择建立无业务 GUI，不创建 Work Plan；初始化后新增真实 GUI 时编辑前使用 `$desktop-plan-change`。GUI 适配器必须与 CLI 解析和 MCP 协议代码相互独立。
...
16. 中性初始化运行 Rust/前端格式、workspace-aware Rust 中文注释检查、严格类型检查、ESLint 与 TypeScript 中文注释门禁、非空 Vitest/Testing Library 测试、Vite 生产构建、最终 `dist` 静态扫描和相关构建后返回 `$desktop-initialize-rust-project`，不调用 `$desktop-verify-delivery`。初始化后 Todo 全部 `done` 时，macOS 原生 DMG 与 macOS→Windows x64 NSIS 候选交给 `$desktop-build-tauri-release`；其他宿主/格式继续使用已批准的适配器专用规则。只有后续 `$desktop-verify-delivery` 决定启动冒烟和界面 E2E。
