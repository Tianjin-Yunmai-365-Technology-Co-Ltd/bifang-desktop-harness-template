...
description: 只收集 Harness 创建终端下游所需的固定初始化信息，拒绝产品业务需求；完成中性脚手架后，在实际基线提交前即时建立独立 Git 仓库。
---
...
- Git 可用性、版本、独立边界、作者身份和提交模板的检查或设置只允许紧邻该次真实基线提交执行；表单、复制、身份改写、环境检查、脚手架编写和测试均不得以未来会提交为由提前触发。
...
报告初始化表单的最终字段、用户输入的项目路径、解析后的唯一目标根目录、源根目录、复制和排除的文件、身份与历史重置、Git 边界、提交模板本地配置与检查结果、四项已确认策略值、可选的 Harness 溯源锁或未来必须执行的初始基线审计、基线提交、干净状态、是否拒绝过超范围产品输入（不得复述或保存其具体内容）、源 Harness 验证结果以及下一个 Skill。中性脚手架可以先于产品批准建立，但不是已验收产品。
