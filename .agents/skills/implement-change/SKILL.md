...
9. Run the development-loop gate from narrow to broad: non-empty unit tests plus change-related format, lint, static, integration, contract, or minimal read-only smoke checks selected by risk. Fix ordinary failures within the authorized scope; do not defer a failing required check. Do not automatically run production artifacts, full startup smoke, Computer Use E2E, cross-platform candidates, archives, or human final review unless the user requests delivery acceptance, prepares a release, or this change directly affects the release path.
