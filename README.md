...
2. 使用 `$desktop-initialize-rust-project` 选择 CLI/TUI/MCP/GUI 接口；对 Agent 策略只需选择一次“推荐预设”或“自定义”。无接口选择时默认 CLI，策略不得在基线残留 `pending`；选择 GUI 时会实际生成 3 个 Logo 候选并等待用户选择，同时创建项目内 660×400 macOS DMG 拖拽背景，并强制初始化单实例、系统托盘显示/退出、关闭隐藏、两种恢复聚焦、动态标题、`@tabler/icons-react` 图标/Tooltip 完整且默认收起并居中的 Logo→版本侧栏、三态主题设置、应用级亮暗主题以及应用导航中的关于/赞助页。初始化基线提交前会先检查单实例依赖/首插件/回调、托盘 feature、源码、稳定 ID、双语资源和固定回归，再自动构建并双启动真实本机调试二进制，实测第二次启动只唤醒既有主窗口后退出、单一长期主进程/主窗口、托盘精确菜单与完整生命周期、收起侧栏居中和所有菜单页面可达；任一缺失都不会创建基线提交。
...
- `$desktop-test-gui-initialization-e2e`：只在含 GUI 的一次性初始化基线提交前，先验证强制单实例/托盘结构，再构建并双启动真实本机 Tauri 调试二进制，确认第二次启动只唤醒同一主窗口后退出且只剩一个长期应用主进程/主窗口，实测托盘精确双项菜单、关闭隐藏、两种恢复和退出生命周期，并验证应用可启动、收起侧栏 Logo/全部图标居中和所有渲染菜单页面可达；无法判定、无法观察或任一失败都阻断，通过后随初始化能力删除。
...
- `$desktop-prepare-gui-support-surfaces`：为所有 GUI 初始化提供动态标题、`@tabler/icons-react` 图标/Tooltip 完整且默认收起并居中的 Logo→版本侧栏、浅色/深色/跟随系统设置、全局亮暗语义主题、含更新入口的关于页、双主题赞助页和完整 sponsor 品牌媒体；真实更新、强更或统计上报启用时，再固化签名、core/GUI 所有权、同意、出站白名单与秘密隔离。
...
- GUI 图标统一使用 `@tabler/icons-react` 命名组件；存在适用图标时不引入其他图标库、手写 SVG、字符或 emoji，图表周边图标优先使用 Tabler，而图表绘制库保持项目特定。默认收起侧栏中的 Logo 与所有图标必须水平居中且无裁切。
- 含 GUI 的下游在唯一基线提交前固定执行单实例/托盘结构检查和一次本机调试 E2E，双启动同一真实二进制并验证第二次启动退出、既有主进程/同一主窗口持续且被唤醒、只剩一个长期应用主进程/主窗口，再验证真实托盘的精确菜单、关闭隐藏、两种恢复、退出生命周期，以及项目可编译启动、收起侧栏居中和全部菜单页面可达；它独立于构建 E2E 建议值，失败或宿主无法判定/观察都阻断初始化，但不构成发布候选或完整验收。
