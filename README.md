...
2. 使用 `$desktop-initialize-rust-project` 选择 CLI/TUI/MCP/GUI 接口；对 Agent 策略只需选择一次“推荐预设”或“自定义”。无接口选择时默认 CLI，策略不得在基线残留 `pending`；选择 GUI 时会实际生成 3 个 Logo 候选并等待用户选择，同时创建项目内 660×400 macOS DMG 拖拽背景，并初始化托盘显示/退出、关闭隐藏、动态标题、`@tabler/icons-react` 图标/Tooltip 完整且默认收起并居中的 Logo→版本侧栏、三态主题设置、应用级亮暗主题以及应用导航中的关于/赞助页。初始化基线提交前还会自动构建并启动真实本机调试二进制，验证收起侧栏居中和所有菜单页面可达。
...
- `$desktop-test-gui-initialization-e2e`：只在含 GUI 的一次性初始化基线提交前，构建并启动真实本机 Tauri 调试二进制，验证应用可启动、收起侧栏 Logo/全部图标居中和所有渲染菜单页面可达；通过后随初始化能力删除。
...
- 含 GUI 的下游在唯一基线提交前固定执行一次本机调试 E2E，验证项目可编译启动、收起侧栏居中和全部菜单页面可达；它独立于构建 E2E 建议值，不构成发布候选或完整验收。
