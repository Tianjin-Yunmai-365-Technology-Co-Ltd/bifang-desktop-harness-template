...
- 用户同意后使用 `$run-parallel-worktrees`：写入型 Subagent 各自使用独立 Git Worktree 和 `codex/` 分支，主 Agent 先公开子任务、文件所有权、依赖与验证边界，并在启动、阻塞、阶段完成、整合和验证节点向用户更新。主 Agent 必须同步等待全部必需结果，不得把未完成 Subagent 留作用户不可见的后台工作；重叠写入必须转为串行。
