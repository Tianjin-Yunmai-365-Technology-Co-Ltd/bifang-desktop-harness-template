...
- 已初始化下游的版本只通过 `$desktop-manage-version` 管理；根 `Cargo.toml` 是当前版本事实源，`.harness/version-state.json` 是受保护的周期/去重状态。首个新功能提升 Minor 后锁到正式发布成功；每个具有新稳定 ID 的问题修复或用户可感知优化使用 `bug-fix` 独立提升 Patch。新生成的 Minor/Patch 采用 `0..99` 的 base-100 进位，自动进位到 Major 不等同于显式 Major 授权；Major 不受 99/100 的业务上限约束，但不得超过 Cargo `u64::MAX`。Cargo、目标版本或发布日志中出现任何 Minor/Patch 为 `100` 的值都立即失败关闭，必须先由负责人手动修正，不能延后到下一次提升时自动规范化。Harness 自身版本只取 `Version.md`。
- Harness 源与终端下游的新功能或独立 Bug 修复在写入前通过 `$desktop-manage-git-lifecycle` 自动创建并切换到独立 `feature-{ascii-kebab摘要}-{YYYYMMDD}` 分支；本地建分支不依赖远端。用户明确说“推送”时，`--remote`/`state.remote` 表示唯一主远端，受管流程普通合并登记分支、切换并推送动态默认主分支；用户逐一明确授权其他具名目标时，只有 `publish` 可重复使用 `--also-remote <name>` 把同一最终 HEAD 非强制推向各自 advertised default branch 并复读。全部目标在首个 push 前解析，最终 HEAD、目标顺序与逐项确认进度在 `pendingPublish` 中临时冻结，成功即清除；补充远端不改绑主远端，也不参与 release、tag 或清理。跨远端推送不是原子操作，后续失败如实报告部分成功并只沿用同一冻结 HEAD 与目标幂等重试，不重新 fetch/merge。明确说“发布”时先询问并把单次 `gitPublication: local | remote` 锁定到 tracked 发布上下文；两种 `release` 调用都必须传 `--release-context-sha256 <sha256>`。`local` 使用 `release ... --release-context-sha256 <sha256> --local-only`，只合并本地默认主分支、创建并复读本地 `v{版本}-{YYYYMMDD}`，再清理登记 Worktree/本地分支；`remote` 使用 `release ... --release-context-sha256 <sha256> --remote <name>`，才把主分支与 tag 推送/远端复读及主远端分支清理作为硬门禁。生命周期清单只记录 Harness 自己创建或明确登记的资源，不设置保护分支、严格线性、fast-forward-only、lease 或 atomic push 门禁，也不创建、迁移或使用名为 `Release` 的分支；流程不创建/配置远端或凭据。
