...
- 每个逻辑提交按 `$desktop-configure-git-commits` 表达一个完整、独立、可解释的结果：简单变化可以只写 Conventional Commit 主题，非简单变化填写 Why/Changes/Impact/Test，未运行测试必须写明 `Not run` 原因。初始化在完整表单确认后、首次脚手架写入前由环境门禁检查并按需安装 Git；目标独立仓库建立后才安装提交模板和补齐缺失身份。模板、`user.name`、`user.email` 只写仓库 local 配置，已有有效身份保持不变，不得修改 global/system。明确发布请求授权复核范围内改动、形成本地逻辑提交并直接构建，不再二次审批；普通构建不自动提交。
...
- 下游初始化必须把 `CLI/TUI/MCP/GUI` 接口组合作为首次写入前的基础问题，允许多选；用户明确使用默认值或跳过时采用 CLI。四类接口分别由 `$desktop-add-cli-adapter`、`$desktop-add-tui-adapter`、`$desktop-add-mcp-adapter`、`$desktop-add-gui-adapter` 独立实施，任何一种都不要求另一种存在。基础问题选择 GUI 后，才在同一表单的条件阶段每轮补全一个字段，依次解析系统托盘、系统通知、开机自启、关于页、赞助页、单实例的启用/禁用，再提供侧栏精简/详细模式选择；六项能力都须明确选择，不得推断、遗漏或残留 `pending`。用户未选择侧栏模式时，初始化器必须在写入 `docs/GUI_APP_PROFILE.md` 唯一 `gui-initialization-config` 代码块前归一化为 `sidebar_mode = detailed`；明确选择 `compact` 或 `detailed` 时尊重该值，显式非法值不得按未选择处理。最终七项配置不得遗漏或残留 `pending`，复制后不得重复询问表单已确认值。
...
- 下游直接依赖和受管工具必须声明经过验证的最低兼容稳定版本范围。新增或主动更新时先从官方 registry 选择当前最新的非预发布候选，再验证 MSRV、peer、三平台与实际 API/feature；通过后把该版本写成完整下界，清单不得写 `latest`、tag、通配符或无下界范围。已安装稳定工具落入受支持范围时直接复用，不因不是最新版而升级。根 `Cargo.lock` 与 `pnpm-lock.yaml` 固定当前真实解析结果；每个直接下界必须在 Rust 1.95 和对应最低 Node.js/pnpm 环境通过最低版本解析与相关测试。上游明确排除的版本或 peer 冲突不能因版本号更大而通过。
...
- `$desktop-instantiate-project` 在完整表单汇总确认后、首次脚手架写入前调用 `$desktop-check-development-environment` 检查 Git；缺失时按受管平台路径安装并复探，失败即停止。脚手架和初始化 E2E 成功后，在目标根创建或确认独立 `main` 仓库，验证 top-level 精确等于项目根且无 remote。已有有效作者字段保持不变；缺失字段由 Agent 把设备账户名翻译/转写为 ASCII 英文名称，邮箱使用 `<ascii-device-username>@gmail.com`，只写 local 配置并在完成报告中标注派生来源。不得复制源 `.git`、配置 remote、push/tag、修改 global/system 或把派生邮箱声称为已验证账户。
- 紧邻真实初始化基线提交时调用 `$desktop-configure-git-commits` 的 `identity`、`install` 与 `check`，确认作者信息和模板均来自当前仓库 local 作用域；完成输出必须包含 Git 版本、安装变化、有效身份及来源、仓库根、模板状态和 40 位基线提交。
...
- 仅当 `system_notification = enabled` 时调用 `$desktop-add-gui-system-notifications`：macOS 使用 modern User Notifications，Windows/Linux 使用官方 Tauri notification 插件，WebView 只调用 Rust command。通知权限和发送由应用生命周期拥有的串行 worker 执行；设置默认关闭，开启必须先取得权限再持久化，拒绝/失败保持关闭并提供可访问反馈，关闭只停止应用发送而不伪造撤销 OS 权限。中性 scaffold 不定义产品通知触发点或正文。`disabled` 时不得声明依赖、插件、worker、命令、设置开关、i18n 或事件。
- 仅当 `autostart = enabled` 时调用 `$desktop-add-gui-autostart` 并使用官方 Tauri autostart 插件。能力加入不表示默认注册；设置默认关闭，OS 登录项状态是权威，Switch 通过 Rust command 启用/禁用，失败回滚并显示错误。未启用托盘时开机启动必须展示可找回的主窗口。初始化 E2E 改动登录项后必须恢复执行前状态；`disabled` 时依赖、插件、命令、设置和权限全部缺席。
...
- 含 GUI 的下游在初始化相关非空单元测试通过后、初始化能力裁剪和唯一基线提交前，必须固定调用一次 `$desktop-test-gui-initialization-e2e`：先运行 `verify-gui-lifecycle-contract.mjs` 读取七项 profile，并对启用能力验证完整结构与固定命名回归、对禁用能力验证依赖/运行时/路由/资源缺席；再以 `pnpm tauri build --debug --no-bundle` 构建真实本机调试二进制。只有启用单实例才执行双启动唯一性场景，只有启用托盘才操作真实托盘，只有启用通知才验证权限和投递可观察结果，只有启用自启才切换并恢复 OS 登录项；托盘禁用时必须实测关闭最后窗口退出。Computer Use 始终验证可见主窗口、所选侧栏、设置页和实际菜单页面，并验证未选页面缺席。任一适用场景失败、无法观察或无法恢复都阻断初始化。
...
- GUI 正式发布必须在完整打包前由 `$desktop-test-gui-release-performance` 对同一干净提交的 release-profile 探针候选测量 5 次冷启动、至少 20 次代表性交互、整进程树空闲/隐藏 CPU、稳定与峰值 RSS、20 轮操作后的内存增长和退出回收；该门禁独立于 E2E 选择。超标先回 `$desktop-implement-change` 修复、增加回归并重建；确实无法解决时才询问用户。用户明确继续只能记录 `performanceStatus: waived` 及原始失败证据，不能改判通过；未确认不得继续打包。
...
- 初始化 Rust 工具链、shared core、接口选择和四项持久 Agent 策略；选择 GUI 时另行询问托盘、系统通知、开机自启、关于页、赞助页、单实例与侧栏模式（侧栏未选时默认详细），完成三候选 Logo、项目内 macOS DMG 背景、所选生命周期/支持页面、精简或详细侧栏、固定设置与主题/i18n，以及按最终配置执行的一次真实本机初始化 E2E：使用 `$desktop-initialize-rust-project`。
...
- GUI profile 选择系统通知时：使用 `$desktop-add-gui-system-notifications`。
- GUI profile 选择开机自启时：使用 `$desktop-add-gui-autostart`。
...
- GUI 正式发布打包前对 release-profile 探针候选执行启动、交互、CPU、RSS、内存增长和进程回收门禁：使用 `$desktop-test-gui-release-performance`。
- 含 GUI 的一次性项目初始化在基线提交前按七项 profile 检查启用能力完整、禁用能力缺席，再构建真实本机调试二进制；按选择验证单实例、托盘、系统通知与开机自启，并检查所选侧栏、设置页和实际菜单页面：使用 `$desktop-test-gui-initialization-e2e`；通过后从终端下游删除该 Skill。
...
| Git 安装、身份、提交模板与消息结构 | `$desktop-check-development-environment` 管理 Git 可执行文件；`$desktop-configure-git-commits` 只管理独立仓库 local 身份、模板和提交，规范正文位于其 `references/commit-convention.md` |
...
| GUI 初始化选择与真实 E2E | `docs/GUI_APP_PROFILE.md` 保存托盘、系统通知、开机自启、关于页、赞助页、单实例和侧栏模式；启用能力必须完整满足契约，禁用能力必须从依赖、运行时、路由和资源中缺席。`$desktop-test-gui-initialization-e2e` 在含 GUI 的基线提交前按选择检查生命周期、权限、登录项恢复、所选侧栏、设置页和实际菜单页面，通过后删除且升级不得恢复 |
| GUI 发布性能 | `$desktop-test-gui-release-performance`；正式打包前绑定 release-profile 探针候选与整进程树指标，失败只可修复重建或由用户显式 waiver |
