...
- 最终产物构建、产物存在性与启动冒烟、Computer Use E2E、跨平台候选、归档和人工整体验收默认延迟到用户明确准备发布、要求交付验收，或本轮直接修改发布链路时；进入该阶段后必须基于当前源码重新运行完整验证，历史开发证据不得替代。
- 下游项目的发布或交付完成声明至少要求在 Agent 当前实际运行的系统中：代码成功编译、非空单元测试套件通过、约定的最终产物确实存在、产物通过启动冒烟测试。
...
- 用户要求交付验收、复核完成状态或准备发布：使用 `$verify-delivery`。
...
3. 检查工作区现状，保护既有修改。
4. 仅修改完成目标所需的最小文件集合。
5. 执行非空单元测试与风险相称的变更相关验证；只在发布/交付验收阶段执行完整闭环。
6. 更新受影响的项目记忆。
