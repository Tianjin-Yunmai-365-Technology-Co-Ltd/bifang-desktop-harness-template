...
- 含 GUI 的下游在初始化相关非空单元测试通过后、初始化能力裁剪和唯一基线提交前，必须固定调用一次 `$desktop-test-gui-initialization-e2e`：先运行 `verify-gui-lifecycle-contract.mjs`，对单实例依赖/首插件/回调、`tray-icon`、真实托盘源码、稳定 ID `show_window`/`quit`、中英文资源和五个固定命名回归失败关闭；再运行 `pnpm tauri build --debug --no-bundle` 并双启动本次真实本机调试二进制。第二次启动必须自行退出，既有主进程与同一主窗口继续存在且窗口恢复、取消最小化并聚焦，宿主枚举只剩一个长期应用主进程和一个主窗口。Computer Use 还必须验证真实系统托盘图标与恰好两个菜单项、原生关闭只隐藏且进程继续、托盘左键与“显示窗口”均恢复并聚焦、托盘退出结束进程并移除图标，以及可见主窗口、收起侧栏 Logo/全部图标居中和所有渲染菜单页面可达。该门禁不读取 `milestone_e2e`，失败/超时/取消/唯一性无法判定/无法观察/无法执行都阻断初始化；它不生成发布候选或 Verification，通过后该 Skill 与初始化能力一同删除。
...
- 初始化 Rust 工具链、shared core、接口选择和四项持久 Agent 策略；选择 GUI 时同时完成三候选 Logo 选择、项目内 macOS DMG 背景、强制单实例/托盘结构与真实双启动/关闭隐藏/恢复/退出生命周期门禁、动态标题、Tabler 图标/Tooltip 完整且默认收起并居中的 Logo→版本侧栏、三态主题设置、应用级亮暗主题、含更新入口的关于页、双主题赞助页和一次真实本机初始化 E2E：使用 `$desktop-initialize-rust-project`。
...
- 含 GUI 的一次性项目初始化在基线提交前先检查强制单实例/托盘结构，再构建并双启动真实本机调试二进制，证明第二次启动只唤醒同一主窗口后退出且只剩一个长期应用主进程/主窗口，再实测托盘精确菜单、关闭隐藏、两种恢复与退出生命周期，并检查收起侧栏居中和全部菜单页面可达：使用 `$desktop-test-gui-initialization-e2e`；通过后从终端下游删除该 Skill。
...
| GUI 单实例、托盘、图标与初始化 E2E | 单实例和系统托盘都是不可省略的 GUI 初始化硬门禁；托盘必须有非透明 32px RGBA 资产/配置引用、从 `.setup` 可达的 Menu/icon/build 接线和真实宿主可见非空图形，并通过真实双启动单一主进程/主窗口及关闭隐藏/恢复/退出生命周期；`@tabler/icons-react` 是固定图标库；`$desktop-test-gui-initialization-e2e` 只在含 GUI 的基线提交前执行这些检查并验证收起侧栏 Logo/图标居中和全部菜单页面可达，通过后删除且升级不得恢复 |
