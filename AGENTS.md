...
- GUI 页面中用于继续工作的活动选项卡、查询/筛选、排序、分页页码/每页数量及同类视图选择必须由应用根 Jotai store 在本次程序进程内跨路由保留；route unmount、关闭隐藏和单实例唤醒不清空，真正退出后恢复默认。此类状态不得写入浏览器/Tauri/文件/数据库或 URL 持久化，也不得镜像 TanStack Query 数据或 core 权威状态；语言、主题等已批准设备偏好仍按其独立契约持久化。查询范围或每页数量变化时页码回到 1；返回页面后只有成功查询得到空结果且当前页大于 1 时才回退第 1 页并重查，加载/错误和第 1 页空结果不得触发循环。
...
- GUI 初始化必须直接依赖官方 `tauri-plugin-single-instance`：根 `[workspace.dependencies]` 声明经过验证的最低兼容稳定范围，GUI member 只使用 `workspace = true`，并把该插件作为 Tauri Builder 的首个 plugin 注册。同一用户会话再次启动相同应用身份时，第二进程必须通知既有实例后退出，只调用共享 `restore_main_window` 恢复、取消最小化并聚焦既有 `main` 窗口，不得创建第二个长期应用主进程或主窗口。中性脚手架必须忽略且不得记录第二次启动的参数和工作目录，不把它们解释为产品动作；文件关联、深链或参数转发需另行批准并由 core 判定。必须包含 `single_instance_plugin_is_registered_first` 与 `second_launch_restores_existing_main_window` 两个固定命名回归；缺少依赖、首插件顺序、回调、回归或真实双启动唯一性证据都阻断初始化。Linux Snap/Flatpak 渠道还必须按官方插件要求声明会话 DBus own/talk 权限并在真实沙箱候选验证。
- GUI 初始化强制启用 Tauri `tray-icon`，系统托盘不是可选增强：托盘使用应用图标，菜单稳定 ID 恰好为 `show_window` 和 `quit`，但稳定 ID 不得成为可见文字；标签必须由 `rust_i18n::t!("tray.show_window")` 与 `rust_i18n::t!("tray.quit")` 按当前规范化 locale 解析，中文精确显示“显示窗口/退出”，英文精确显示“Show Window/Quit”，未知 locale 回退英文，运行时语言切换必须在不重启应用的情况下刷新既有托盘标签，任何 `tray.*` 原始键可见都失败。主鼠标左键释放或显示动作恢复、取消最小化并聚焦主窗口，主窗口关闭只 `prevent_close()` 后隐藏且进程继续，只有托盘退出结束应用，且不得默认加入自动启动。不得因 Draft、宿主差异、页面简单或实现偏好省略/降级托盘；结构检查、六个托盘生命周期/i18n 固定命名回归或真实宿主生命周期任一缺失都阻断初始化基线提交，当前宿主无法观察托盘同样按失败处理。初始化同时建立动态标题、`136px` 固定单态左侧菜单和 `/settings`、`/about`、`/sponsor`；不得提供展开/折叠状态或开关。侧栏顶部永远先显示 `56px` 选中 Logo、再紧接带一个小写 `v` 的权威当前版本；每个产品功能项和赞助/设置/关于固定项都必须提供 `30px` 图标，图标在上、名称在下，名称使用 `11px` 字号、`10em` 行内宽度、水平居中和最多两行，并保留完整可访问名称，不得用 Tooltip 代替可见名称。功能项从顶部向下增长，底部固定组按视觉顺序为赞助、设置、关于。主应用窗口使用 1440×900 逻辑像素、最小 960×640、居中且防止溢出，默认尺寸应同时容纳固定 `136px` 侧栏与三张赞助档位卡；它与 660×400 的 DMG 安装卷窗口独立。Mantine 根默认跟随系统主题并初始化完整的亮色/暗色语义主题，包括页面背景、surface、主/次文字、边框与强调色；设置页固定只提供应用/版本、中英文和浅色/深色/跟随系统三态选择并持久化设备级偏好，不得包含隐私标题、统计同意、未配置占位或对应固定翻译键。赞助页按运行时有效主题选择背景叠层、surface 与对比色，不得只生成初始化宿主当前主题。手动检查更新及 `NotConfigured`/禁用状态位于关于页；“检查更新”旁固定提供自身绑定的“更新日志”按钮，从候选内同一 `release-notes.json` 展示最新至多 5 个版本，每版“功能优化”和“问题修复”各至多 10 条，标题及分类格式固定且版本只带一个小写 `v`。关于页还显示作者、联系方式和三段免责声明，赞助页打包完整 sponsor 媒体。真实 updater/强更/统计传输启用时才创建受保护的 `docs/GUI_SUPPORT_SURFACES.md`：updater 必须使用官方签名制品且验证不可关闭，强更由 core 对 adapter 已认证的 `minimumSupportedVersion` 作 SemVer 判定；只有产品明确启用统计时才另建产品级明确同意界面，统计仅在同意后以 HTTPS JSON POST 发送固定最小字段，撤回时取消请求和清空有界内存队列。桌面 bundle 不得包含服务端 secret 或发布私钥；任务必须受应用生命周期拥有并回收，失败默认 fail-open。
- GUI 托盘图标可见性是上述硬门禁的一部分：项目本地 Tauri `icon` 命令产出的 `src-tauri/icons/32x32.png` 必须是普通非符号链接、32×32 8-bit RGBA 非交错 PNG 且至少含一个非透明像素，`tauri.conf.json` 的 `bundle.icon` 必须精确引用 `icons/32x32.png`。托盘安装函数必须从 Tauri Builder `.setup(...)` 实际调用，在同一实现中绑定双项 `Menu`、必需的 `default_window_icon()`、`.icon(...)` 并成功 `.build(app)`；图标缺失必须阻断启动，禁止无图标继续。关闭隐藏处理必须由 `.on_window_event(...)` 实际注册。结构检查或真实 E2E 只能定位空白点击区域、可弹菜单但没有可见图形时仍按失败处理；Linux 的托盘必须绑定菜单以满足 Tauri 官方可见性要求。
- GUI 图标固定使用直接依赖 `@tabler/icons-react` 的命名组件；菜单、操作、状态、空态和图表周边控件存在适用图标时优先从该包选择，不得引入其他图标库、手写 SVG、字符或 emoji。图表绘制能力仍按真实数据可视化需求选择。固定单态侧栏中的 Logo、所有当前渲染图标和下方文字必须沿同一中心线水平居中且无裁切。
- 含 GUI 的下游在初始化相关非空单元测试通过后、初始化能力裁剪和唯一基线提交前，必须固定调用一次 `$desktop-test-gui-initialization-e2e`：先运行 `verify-gui-lifecycle-contract.mjs`，对单实例依赖/首插件/回调、`tray-icon`、真实托盘源码、稳定 ID `show_window`/`quit`、`rust-i18n` 标签解析、中英文资源和八个有断言的固定命名回归失败关闭；再运行 `pnpm tauri build --debug --no-bundle` 并双启动本次真实本机调试二进制。第二次启动必须自行退出，既有主进程与同一主窗口继续存在且窗口恢复、取消最小化并聚焦，宿主枚举只剩一个长期应用主进程和一个主窗口。Computer Use 还必须验证真实系统托盘图标、中文“显示窗口/退出”与英文“Show Window/Quit”可在不重启时刷新且无 `tray.*` 原始键、原生关闭只隐藏且进程继续、托盘左键与本地化显示项均恢复并聚焦、本地化退出项结束进程并移除图标，以及可见主窗口、固定单态侧栏尺寸/排列/居中、默认设置页无隐私/统计区块和所有渲染菜单页面可达。该门禁不读取 `milestone_e2e`，失败/超时/取消/唯一性无法判定/无法观察/无法执行都阻断初始化；它不生成发布候选或 Verification，通过后该 Skill 与初始化能力一同删除。
...
- 每次正式发布的候选构建前，`$desktop-prepare-release` 必须从上一次真实正式发布版本/提交到当前发布源码整理用户可见变化，写入根 `release-notes.json`；首发从仓库起点计算，比较范围无法可靠界定时停止。每版固定按 `-----------更新日志 {发布日期} {发布版本}----------`、`###功能优化`、`###问题修复` 表达，两类各选至多 10 条最重要内容且合计至少一条，文件按最新在前只保留近 5 版。构建只读校验当前版本和 SHA-256并把同一字节打入候选，manifest 记录 `releaseNotesVersion`、`releaseNotesSha256`、`releaseNotesPath`；候选形成后不得补写，任何变化都必须重新提交、构建和验收。升级器必须把该文件视为 `protected`。
...
- 初始化 Rust 工具链、shared core、接口选择和四项持久 Agent 策略；选择 GUI 时同时完成三候选 Logo 选择、项目内 macOS DMG 背景、强制单实例/托盘结构与运行时 i18n、真实双启动/关闭隐藏/恢复/退出生命周期门禁、动态标题、`136px` 单态图标上文字下的 Logo→版本侧栏、无隐私/统计区块的语言与三态主题设置、应用级亮暗主题、含更新入口的关于页、双主题赞助页和一次真实本机初始化 E2E：使用 `$desktop-initialize-rust-project`。
...
- 含 GUI 的一次性项目初始化在基线提交前先检查强制单实例/托盘结构与 `rust-i18n` 标签解析，再构建并双启动真实本机调试二进制，证明第二次启动只唤醒同一主窗口后退出且只剩一个长期应用主进程/主窗口，再实测托盘中英文精确标签可无重启刷新、关闭隐藏、两种恢复与退出生命周期，并检查固定单态侧栏、设置页无隐私区块和全部菜单页面可达：使用 `$desktop-test-gui-initialization-e2e`；通过后从终端下游删除该 Skill。
...
| GUI 固定本地支持基线与可选出站事实 | `$desktop-prepare-gui-support-surfaces`；Skill 携带动态标题、`136px` 单态图标上文字下的 Logo→版本侧栏、无隐私/统计区块的语言与三态主题设置、全局亮色/暗色语义主题、含更新入口的关于页、双主题赞助页和完整产品家族品牌依赖资产，终端下游只在修改基线或增加能力时按需创建 `docs/GUI_SUPPORT_SURFACES.md`，Harness 不预创建或覆盖 |
| GUI 单实例、托盘、图标与初始化 E2E | 单实例和系统托盘都是不可省略的 GUI 初始化硬门禁；托盘必须有非透明 32px RGBA 资产/配置引用、从 `.setup` 可达的 Menu/icon/build 接线、`rust-i18n` 中英文/回退/运行时刷新和真实宿主可见非空图形，并通过真实双启动单一主进程/主窗口及关闭隐藏/恢复/退出生命周期；`@tabler/icons-react` 是固定图标库；`$desktop-test-gui-initialization-e2e` 只在含 GUI 的基线提交前执行这些检查并验证固定侧栏、默认设置页无隐私区块和全部菜单页面可达，通过后删除且升级不得恢复 |
