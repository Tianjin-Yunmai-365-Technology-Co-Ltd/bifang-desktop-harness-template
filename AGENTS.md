...
- 文件组织、中文业务注释、文档职责、测试组织和规则例外统一遵守 `docs/ENGINEERING_RULES.md`。人工或 Agent 维护的文本文件超过 500 个物理行时必须复核业务是否高内聚、职责单一且职责相近，不满足则重构拆分；超过 2000 行必须强制拆分且门禁失败。Rust 模块拆分使用目录/`mod.rs` 结构；工具生成锁文件/生成物和原样内嵌第三方文件按封闭范围定义处理。
...
- 每轮实现还必须运行 `.agents/skills/implement-change/scripts/check_file_line_limits.py`；检查器列出 501 至 2000 行的语义复核候选，2000 行通过、2001 行失败。该门禁适用于全部人工维护文本，不只检查本次改动文件；候选只有在确认高内聚、职责单一且职责相近后才能保留。
...
- 行为保持的结构性清理（单文件行数、文件组织结构、命名、常量提取、潜在性能与死锁风险、core-first 归属）：使用 `$refactor-code`。
- 已选 GUI 适配器中出现硬编码用户可见文案需要迁移为 i18n 翻译键：使用 `$extract-i18n-strings`；不触碰共享 core。
