...
- 安全/隐私、数据迁移、破坏性操作、生产/付费/凭据副作用、对外兼容契约、渠道硬要求、签名、发布和跨平台最终候选必须进入对应专用门禁；精简上下文不降低授权、失败关闭或人工复核要求。
...
- 已有远端的下游在新需求、Bug 或维护写入前通过 `$desktop-manage-git-branch-chain` 建立并登记串行 `feature-{ascii-kebab摘要}-{YYYYMMDD}` 链；日常闭环提交后推送活动叶子。每条活动叶子同一时刻只允许一个产品写入 Task，禁止 sibling 写入 Worktree，前一结果 fast-forward 整合并推送后才开始下一写入。`main`、`master`、动态远端默认分支和精确 `Release` 都是只读保护分支；明确发布只能把已复核链快进到 `Release` 并按登记精确清理链路，`Release` 到默认分支始终由用户自行 Merge/PR。
- 对产出物声称“完成”“可用”或“已验证”必须基于真实产物的可观察结果；Mock、stub、源码片段、占位页面、中性 scaffold 或开发预览不能冒充候选验收。人工批准也不能把失败或未执行改判为通过。
- Product Spec、ADR、Changelog、Product Status、Work Plan 与 Verification 只由各自独立事件触发；普通维护和构建过程不写占位记录。发现但不在当前范围的问题写入 `docs/TECH_DEBT.md`。
...
| 验证方式、候选证据与人工复核 | `docs/VERIFICATION.md` 与精确证据卷 | E2E、完整验收、发布或审计 |
