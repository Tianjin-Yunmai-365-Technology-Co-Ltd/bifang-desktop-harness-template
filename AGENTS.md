...
- 仅在代码或实现变更阶段，在任何实质编码前询问一次用户是否启用并行 Worktree + Subagent 模式；授权只对当前任务有效。产品定义、范围设计、实施计划设计、纯文档维护、纯问答、只读调查、验证复核、构建、发布准备、交付工作和避免即时损害所必需的安全处置不触发。用户拒绝、未答复或编码任务无法安全拆成至少两个独立写入范围时，使用单 Agent 当前工作树流程。
- 用户同意后使用 `$run-parallel-worktrees`：写入型 Subagent 各自使用独立 Git Worktree 和 `codex/` 分支，主 Agent 先公开子任务、文件所有权、依赖与验证边界，并在启动、阻塞、阶段完成、整合和验证节点向用户更新。写入前必须从精确单元 Worktree 调用 helper `guard` 并逐项声明写入目标；该门禁校验 cwd、Git 根、登记 Worktree、分支和符号链接解析后的目标边界，但不替代宿主全局 sandbox。主 Agent 必须同步等待全部必需结果，不得把未完成 Subagent 留作用户不可见的后台工作；重叠写入必须转为串行。
...
- 每轮开发必须执行非空单元测试和变更相关的必要验证；按风险选择格式、lint、静态检查、目标集成/契约测试和最小只读冒烟。检查失败时不得把结果延迟到下一轮，也不得把开发验证误报为发布就绪。
- Computer Use E2E 及同类重型、交互式或真实环境验收不得在日常开发、普通交付复核或发布链路修改中自动触发。只有用户发起最终产物构建或发布准备时，Agent 才在首次发布构建前逐项询问本次是否启用；选择只对当前任务有效，不得跨任务继承。产品规格或发布渠道明确要求的验收项必须标为必需项，不得提供跳过选项。
- 发布构建的不可跳过门禁仍包括基于当前源码的代码编译、非空单元测试、相关集成/契约验证、最终产物构建与存在性检查，以及真实最终产物的只读启动冒烟；任一失败都必须阻断打包、上传、产物收集、签名和发布。
- 用户启用手动验收后，先构建所需最终产物，再在任何打包、上传、产物收集、签名或发布动作前执行。失败、超时、取消或选择后未执行均阻断当前任务的后续打包，不得通过在同一任务中取消选择来绕过；未启用且没有产品或渠道硬要求时，记录 `Not run`、原因和风险后可以继续。
- 下游项目的发布或交付完成声明至少要求在 Agent 当前实际运行的系统中：代码成功编译、非空单元测试套件通过、相关集成/契约验证通过、约定的最终产物确实存在、产物通过启动冒烟测试。
...
- Windows、macOS 和 Linux 是兼容目标；开发轮次只验证变更相关的当前系统范围，发布/交付验收重新验证当前系统完整闭环。其他平台必须明确标记为 `Unverified`，不得声称已通过。
...
- 当前任务已明确批准并行 Worktree + Subagent 模式：使用 `$run-parallel-worktrees`。
- 用户要求交付验收、复核完成状态或准备发布：使用 `$verify-delivery`；它复核必需证据并记录未运行项，不得自行启用重型、交互式或真实环境验收。
...
- 仅当本次构建/发布任务中用户明确启用，或产品/渠道把它列为必需项时，对真实最终产物执行 Computer Use E2E：使用 `$test-final-artifact-e2e`。
...
| 并行协作授权、Worktree 隔离与前台状态 | `$run-parallel-worktrees` |
...
1. 读取相关事实来源并确认当前边界。
2. 仅在代码或实现变更阶段询问一次是否启用并行 Worktree + Subagent；按用户选择进入并行或单 Agent 流程，产品/范围设计、实施计划设计和其他非编码阶段直接按任务对应流程执行。
3. 检查工作区现状，保护既有修改。
4. 仅修改完成目标所需的最小文件集合。
5. 执行非空单元测试与风险相称的变更相关验证；只在用户发起构建或发布准备时执行不可跳过的发布基础闭环，并按当次选择执行发布构建后的手动验收。
6. 更新受影响的项目记忆。
7. 报告变更、证据、未执行的发布检查和下一步。
