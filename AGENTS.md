...
- 每个会修改仓库或执行交付工作的任务，在任何实质执行前必须询问一次用户是否启用并行 Worktree + Subagent 模式；授权只对当前任务有效。纯问答、只读调查和避免即时损害所必需的安全处置不触发。用户拒绝、未答复或任务无法安全拆成至少两个独立写入范围时，使用单 Agent 当前工作树流程。
...
1. 读取相关事实来源并确认当前边界。
2. 对适用任务询问一次是否启用并行 Worktree + Subagent；按用户选择进入并行或单 Agent 流程。
3. 检查工作区现状，保护既有修改。
4. 仅修改完成目标所需的最小文件集合。
